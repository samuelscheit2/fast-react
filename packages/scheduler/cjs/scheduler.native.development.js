...
const entrypoint = 'scheduler/index.native';
...
function createUnsupportedError(exportName) {
  const error = new Error(
    `[fast-react] ${entrypoint}.${exportName} was called, but this ` +
      `placeholder has no native scheduler runtime implementation yet. It ` +
...
function createUnsupportedFunction(exportName) {
  const fn = function fastReactSchedulerNativeUnimplementedPlaceholder() {
