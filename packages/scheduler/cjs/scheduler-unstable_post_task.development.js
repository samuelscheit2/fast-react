...
const entrypoint = 'scheduler/unstable_post_task';
...
function createUnsupportedError(exportName) {
  const error = new Error(
    `[fast-react] ${entrypoint}.${exportName} was called, but this ` +
      `placeholder has no postTask scheduler behavior implementation yet. It ` +
...
function createUnsupportedFunction(exportName) {
  const fn = function fastReactSchedulerPostTaskUnimplementedPlaceholder() {
