...
const privateRootCreateWorkLoopFinishedWorkPreflightId =
...
const privateRootCreateWorkLoopFinishedWorkPreflightMetadataId =
  'fast-react-test-renderer-root-work-loop-finished-work-preflight-metadata';
const privateRootCreateWorkLoopFinishedWorkPreflightMetadataStatus =
...
function getRejectedPrivateActUpdateLifecycleBoundaryReason(
  ...
  if (
    ...
    rootLifecycleExecutionEvidence.publicSchedulerAvailable === true ||
    rootLifecycleExecutionEvidence.compatibilityClaimed === true
  ) {
    return 'act-update-lifecycle-evidence-public-claim';
  }
...
function createPrivateCreateRouteAdmissionRecord(createRequest) {
  ...
  const workLoopFinishedWorkPreflight =
    createRootCreateWorkLoopFinishedWorkPreflightRecord(
      createRequest,
      ready,
      failureReason
    );
...
function createRootCreateWorkLoopFinishedWorkPreflightRecord(
  createRequest,
  ready,
  failureReason
) {
  const previousCurrent = freezeRecord({
    arenaId: 1,
    slot: 1,
    generation: 1
  });
  const finishedWork = freezeRecord({
    arenaId: 1,
    slot: 2,
    generation: 1
  });
  ...
  return freezeRecord({
    id: privateRootCreateWorkLoopFinishedWorkPreflightId,
    ...
    rootRequestSequence: createRequest.requestSequence,
    acceptedInputShape: 'HostComponentWithTextChild',
    supportedChildren: 'single-host-text-child',
    ...
    workLoopFinishedWorkMetadata: freezeRecord({
      metadataId: privateRootCreateWorkLoopFinishedWorkPreflightMetadataId,
      metadataStatus:
        privateRootCreateWorkLoopFinishedWorkPreflightMetadataStatus,
      acceptedWorker: 'worker-534-root-work-loop-finished-work-commit-handoff',
      acceptedRustModule: 'fast-react-test-renderer',
      renderPhaseApi:
        'TestRendererRoot::render_latest_scheduled_host_root_for_commit_handoff',
      renderPhaseRecord: 'HostRootRenderPhaseRecord',
      finishedWorkRecord: 'HostRootFinishedWorkPendingCommitRecordForCanary',
...
function createRootCreateExecutionEvidence(
  createRequest,
  workLoopFinishedWorkPreflight
) {
  return freezeRecord({
...
function consumeAcceptedRustRootCreateRouteAdmissionForRequest(
  ...
  const admission = getRootCreateRouteAdmissionForRootRequest(record);
  if (admission === null || admission.ready !== true) {
    throwInvalidRootRequest(
...
function assertAcceptedRustRootCreateRouteAdmissionMatchesRequest(
  ...
  if (
    diagnostic.id !== privateCreateRouteAdmissionRecordId ||
  ...
  const workLoop = diagnostic.workLoopFinishedWorkPreflight;
  const expectedWorkLoop = admission.workLoopFinishedWorkPreflight;
  if (
    !createRouteFiberHandlesEqual(
      workLoop.finishedWork,
      expectedWorkLoop.finishedWork
    ) ||
    workLoop.finishedWorkMatchesRenderPhase !== true
  ) {
    throwInvalidRootRequest(
      'Rust create-route admission finished work is stale.'
    );
  }
  if (
    workLoop.renderLanesBits !== expectedWorkLoop.renderLanesBits ||
    workLoop.remainingLanesBits !== expectedWorkLoop.remainingLanesBits ||
    workLoop.renderLanesBits !== 1 ||
    workLoop.remainingLanesBits !== 0
  ) {
    throwInvalidRootRequest(
      'Rust create-route admission finished lanes are stale.'
    );
  }
...
function consumeRootExecutionResult(record, result, handoff) {
  ...
  const privateCreateNativeBridgeHostOutputHandoffEvidence =
    readDiagnosticField(result, [
      'privateCreateNativeBridgeHostOutputHandoff',
      'createNativeBridgeHostOutputHandoff',
      'createHostOutputHandoff',
  ...
  const privateCreateNativeBridgeHostOutputHandoff =
    ...
    privateCreateNativeBridgeHostOutputHandoffEvidence !== undefined
      ? consumePrivateCreateNativeBridgeHostOutputHandoffForRequest(
          record,
          privateCreateNativeBridgeHostOutputHandoffEvidence
        )
      : null;
...
function createPrivateTestInstanceQueryBridgePreflightRecord(
  ...
  assertSourceOwnedTestInstanceQueryDiagnostics(
    findAllDiagnostic,
    findByDiagnostic
  );
...
function isAcceptedTestInstanceLifecycleEvidenceForRootRequest(
  ...
  if (
    ...
    !rootLifecycleExecutionEvidences.has(evidence) ||
    ...
    !isAcceptedTestInstanceLifecycleEvidenceSequencedForRootRequest(
      rootRequest,
      evidence
    )
  ) {
  ...
  return (
    ...
    evidence.sourceLifecycleRowsAccepted === true &&
    evidence.publicRootAvailable === false &&
    evidence.publicSerializationAvailable === false &&
    evidence.publicTestInstanceAvailable === false &&
    evidence.publicActAvailable === false &&
    evidence.publicSchedulerAvailable === false &&
...
function isAcceptedTestInstanceLifecycleEvidenceSequencedForRootRequest(
  rootRequest,
  evidence
) {
  const requestSequences = evidence.requestSequences;
  const create = evidence.create;
  const update = evidence.update;
  const unmount = evidence.unmount;
  return (
    ...
    unmount !== null &&
    typeof unmount === 'object' &&
    Object.isFrozen(requestSequences) &&
    Object.isFrozen(create) &&
    Object.isFrozen(update) &&
    Object.isFrozen(unmount) &&
    Array.isArray(evidence.operationEvidence) &&
    Object.isFrozen(evidence.operationEvidence) &&
    evidence.operationEvidence.length === 3 &&
    evidence.operationEvidence[0] === create &&
    evidence.operationEvidence[1] === update &&
    evidence.operationEvidence[2] === unmount &&
    create.operation === 'create' &&
    update.operation === 'update' &&
    unmount.operation === 'unmount' &&
    create.diagnosticName === privateRootLifecycleExecutionDiagnosticName &&
    update.diagnosticName === privateRootLifecycleExecutionDiagnosticName &&
    unmount.diagnosticName === privateRootLifecycleExecutionDiagnosticName &&
    create.status === privateRootLifecycleExecutionStatus &&
    update.status === privateRootLifecycleExecutionStatus &&
    unmount.status === privateRootLifecycleExecutionStatus &&
    create.sourceOwnedExecutionAccepted === true &&
    update.sourceOwnedExecutionAccepted === true &&
    unmount.sourceOwnedExecutionAccepted === true &&
    create.sourceExecutionRecordId ===
      privateCreateNativeBridgeHostOutputHandoffDiagnosticId &&
    update.sourceExecutionRecordId ===
      privateUpdateNativeBridgeAdmissionDiagnosticId &&
    unmount.sourceExecutionRecordId ===
      privateUnmountNativeBridgeAdmissionDiagnosticId &&
    requestSequences.create === create.requestSequence &&
    requestSequences.update === update.requestSequence &&
    requestSequences.unmount === unmount.requestSequence &&
    create.requestSequence < update.requestSequence &&
    update.requestSequence < unmount.requestSequence &&
    unmount.requestSequence === rootRequest.requestSequence &&
    create.rootId === rootRequest.rootId &&
    update.rootId === rootRequest.rootId &&
    unmount.rootId === rootRequest.rootId &&
    create.rootSequence === rootRequest.rootSequence &&
    update.rootSequence === rootRequest.rootSequence &&
    unmount.rootSequence === rootRequest.rootSequence
  );
}
...
function assertSourceOwnedTestInstanceQueryDiagnostics(
  findAllDiagnostic,
  findByDiagnostic
) {
  if (
    findAllDiagnostic !== privateTestInstanceFindAllPredicateDiagnostics ||
    findByDiagnostic !== privateTestInstanceFindByQueryDiagnostics
  ) {
    throwInvalidRootRequest(
      'Accepted private TestInstance query diagnostics must be source-owned records.'
    );
  }
}
...
function validatePrivateSerializationUnmountHandoffIdentity(
  ...
  assertPrivateSerializationRecord(
    publicSurface,
    admission,
    'Expected private unmount finished-work identity native bridge admission evidence.'
  );
  const cleanupHandoff = readPrivateSerializationField(admission, [
    'cleanupHandoff',
    'cleanup_handoff',
  ...
  const deletionHandoff =
    readPrivateSerializationField(admission, [
      'deletionCommitHandoff',
      'deletion_commit_handoff',
      'unmountDeletionCommitHandoff',
    ...
    readPrivateSerializationField(cleanupHandoff, [
      'deletionCommitHandoff',
      'deletion_commit_handoff'
    ]);
  const cleanup = normalizePrivateSerializationUnmountCleanupHandoff(
    publicSurface,
    identityRootRequest,
    cleanupHandoff
  );
  const deletion = normalizePrivateSerializationUnmountDeletionHandoff(
    publicSurface,
    identityRootRequest,
    deletionHandoff
  );
  ...
  if (
    ...
    readPrivateSerializationRequiredString(
      ...
      'admission update kind'
    ) !== identityRootRequest.updateKind
  ) {
    throwPrivateSerializationFinishedWorkIdentityError(
      publicSurface,
      'Private unmount finished-work identity native bridge admission does not match the private unmount request.'
    );
  }
  if (
    cleanup.deletionCommitHandoffId !== deletion.diagnosticId ||
    cleanup.hostNodeCleanupCount !== deletion.hostNodeCleanupCount ||
    cleanup.cleanupOrderRecordCount !== deletion.cleanupOrderRecordCount
  ) {
    throwPrivateSerializationFinishedWorkIdentityError(
      publicSurface,
      'Private unmount finished-work identity cleanup handoff does not match the deletion handoff.'
    );
  }

  return freezeRecord({
    deletionCommitHandoff: deletion,
    cleanupHandoff: cleanup,
    cleanupHandoffVariant:
      cleanup.refCleanupReturnCount === 0 &&
      cleanup.passiveDestroyCount === 0
        ? 'host-only'
        : 'ref-passive'
  });
}
