...
const privateRootDangerousHtmlTextResetCommitHandoffRecordType =
  'fast.react_dom.private_root_dangerous_html_text_reset_commit_handoff_record';
...
const privateHydrateRootPublicFacadeTargetClaimingPreflightRecordType =
...
const ROOT_BRIDGE_DANGEROUS_HTML_TEXT_RESET_COMMIT_APPLIED =
  'applied-private-root-dangerous-html-text-reset-fake-dom-commit';
...
const ROOT_BRIDGE_HYDRATE_ROOT_PUBLIC_FACADE_TARGET_CLAIMING_PREFLIGHTED =
...
const ROOT_BRIDGE_PUBLIC_FACADE_UNMOUNT_REF_CLEANUP_HANDLE_METADATA_ACCEPTED =
  'accepted-private-root-public-facade-unmount-ref-cleanup-handle-metadata';
const ROOT_BRIDGE_PUBLIC_FACADE_UNMOUNT_REF_CLEANUP_EXECUTION_ACCEPTED =
...
const ROOT_BRIDGE_PUBLIC_FACADE_UNMOUNT_PASSIVE_DESTROY_ORDERING_ACCEPTED =
...
const ROOT_BRIDGE_HYDRATE_ROOT_TARGET_CLAIMING_PREFLIGHT_ACCEPTED_CAPABILITIES =
  freezeArray([
    freezeRecord({
      id: 'hydrate-root-marker-listener-preflight-required',
      accepted: true,
      reason:
    ...
    freezeRecord({
      ...
      accepted: true,
      reason:
        'The private hydration gate linked one blocked replay target to its target-dispatch path without dispatching events.'
    }),
    freezeRecord({
      id: 'hydrate-root-target-claiming-canonical-evidence',
      accepted: true,
      reason:
        'The private target-claiming diagnostic was accepted only through canonical immutable WeakMap-backed hydration evidence.'
    }),
    freezeRecord({
      id: 'hydrate-root-target-claiming-state-unchanged',
      accepted: true,
      reason:
        'The preflight proved marker/listener state remained unchanged while collecting target-claiming diagnostics.'
    })
  ]);
const ROOT_BRIDGE_HYDRATE_ROOT_TARGET_CLAIMING_PREFLIGHT_BLOCKED_CAPABILITIES =
  ROOT_BRIDGE_HYDRATE_ROOT_PUBLIC_FACADE_PREFLIGHT_BLOCKED_CAPABILITIES;
const ROOT_BRIDGE_HYDRATE_ROOT_EVENT_REPLAY_PREFLIGHT_ACCEPTED_CAPABILITIES =
  freezeArray([
    freezeRecord({
      id: 'hydrate-root-marker-listener-preflight-required',
      accepted: true,
    ...
    freezeRecord({
      ...
      accepted: true,
      reason:
        'The private event replay preflight consumes accepted hydrateRoot target-claiming evidence for the same request.'
    }),
    freezeRecord({
      id: 'hydrate-root-replay-target-dispatch-execution-metadata',
      accepted: true,
      reason:
    ...
    freezeRecord({
      ...
      accepted: true,
      reason:
        'The preflight proved marker/listener state remained unchanged while validating blocked replay dispatch metadata.'
    })
  ]);
...
const ROOT_BRIDGE_HYDRATE_ROOT_TEXT_NODE_CLAIM_PATCH_ACCEPTED_CAPABILITIES =
  freezeArray([
    freezeRecord({
      id: 'hydrate-root-execution-preflight-required',
      accepted: true,
      reason:
    ...
    freezeRecord({
      ...
      accepted: true,
      reason:
        'The bridge executed the private hydration text-node claim patch record for one fake DOM text mismatch.'
    }),
    freezeRecord({
      id: 'hydrate-root-text-node-claim-patch-options-owned',
      accepted: true,
      reason:
        'The patch consumed the original hydrateRoot boundary and options object.'
    }),
    freezeRecord({
      id: 'hydrate-root-text-node-claim-patch-state-unchanged',
      accepted: true,
      reason:
        'The marker/listener state remained unchanged while applying the private fake text patch.'
    })
  ]);
const ROOT_BRIDGE_HYDRATE_ROOT_TEXT_NODE_CLAIM_PATCH_BLOCKED_CAPABILITIES =
  freezeArray([
    freezeRecord({
      id: 'public-hydrate-root-execution',
      blocked: true,
      reason:
        'The execution is private and does not call public hydrateRoot behavior.'
    }),
    freezeRecord({
      id: 'public-root-object',
      blocked: true,
    ...
    freezeRecord({
      ...
      blocked: true,
      reason:
        'No browser DOM mutation is admitted; only a private fake text node is patched.'
    }),
    freezeRecord({
      id: 'marker-writes',
      blocked: true,
    ...
    freezeRecord({
      ...
      blocked: true,
      reason:
        'Hydration root creation, host instance hydration, and replay remain blocked.'
    }),
    freezeRecord({
      id: 'events',
      blocked: true,
    ...
    freezeRecord({
      id: 'recoverable-error-callbacks',
      blocked: true,
      reason:
        'Recoverable errors are not queued and root callbacks are not invoked.'
    }),
    freezeRecord({
      id: 'compatibility-claims',
      blocked: true,
...
const ROOT_BRIDGE_DANGEROUS_HTML_TEXT_RESET_EXECUTION_ACCEPTED_CAPABILITIES =
  freezeArray([
    freezeRecord({
      id: 'root-commit-host-component-update-metadata',
      accepted: true,
    ...
    freezeRecord({
      id: 'fake-dom-dangerous-html-text-reset-mutation',
      accepted: true,
      reason:
        'Admitted dangerousHTML/text reset rows executed on a private fake-DOM HostComponent.'
    }),
    freezeRecord({
      id: 'latest-props-after-dangerous-html-text-reset-mutation',
      accepted: true,
      reason:
        'The component-tree latest-props map was published only after admitted fake-DOM mutation completed.'
    })
  ]);
const ROOT_BRIDGE_DANGEROUS_HTML_TEXT_RESET_EXECUTION_BLOCKED_CAPABILITIES =
  freezeArray([
    freezeRecord({
      id: 'public-root-execution',
      blocked: true,
    ...
    freezeRecord({
      id: 'public-text-content-compatibility',
      blocked: true,
      reason:
        'The fake-DOM text reset evidence does not claim public React DOM text-content compatibility.'
    }),
    freezeRecord({
      id: 'public-dangerous-html-compatibility',
      blocked: true,
      reason:
        'The fake-DOM dangerousHTML evidence does not claim public React DOM dangerousHTML compatibility.'
    }),
    freezeRecord({
      id: 'hydration',
      blocked: true,
...
const ROOT_BRIDGE_PUBLIC_FACADE_HOST_OUTPUT_UNMOUNT_REF_CLEANUP_PASSIVE_ORDER_ACCEPTED_CAPABILITIES =
  freezeArray([
    freezeRecord({
      id: 'root-unmount-ref-cleanup-execution',
      accepted: true,
      reason:
        'The private facade root.unmount cleanup consumed the accepted ref cleanup-return execution gate under explicit test control.'
    }),
    freezeRecord({
      id: 'root-unmount-passive-destroy-ordering-metadata',
      accepted: true,
      reason:
        'The private facade root.unmount cleanup consumed accepted deleted-subtree passive destroy ordering metadata without running passive callbacks.'
    }),
    freezeRecord({
      id: 'ref-cleanup-passive-destroy-before-host-cleanup-order',
      accepted: true,
      reason:
        'The private facade root.unmount cleanup recorded ref cleanup, passive destroy ordering metadata, then fake-DOM host cleanup.'
    })
  ]);
...
const ROOT_BRIDGE_PUBLIC_FACADE_HOST_OUTPUT_UNMOUNT_REF_CLEANUP_PASSIVE_ORDER_BLOCKED_CAPABILITIES =
  freezeArray([
    freezeRecord({
      id: 'public-root-execution',
      blocked: true,
    ...
    freezeRecord({
      ...
      blocked: true,
      reason:
        'No public reconciler scheduling, render, complete-work, deletion, ref, passive, or commit traversal is admitted.'
    }),
    freezeRecord({
      id: 'browser-dom-compatibility',
      blocked: true,
    ...
    freezeRecord({
      id: 'public-ref-compatibility',
      blocked: true,
      reason:
        'Ref cleanup execution is private test-control evidence only and does not enable public root ref behavior.'
    }),
    freezeRecord({
      id: 'passive-effect-execution',
      blocked: true,
...
const rootDangerousHtmlTextResetCommitHandoffPayloads = new WeakMap();
const rootDangerousHtmlTextResetCommitHandoffRecords = new WeakMap();
...
function createPrivateRootBridgeShell(options) {
  ...
  return Object.freeze({
    ...
    applyDangerousHtmlTextResetCommit(
      record,
      rootCommitHostComponentUpdateMetadata,
      diagnostic,
      options
    ) {
      return applyPrivateRootDangerousHtmlTextResetCommitWithBridge(
        bridgeState,
        record,
        rootCommitHostComponentUpdateMetadata,
        diagnostic,
...
function createPrivateHydrateRootPublicFacadePreflight(options) {
  ...
  const preflight = freezeRecord({
    ...
    preflightTargetClaiming(hydrateRootPreflightRecord, dispatchRecord, options) {
      const targetClaimingPreflight =
      ...
      preflightState.targetClaimingPreflightRecords.push(
        targetClaimingPreflight
      );
      return targetClaimingPreflight;
    },
    getHydrateRootTargetClaimingPreflightRecords() {
      return freezeArray(preflightState.targetClaimingPreflightRecords);
    },
    preflightEventReplay(targetClaimingPreflightRecord, options) {
      const eventReplayPreflight =
        createPrivateHydrateRootPublicFacadeEventReplayPreflightRecord(
          preflightState,
          targetClaimingPreflightRecord,
          options
        );
      preflightState.eventReplayPreflightRecords.push(
        eventReplayPreflight
      );
      return eventReplayPreflight;
    },
    ...
    postPreflightExecution(executionPreflightRecord, options) {
      const textNodeClaimPatchExecution =
        createPrivateHydrateRootPublicFacadeTextNodeClaimPatchExecutionRecord(
          preflightState,
          executionPreflightRecord,
          options
        );
      preflightState.textNodeClaimPatchExecutionRecords.push(
        textNodeClaimPatchExecution
      );
      return textNodeClaimPatchExecution;
    },
    getHydrateRootExecutionPreflightRecords() {
      return freezeArray(preflightState.executionPreflightRecords);
    },
    getHydrateRootTextNodeClaimPatchExecutionRecords() {
      return freezeArray(
        preflightState.textNodeClaimPatchExecutionRecords
      );
    }
  });
...
function createPrivateHydrateRootPublicFacadeTargetClaimingPreflightRecord(
  preflightState,
  hydrateRootPreflightRecord,
  dispatchRecord,
  options
) {
  const preflightRecordPayload =
    assertPrivateHydrateRootPublicFacadePreflightRecordForPreflight(
      hydrateRootPreflightRecord,
      preflightState
    );
  const requestRecord = preflightRecordPayload.requestRecord;
  const requestPayload = rootRecordPayloads.get(requestRecord);
  if (requestPayload === undefined) {
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot target-claiming preflight requires the private hydrateRoot request payload.'
    );
  }

  const markerListenerPayload =
    assertHydrateRootTargetClaimingMarkerListenerPreflight(
  ...
  const preflightOptions =
    normalizeHydrateRootTargetClaimingPreflightOptions(options);
  const beforeState =
    inspectPublicFacadeMarkerListenerPreflightState(container);
  if (!markerListenerStateMatches(markerListenerPayload.afterState, beforeState)) {
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot target-claiming preflight requires the current marker/listener state to match the accepted marker/listener preflight.'
    );
  }

  const hydrationBoundaryGate = requestPayload.bridgeState.hydrationBoundaryGate;
  const hydrationBoundaryRecord = requestRecord.hydrationBoundaryRecord;
  const targetDispatchLinkDiagnostic =
    createHydrationReplayTargetDispatchLinkDiagnostic(
      hydrationBoundaryRecord,
      dispatchRecord,
      preflightOptions.targetDispatchLinkOptions
    );
  const ownershipDiagnostics =
    createHydrationReplayOwnershipGateDiagnostic(
      hydrationBoundaryRecord,
      dispatchRecord,
      preflightOptions.ownershipOptions
    );
  const targetClaimingDiagnostic =
    hydrationBoundaryGate.createHydrationTargetClaimingDiagnostic(
      hydrationBoundaryRecord,
      ownershipDiagnostics,
      targetDispatchLinkDiagnostic,
      preflightOptions.targetClaimingOptions
    );
  const targetClaimingPayload =
    assertCanonicalPrivateHydrationTargetClaimingDiagnostic(
  ...
  assertHydrateRootTargetClaimingPreflightEvidenceBlocked({
    ownershipDiagnostics,
    targetClaimingDiagnostic,
    targetDispatchLinkDiagnostic
  });
  ...
  if (!stateUnchanged) {
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot target-claiming preflight state changed while collecting blocked diagnostics.'
    );
  }

  const targetClaimingPreflightSequence =
    preflightState.nextTargetClaimingPreflightSequence++;
  const targetClaimingPreflightId =
    `${hydrateRootPreflightRecord.preflightId}:target-claiming:${targetClaimingPreflightSequence}`;
  const preconditions = freezeRecord({
    accepted: true,
    stateUnchanged,
    ...
    targetClaimingPayloadAccepted: targetClaimingPayload !== null,
    targetDispatchLinkPayloadAccepted:
      targetClaimingPayload.targetDispatchLinkPayload !== null,
  ...
  const record = freezeRecord({
    ...
    operation: 'hydrate-root-target-claiming-preflight',
    facadeCall: 'hydrateRoot',
    entrypoint: 'react-dom/client',
    preflightId: hydrateRootPreflightRecord.preflightId,
    preflightSequence: hydrateRootPreflightRecord.preflightSequence,
    ...
    markerListenerPreflightId:
      preflightRecordPayload.markerListenerPreflight
        .markerListenerPreflightId,
    markerListenerPreflightStatus:
      preflightRecordPayload.markerListenerPreflight.preflightStatus,
    markerListenerPreconditionsAccepted:
      preflightRecordPayload.markerListenerPreflight.preconditions.accepted,
    markerListenerStateUnchanged:
      preflightRecordPayload.markerListenerPreflight.preconditions
        .stateUnchanged,
    ...
    blockerEvidence,
    targetDispatchLinkDiagnostic,
    targetDispatchLinkStatus: targetDispatchLinkDiagnostic.status,
    targetDispatchLinkAccepted: true,
    ownershipDiagnostics,
    ownershipDiagnosticStatus: ownershipDiagnostics.status,
    ownershipRetainedThroughDrainOrder:
      targetClaimingDiagnostic.ownershipRetainedThroughDrainOrder,
    targetClaimingDiagnostic,
    targetClaimingGateId: targetClaimingDiagnostic.gateId,
    targetClaimingStatus: targetClaimingDiagnostic.status,
    ...
    markerReplayTargetCandidate:
      targetClaimingDiagnostic.markerReplayTargetCandidate,
    ...
    targetPathParentPath:
      targetClaimingDiagnostic.targetPathParentPath,
    targetPathIndex: targetClaimingDiagnostic.targetPathIndex,
    targetPathRootIndex: targetClaimingDiagnostic.targetPathRootIndex,
    targetPathSegmentCount:
      targetClaimingDiagnostic.targetPathSegmentCount,
    targetPathSegments: targetClaimingDiagnostic.targetPathSegments,
    targetPathResolvedPath:
      targetClaimingDiagnostic.targetPathResolvedPath,
    ...
    targetPathDeterministicallySelected:
      targetClaimingDiagnostic.targetPathDeterministicallySelected,
    targetPathMatchCount:
      targetClaimingDiagnostic.targetPathMatchCount,
    targetPathMatchedPaths:
      targetClaimingDiagnostic.targetPathMatchedPaths,
...
function createPrivateHydrateRootPublicFacadeEventReplayPreflightRecord(
  preflightState,
  targetClaimingPreflightRecord,
  options
) {
  const targetClaimingPayload =
    assertPrivateHydrateRootPublicFacadeTargetClaimingPreflightRecordForPreflight(
      targetClaimingPreflightRecord,
      preflightState
    );
  const requestRecord = targetClaimingPayload.requestRecord;
  const requestPayload = rootRecordPayloads.get(requestRecord);
  if (requestPayload === undefined) {
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot event replay preflight requires the private hydrateRoot request payload.'
    );
  }
  ...
  const ownerDocument = getOwnerDocument(container);
  const preflightOptions =
    normalizeHydrateRootEventReplayPreflightOptions(options);
  const beforeState =
    inspectPublicFacadeMarkerListenerPreflightState(container);
  if (!markerListenerStateMatches(targetClaimingPayload.afterState, beforeState)) {
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot event replay preflight requires the current marker/listener state to match the accepted target-claiming preflight.'
    );
  }
  ...
  const replayExecutionRecord =
    hydrationBoundaryGate
      .createHydrationClaimedReplayTargetDispatchExecutionRecord(
        ...
        preflightOptions.executionOptions
      );
  const replayExecutionPayload =
    assertCanonicalPrivateHydrationClaimedReplayTargetDispatchExecutionRecord(
      replayExecutionRecord,
      {
        dispatchRecord: targetClaimingPayload.dispatchRecord,
        hydrationBoundaryRecord,
        targetClaimingDiagnostic:
          targetClaimingPayload.targetClaimingDiagnostic,
        targetDispatchLinkDiagnostic:
  ...
  if (
    replayExecutionPayload.targetClaimingDiagnosticPayload !==
      targetClaimingPayload.targetClaimingPayload ||
    replayExecutionPayload.targetDispatchLinkPayload !==
      targetClaimingPayload.targetClaimingPayload.targetDispatchLinkPayload
  ) {
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot event replay preflight requires replay metadata from the accepted target-claiming preflight.'
    );
  }
  ...
  if (!stateUnchanged) {
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot event replay preflight state changed while validating blocked replay dispatch metadata.'
    );
  }

  const eventReplayPreflightSequence =
  ...
  const preconditions = freezeRecord({
    ...
    targetClaimingStateUnchanged:
      targetClaimingPreflightRecord.preconditions.stateUnchanged,
  ...
  const record = freezeRecord({
    $$typeof: privateHydrateRootPublicFacadeEventReplayPreflightRecordType,
    kind:
      'FastReactDomPrivateHydrateRootPublicFacadeEventReplayPreflightRecord',
    operation: 'hydrate-root-event-replay-preflight',
    facadeCall: 'hydrateRoot',
    entrypoint: 'react-dom/client',
    ...
    markerListenerPreflightId:
      targetClaimingPayload.markerListenerPreflight
        .markerListenerPreflightId,
    markerListenerPreflightStatus:
      targetClaimingPayload.markerListenerPreflight.preflightStatus,
    markerListenerPreconditionsAccepted:
      targetClaimingPayload.markerListenerPreflight.preconditions.accepted,
    markerListenerStateUnchanged:
      targetClaimingPayload.markerListenerPreflight.preconditions
        .stateUnchanged,
    ...
    targetClaimingPreconditionsAccepted:
      targetClaimingPreflightRecord.preconditions.accepted,
    targetClaimingStateUnchanged:
      targetClaimingPreflightRecord.preconditions.stateUnchanged,
    markerGuard: requestRecord.markerGuard,
    listenerGuard: requestRecord.listenerGuard,
    ...
    targetDispatchLinkAccepted: true,
    ownershipDiagnostics: targetClaimingPreflightRecord.ownershipDiagnostics,
    ownershipDiagnosticStatus:
      targetClaimingPreflightRecord.ownershipDiagnosticStatus,
    ownershipRetainedThroughDrainOrder:
      targetClaimingPreflightRecord.ownershipRetainedThroughDrainOrder,
    targetClaimingDiagnostic:
      targetClaimingPayload.targetClaimingDiagnostic,
    targetClaimingGateId:
      targetClaimingPayload.targetClaimingDiagnostic.gateId,
...
function createPrivateHydrateRootPublicFacadeTextNodeClaimPatchExecutionRecord(
  ...
  const requestRecord = executionPayload.requestRecord;
  const requestPayload = rootRecordPayloads.get(requestRecord);
  if (requestPayload === undefined) {
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot text-node claim patch requires the private hydrateRoot request payload.'
    );
  }
  ...
  const beforeState =
    inspectPublicFacadeMarkerListenerPreflightState(container);
  if (!markerListenerStateMatches(executionPayload.afterState, beforeState)) {
    throwInvalidRootPublicFacadePreflight(
  ...
  if (!stateUnchanged) {
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot text-node claim patch changed marker/listener state while applying the private fake text patch.'
    );
  }

  const textNodeClaimPatchExecutionSequence =
    preflightState.nextTextNodeClaimPatchExecutionSequence++;
  const textNodeClaimPatchExecutionId =
    `${executionPreflightRecord.executionPreflightId}:text-node-claim-patch:${textNodeClaimPatchExecutionSequence}`;
  const preconditions = freezeRecord({
    accepted: true,
    stateUnchanged,
    ...
    replayExecutionPayloadAccepted:
      executionPreflightRecord.replayExecutionPayloadAccepted,
    canonicalTextNodeClaimPatchExecution: true,
    textNodeClaimPatchExecutionRecordImmutable:
      Object.isFrozen(textNodeClaimPatchExecutionRecord),
    hydrationOptionsOwned:
      textNodeClaimPatchExecutionPayload.hydrationOptions ===
      requestPayload.hydrationOptions,
    acceptedBoundaryMetadataConsumed:
      textNodeClaimPatchExecutionRecord
        .acceptedBoundaryMetadataConsumed,
    fakeTextNodeClaimed:
      textNodeClaimPatchExecutionRecord.fakeTextNodeClaimed,
    fakeTextNodePatched:
      textNodeClaimPatchExecutionRecord.fakeTextNodePatched,
    textPatchApplied:
      textNodeClaimPatchExecutionRecord.textPatchApplied,
    rootMarkerPropertyCount: beforeState.containerMarker.propertyCount,
    rootListeningMarkerPropertyCount:
      beforeState.rootListeningMarker.propertyCount,
    rootListenerRegistrationCount:
  ...
  const record = freezeRecord({
    ...
    operation: 'hydrate-root-text-node-claim-patch-execution',
    facadeCall: 'hydrateRoot',
    entrypoint: 'react-dom/client',
    preflightId: executionPreflightRecord.preflightId,
    preflightSequence: executionPreflightRecord.preflightSequence,
    targetClaimingPreflightId:
      executionPreflightRecord.targetClaimingPreflightId,
    targetClaimingPreflightSequence:
      executionPreflightRecord.targetClaimingPreflightSequence,
    eventReplayPreflightId:
      executionPreflightRecord.eventReplayPreflightId,
    eventReplayPreflightSequence:
      executionPreflightRecord.eventReplayPreflightSequence,
    executionPreflightId:
      executionPreflightRecord.executionPreflightId,
    executionPreflightSequence:
      executionPreflightRecord.executionPreflightSequence,
    textNodeClaimPatchExecutionId,
    textNodeClaimPatchExecutionSequence,
    preflightStatus:
      ROOT_BRIDGE_HYDRATE_ROOT_PUBLIC_FACADE_EXECUTION_PREFLIGHTED,
    executionStatus:
      ROOT_BRIDGE_HYDRATE_ROOT_PUBLIC_FACADE_TEXT_NODE_CLAIM_PATCH_EXECUTED,
    ...
    markerListenerPreflightId:
      executionPreflightRecord.markerListenerPreflightId,
    markerListenerPreflightStatus:
      executionPreflightRecord.markerListenerPreflightStatus,
    markerListenerStateUnchanged:
      executionPreflightRecord.markerListenerStateUnchanged,
    targetClaimingPreflight:
      executionPayload.targetClaimingPreflight,
    targetClaimingPreflightStatus:
      executionPreflightRecord.targetClaimingPreflightStatus,
    eventReplayPreflight:
      executionPayload.eventReplayPreflight,
    eventReplayPreflightStatus:
      executionPreflightRecord.eventReplayPreflightStatus,
    ...
    replayExecutionGateId:
      executionPreflightRecord.replayExecutionGateId,
    replayExecutionStatus:
      executionPreflightRecord.replayExecutionStatus,
    replayExecutionPayloadAccepted: true,
    textNodeClaimPatchExecutionRecord,
    textNodeClaimPatchExecutionKind:
      textNodeClaimPatchExecutionRecord.kind,
    textNodeClaimPatchExecutionGateId:
      textNodeClaimPatchExecutionRecord.gateId,
    textNodeClaimPatchExecutionMetadataId:
      textNodeClaimPatchExecutionRecord.metadataId,
    textNodeClaimPatchExecutionStatus:
      textNodeClaimPatchExecutionRecord.status,
    textNodeClaimPatchExecutionPayloadAccepted: true,
    sourceTextMismatchDiagnosticKind:
      textNodeClaimPatchExecutionRecord.sourceTextMismatchDiagnosticKind,
    sourceTextMismatchDiagnosticStatus:
      textNodeClaimPatchExecutionRecord.sourceTextMismatchDiagnosticStatus,
    sourceRecoverableErrorMetadataKind:
      textNodeClaimPatchExecutionRecord.sourceRecoverableErrorMetadataKind,
    sourceRecoverableErrorMetadataStatus:
      textNodeClaimPatchExecutionRecord.sourceRecoverableErrorMetadataStatus,
    acceptedPrivateMetadataDiagnostics:
      requestRecord.acceptedPrivateMetadataDiagnostics,
    acceptedPrivateMetadataIds:
      requestRecord.acceptedPrivateMetadataIds,
    acceptedPrivateMetadataGateIds:
      requestRecord.acceptedPrivateMetadataGateIds,
    acceptedBoundaryMetadataConsumed:
      textNodeClaimPatchExecutionRecord.acceptedBoundaryMetadataConsumed,
    acceptedBoundaryMetadataId:
      textNodeClaimPatchExecutionRecord.acceptedBoundaryMetadataId,
    acceptedBoundaryMetadataRow:
      textNodeClaimPatchExecutionRecord.acceptedBoundaryMetadataRow,
    recoverableErrorMetadata:
      textNodeClaimPatchExecutionRecord.recoverableErrorMetadata,
    recoverableErrorMetadataAccepted:
      textNodeClaimPatchExecutionRecord.recoverableErrorMetadataAccepted,
    textMismatchRowCount:
      textNodeClaimPatchExecutionRecord.textMismatchRowCount,
    patchedTextMismatchRowCount:
      textNodeClaimPatchExecutionRecord.patchedTextMismatchRowCount,
    textMismatchRow:
      textNodeClaimPatchExecutionRecord.textMismatchRow,
    textMismatchRowId:
      textNodeClaimPatchExecutionRecord.textMismatchRowId,
    textMismatchReason:
      textNodeClaimPatchExecutionRecord.textMismatchReason,
    expectedPath: textNodeClaimPatchExecutionRecord.expectedPath,
    actualPath: textNodeClaimPatchExecutionRecord.actualPath,
    expectedText: textNodeClaimPatchExecutionRecord.expectedText,
    actualTextBefore:
      textNodeClaimPatchExecutionRecord.actualTextBefore,
    actualTextAfter:
      textNodeClaimPatchExecutionRecord.actualTextAfter,
    normalizedTextMatchesBefore:
      textNodeClaimPatchExecutionRecord.normalizedTextMatchesBefore,
    normalizedTextMatchesAfter:
      textNodeClaimPatchExecutionRecord.normalizedTextMatchesAfter,
    claimedTextNodeInfo:
      textNodeClaimPatchExecutionRecord.claimedTextNodeInfo,
    claimedTextNodePath:
      textNodeClaimPatchExecutionRecord.claimedTextNodePath,
    claimedTextNodePathStatus:
      textNodeClaimPatchExecutionRecord.claimedTextNodePathStatus,
    claimedTextNodeParentPath:
      textNodeClaimPatchExecutionRecord.claimedTextNodeParentPath,
    claimedTextNodeIndex:
      textNodeClaimPatchExecutionRecord.claimedTextNodeIndex,
    claimedTextNodeSegmentCount:
      textNodeClaimPatchExecutionRecord.claimedTextNodeSegmentCount,
    fakeTextNodeMarkerAccepted:
      textNodeClaimPatchExecutionRecord.fakeTextNodeMarkerAccepted,
    fakeTextNodeConstructorName:
      textNodeClaimPatchExecutionRecord.fakeTextNodeConstructorName,
    patchWriteProperty:
      textNodeClaimPatchExecutionRecord.patchWriteProperty,
    patchWriteCount:
      textNodeClaimPatchExecutionRecord.patchWriteCount,
    patchWrites: textNodeClaimPatchExecutionRecord.patchWrites,
    claimLabel: textNodeClaimPatchExecutionRecord.claimLabel,
    privateExecutionPreflight: false,
    ...
    textPatched: true,
    targetClaimAccepted:
      executionPreflightRecord.targetClaimAccepted,
    targetClaimExecuted: false,
    publicHydrationTargetClaimed: false,
    targetDispatchExecuted: false,
    eventReplayDispatchAttempted: false,
    ...
    recoverableErrorsQueued: false,
    onRecoverableErrorConfigured:
      executionPreflightRecord.onRecoverableErrorConfigured,
    onRecoverableErrorInvoked: false,
    publicOnRecoverableErrorInvoked: false,
    rootErrorCallbacksInvoked: false,
    publicRootErrorCallbacksInvoked: false,
  ...
  rootHydratePublicFacadeTextNodeClaimPatchExecutionPayloads.set(
    record,
    freezeRecord({
      afterState,
      beforeState,
      ...
      bridge: preflightState.bridge,
      container,
      eventReplayPayload: executionPayload.eventReplayPayload,
      ...
      preflight: preflightState.preflight,
      requestRecord,
      targetClaimingPayload: executionPayload.targetClaimingPayload,
      targetClaimingPreflight:
        executionPayload.targetClaimingPreflight,
      textNodeClaimPatchExecutionPayload,
      textNodeClaimPatchExecutionRecord,
...
function assertPrivateHydrateRootPublicFacadePreflightRecordForPreflight(
  record,
  preflightState
) {
  ...
  if (
    ...
    record.$$typeof !== privateHydrateRootPublicFacadePreflightRecordType ||
    record.kind !== 'FastReactDomPrivateHydrateRootPublicFacadePreflightRecord' ||
    ...
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot target-claiming preflight requires a private hydrateRoot public-facade preflight record with accepted marker/listener diagnostics.'
    );
  }
...
function assertHydrateRootTargetClaimingMarkerListenerPreflight(
  markerListenerPreflight,
  requestRecord,
  preflightState
) {
  const payload =
    rootHydratePublicFacadeMarkerListenerPreflightPayloads.get(
      markerListenerPreflight
    );
  if (
    payload === undefined ||
    ...
    markerListenerPreflight.$$typeof !==
      privateHydrateRootPublicFacadeMarkerListenerPreflightRecordType ||
    markerListenerPreflight.preflightStatus !==
      ROOT_BRIDGE_HYDRATE_ROOT_PUBLIC_FACADE_MARKER_LISTENER_PREFLIGHTED ||
    ...
    markerListenerPreflight.preconditions.listenerGuardMatchesContainerState !==
      true ||
    markerListenerPreflight.blockerEvidence.rootMarkerWriteBlocked !== true ||
    markerListenerPreflight.blockerEvidence.rootListenerInstallationBlocked !==
      true ||
    markerListenerPreflight.blockerEvidence.publicHydrateRootBlocked !== true ||
    markerListenerPreflight.markerWrites !== false ||
    markerListenerPreflight.listenerInstallation !== false ||
    markerListenerPreflight.hydration !== false ||
    markerListenerPreflight.eventDispatch !== false ||
    markerListenerPreflight.compatibilityClaimed !== false
  ) {
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot target-claiming preflight requires the accepted marker/listener preflight gate.'
    );
  }
...
function assertPrivateHydrateRootPublicFacadeTargetClaimingPreflightRecordForPreflight(
  ...
  const payload =
    rootHydratePublicFacadeTargetClaimingPreflightPayloads.get(record);
  if (
    payload === undefined ||
    payload.preflight !== preflightState.preflight ||
    payload.bridge !== preflightState.bridge ||
    ...
    record.targetClaimingPayloadAccepted !== true ||
    ...
    payload.targetClaimingPayload === null ||
    payload.targetClaimingPayload === undefined ||
    payload.targetClaimingPayload.targetDispatchLinkPayload === null ||
    payload.targetClaimingPayload.targetDispatchLinkPayload === undefined
  ) {
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot event replay preflight requires an accepted private hydrateRoot target-claiming preflight record.'
    );
  }
...
function assertPrivateHydrateRootPublicFacadeExecutionPreflightRecordForTextNodeClaimPatch(
  record,
  preflightState
) {
  const payload =
  ...
  if (
    ...
    record.privateExecutionPreflight !== true ||
    record.privateExecution !== false ||
    record.preconditions.accepted !== true ||
    record.preconditions.stateUnchanged !== true ||
    ...
    record.compatibilityClaimed !== false ||
    record.rootRecordId !==
      payload.requestRecord.hydrationBoundaryRecord.recordId
  ) {
    if (hasHydrateRootExecutionPreflightPublicClaim(record)) {
      throwInvalidRootPublicFacadePreflight(
        'hydrateRoot text-node claim patch cannot accept public hydration, replay, dispatch, callbacks, execution, or compatibility claims.'
      );
    }
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot text-node claim patch requires an accepted private hydrateRoot execution preflight record.'
    );
  }
...
function normalizeHydrateRootTextNodeClaimPatchExecutionOptions(
  options,
  hydrationOptions
) {
  ...
  if (
    ...
    normalizedOptions.hydrationOptions !== hydrationOptions
  ) {
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot text-node claim patch requires the current hydrateRoot options object.'
    );
  }
  ...
  const source =
    typeof normalizedOptions.source === 'string'
      ...
      : 'private-hydrate-root-text-node-claim-patch-execution';
  const textNodeClaimPatchOptions = {
    enableTextNodeClaimPatchExecution: true,
    hydrationOptions,
    source: `${source}:hydration-text-node-claim-patch`
  };
  if (
    Object.prototype.hasOwnProperty.call(normalizedOptions, 'mismatchRow')
  ) {
    textNodeClaimPatchOptions.mismatchRow = normalizedOptions.mismatchRow;
  }
  if (typeof normalizedOptions.claimLabel === 'string') {
    textNodeClaimPatchOptions.claimLabel = normalizedOptions.claimLabel;
  }

  return freezeRecord({
    hasMismatchRow: Object.prototype.hasOwnProperty.call(
      normalizedOptions,
      'mismatchRow'
    ),
    hydrationOptions,
    ...
    rawOptions: options,
    source,
    textNodeClaimPatchOptions: freezeRecord(textNodeClaimPatchOptions)
  });
}

function assertHydrateRootTextNodeClaimPatchExecutionRecord(
  record,
  executionPayload,
  requestRecord,
  requestPayload,
  ...
  const payload =
    getPrivateHydrationTextNodeClaimPatchExecutionPayload(record);
  if (
    payload === null ||
    !isPrivateHydrationTextNodeClaimPatchExecutionRecord(record) ||
    record.kind !== HYDRATION_TEXT_NODE_CLAIM_PATCH_EXECUTION_RECORD_KIND ||
    record.gateId !== privateHydrationTextNodeClaimPatchExecutionGateId ||
    record.metadataId !== privateHydrationTextNodeClaimPatchMetadataId ||
    record.status !== privateHydrationTextNodeClaimPatchExecutionStatus ||
    record.executionStatus !==
      privateHydrationTextNodeClaimPatchExecutionStatus ||
    record.operation !== 'hydration-text-node-claim-patch-execution' ||
    record.privateExecution !== true ||
    record.fakeDomOnly !== true ||
    record.fakeDomMutation !== true ||
    record.fakeTextNodeClaimed !== true ||
    record.fakeTextNodePatched !== true ||
    record.textNodeClaimRecorded !== true ||
    record.textNodeClaimExecuted !== true ||
    record.textPatchAdmitted !== true ||
    record.textPatchApplied !== true ||
    record.textPatched !== true ||
    record.rootRecordId !== requestRecord.hydrationBoundaryRecord.recordId ||
    record.acceptedBoundaryMetadataDiagnostics !==
      requestRecord.acceptedPrivateMetadataDiagnostics ||
    record.recoverableErrorMetadata !==
      requestRecord.recoverableErrorMetadata ||
    record.sourceTextMismatchDiagnosticKind !==
      HYDRATION_TEXT_MISMATCH_DIAGNOSTIC_KIND ||
    record.sourceRecoverableErrorMetadataKind !==
      HYDRATION_TEXT_MISMATCH_RECOVERABLE_ERROR_METADATA_KIND ||
    record.recoverableErrorMetadataAccepted !== true ||
    record.patchedTextMismatchRowCount !== 1 ||
    record.patchWriteCount !== 1 ||
    record.exactTextMatchesAfter !== true ||
    record.normalizedTextMatchesAfter !== true ||
    payload.hydrationBoundaryRecord !==
      requestRecord.hydrationBoundaryRecord ||
    payload.acceptedBoundaryMetadataDiagnostics !==
      requestRecord.acceptedPrivateMetadataDiagnostics ||
    payload.hydrationOptions !== requestPayload.hydrationOptions ||
    payload.initialChildren !== requestPayload.initialChildren ||
    payload.recoverableErrorMetadata !==
      requestRecord.recoverableErrorMetadata ||
    payload.textMismatchDiagnostics !==
      requestRecord.textMismatchDiagnostics ||
    (executionOptions.hasMismatchRow &&
      payload.mismatchRow !== executionOptions.mismatchRow) ||
    executionPayload.requestRecord !== requestRecord
  ) {
    if (hasHydrateRootTextNodeClaimPatchPublicClaim(record)) {
      throwInvalidRootPublicFacadePreflight(
        'hydrateRoot text-node claim patch cannot accept public hydration, DOM, callback, execution, or compatibility claims.'
      );
    }
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot text-node claim patch requires canonical private text patch execution for the accepted hydrateRoot boundary.'
    );
  }
...
function normalizeHydrateRootTargetClaimingPreflightOptions(options) {
  ...
  const source =
    typeof normalizedOptions.source === 'string'
      ? normalizedOptions.source
      : 'private-hydrate-root-target-claiming-preflight';
  const targetDispatchLinkOptions = {
    source: `${source}:target-dispatch-link`
  };
  if (
    Object.prototype.hasOwnProperty.call(
      normalizedOptions,
    ...
    targetDispatchLinkOptions.targetDispatchPathRecord =
      normalizedOptions.targetDispatchPathRecord;
  }

  const targetClaimingOptions = {
    source: `${source}:target-claiming`
  };
  if (
    Object.prototype.hasOwnProperty.call(normalizedOptions, 'markerRow')
  ) {
    targetClaimingOptions.markerRow = normalizedOptions.markerRow;
  }

  return freezeRecord({
    ownershipOptions: freezeRecord({
      source: `${source}:ownership`
    }),
    rawOptions: options,
    source,
    targetClaimingOptions: freezeRecord(targetClaimingOptions),
    targetDispatchLinkOptions: freezeRecord(targetDispatchLinkOptions)
  });
}

function normalizeHydrateRootEventReplayPreflightOptions(options) {
  const normalizedOptions = isObjectOrFunction(options) ? options : {};
  const source =
    typeof normalizedOptions.source === 'string'
      ? normalizedOptions.source
      : 'private-hydrate-root-event-replay-preflight';
  const executionOptions = {
    source: `${source}:claimed-replay-target-dispatch-execution`
  };
  if (
    Object.prototype.hasOwnProperty.call(
      normalizedOptions,
      'clickReplayDispatchSource'
    )
  ) {
    executionOptions.clickReplayDispatchSource =
...
function assertHydrateRootTargetClaimingPreflightEvidenceBlocked({
  ownershipDiagnostics,
  targetClaimingDiagnostic,
  targetDispatchLinkDiagnostic
}) {
  if (
    ...
    targetDispatchLinkDiagnostic.compatibilityClaimed !== false ||
    targetDispatchLinkDiagnostic.browserDomEventCompatibilityClaimed !==
      false ||
    targetDispatchLinkDiagnostic.publicRootBehaviorChanged !== false ||
    targetDispatchLinkDiagnostic.eventReplaySupported !== false ||
    targetDispatchLinkDiagnostic.hydrationReplaySupported !== false ||
    targetDispatchLinkDiagnostic.queueMutationAllowed !== false ||
    targetDispatchLinkDiagnostic.replayQueuesDrained !== false ||
    targetDispatchLinkDiagnostic.eventDispatch !== false ||
    targetDispatchLinkDiagnostic.publicDispatchEnabled !== false ||
    targetDispatchLinkDiagnostic.willDispatch !== false ||
    targetDispatchLinkDiagnostic.willHydrate !== false ||
    targetDispatchLinkDiagnostic.willReplay !== false ||
    targetClaimingDiagnostic.compatibilityClaimed !== false ||
    targetClaimingDiagnostic.browserDomEventCompatibilityClaimed !== false ||
    targetClaimingDiagnostic.publicRootBehaviorChanged !== false ||
    targetClaimingDiagnostic.publicHydrationCompatibilityClaimed !== false ||
    targetClaimingDiagnostic.publicHydrationReplayCompatibilityClaimed !==
      false ||
    targetClaimingDiagnostic.eventReplaySupported !== false ||
    targetClaimingDiagnostic.hydrationReplaySupported !== false ||
    targetClaimingDiagnostic.eventDispatch !== false ||
...
function assertHydrateRootEventReplayPreflightEvidenceBlocked(
  ...
  if (
    replayExecutionRecord.compatibilityClaimed !== false ||
    replayExecutionRecord.browserDomEventCompatibilityClaimed !== false ||
    replayExecutionRecord.publicRootBehaviorChanged !== false ||
    replayExecutionRecord.publicHydrationCompatibilityClaimed !== false ||
    replayExecutionRecord.publicHydrationReplayCompatibilityClaimed !==
      false ||
    replayExecutionRecord.publicHydrationTargetClaimed !== false ||
    replayExecutionRecord.publicHydrateRootSupported !== false ||
    replayExecutionRecord.publicDispatchEnabled !== false ||
    replayExecutionRecord.eventReplayInstalled !== false ||
    replayExecutionRecord.eventReplaySupported !== false ||
    replayExecutionRecord.hydrationReplaySupported !== false ||
    replayExecutionRecord.queueMutationAllowed !== false ||
    replayExecutionRecord.replayQueuesDrained !== false ||
    replayExecutionRecord.eventsReplayed !== false ||
    replayExecutionRecord.eventDispatch !== false ||
    replayExecutionRecord.willDispatch !== false ||
    replayExecutionRecord.willHydrate !== false ||
    replayExecutionRecord.willReplay !== false ||
    replayExecutionRecord.targetClaimExecuted !== false ||
    replayExecutionRecord.targetDispatchExecuted !== false ||
    replayExecutionRecord.eventReplayDispatchAttempted !== false ||
    replayExecutionRecord.pluginDispatchEventForPluginEventSystemCalled !==
      false ||
    replayExecutionRecord.nativeEventRedispatched !== false ||
    replayExecutionRecord.syntheticEventCreated !== false ||
    replayExecutionRecord.listenerInvocationCount !== 0 ||
    replayExecutionRecord.willInvokeListeners !== false ||
    replayExecutionRecord.hydrateInstanceCalled !== false ||
    replayExecutionRecord.hydrateTextInstanceCalled !== false ||
    replayExecutionRecord.replayQueueDrained !== false ||
    replayExecutionRecord.queued !== false ||
    ...
    throwInvalidRootPublicFacadePreflight(
      'hydrateRoot event replay preflight cannot accept public hydration, replay, dispatch, callbacks, or compatibility claims.'
    );
  }
  ...
  if (
    ...
    (clickReplayDispatchDiagnostic.publicDispatchEnabled !== false ||
      clickReplayDispatchDiagnostic.liveEventListenerInstalled !== false ||
      clickReplayDispatchDiagnostic.rootListenerInstallationAttempted !==
        false ||
      clickReplayDispatchDiagnostic.eventReplayInstalled !== false ||
      clickReplayDispatchDiagnostic.eventDispatch !== false ||
      clickReplayDispatchDiagnostic.targetDispatchExecuted !== false ||
      clickReplayDispatchDiagnostic.eventReplayDispatchAttempted !== false ||
      clickReplayDispatchDiagnostic.privateClickDelegationDispatchGateCalled !==
        false ||
      clickReplayDispatchDiagnostic.syntheticEventCreated !== false ||
      clickReplayDispatchDiagnostic.listenerInvocationCount !== 0 ||
      clickReplayDispatchDiagnostic.willInvokeListeners !== false)
  ) {
    throwInvalidRootPublicFacadePreflight(
...
function hasHydrateRootTextNodeClaimPatchPublicClaim(record) {
  if (!isObjectOrFunction(record)) {
    return false;
  }
      ...
      value: function unmount() {
        ...
        if (preparedUnmount !== null) {
          assertPrivateRootPublicFacadeRootUnmountLifecycleExecutionMatchesRecord(
            preparedUnmount.lifecycleExecution,
            record
          );
        }
...
function createPrivateRootPublicFacadeHostOutputUnmountCleanupDiagnostic({
  ...
  const diagnosticRecord = freezeRecord({
    ...
    unmountRefCleanupExecutionAccepted:
      unmountRefPassiveEvidence === null
        ? false
        : unmountRefPassiveEvidence.refCleanupExecutionAccepted,
    unmountPassiveDestroyOrderingAccepted:
      unmountRefPassiveEvidence === null
        ? false
        : unmountRefPassiveEvidence.passiveDestroyOrderingAccepted,
    ...
    unmountRefCleanupPassiveDestroyBeforeHostCleanup:
      unmountRefPassiveEvidence === null
        ? false
        : unmountRefPassiveEvidence.refCleanupPassiveDestroyBeforeHostCleanup,
    acceptedCapabilities:
    ...
    eventDispatch: false,
    refEffects: false,
    privateRefCleanupExecution:
      unmountRefPassiveEvidence !== null &&
      unmountRefPassiveEvidence.refCleanupExecutionAccepted === true,
...
function assertPrivateRootPublicFacadeRootUnmountLifecycleExecutionRecord({
  ...
  if (
    ...
    record.sourceOwned !== true ||
    record.requestBoundaryCurrent !== true ||
...
function createPrivateRootPublicFacadeUnmountRefPassiveEvidence({
  ...
  const refContext = createPublicFacadeUnmountRefEvidenceContext({
    hostOutputPayload,
    payload,
    ref,
    unmountRecord
  });
  const consumeRefCleanupExecution =
    shouldConsumePublicFacadeUnmountRefCleanupExecution(
      passiveDestroyDescriptor
    );
  const initialRefAttachEvidence = consumeRefCleanupExecution
    ? createPublicFacadeUnmountInitialRefAttachEvidence({
        hostOutputPayload,
        payload,
        ref,
        refContext
  ...
  const refCleanupExecutionEvidence = consumeRefCleanupExecution
    ? createPublicFacadeUnmountRefCleanupExecutionEvidence({
        initialRefAttachEvidence,
        refContext,
        refDetachEvidence
      })
    : null;
  const passiveDestroyEvidence =
    createPublicFacadeUnmountPassiveDestroyEvidence(passiveDestroyDescriptor, {
      requiresOrderingMetadata: consumeRefCleanupExecution
    });
  const passiveDestroyOrderingAccepted =
    passiveDestroyEvidence.destroyOrderingMetadataAccepted === true;
  const refCleanupPassiveDestroyBeforeHostCleanup =
    refCleanupExecutionEvidence !== null &&
    passiveDestroyOrderingAccepted === true &&
    passiveDestroyEvidence.refCleanupBeforePassiveDestroy === true &&
    passiveDestroyEvidence.passiveDestroyBeforeHostCleanup === true;
  ...
  return freezeRecord({
    ...
    refCleanupExecutionAccepted: refCleanupExecutionEvidence !== null,
    passiveDestroyOrderingAccepted,
    beforeHostCleanup: true,
    refCleanupPassiveDestroyBeforeHostCleanup,
    order:
      refCleanupExecutionEvidence === null
        ? freezeArray([
...
function shouldConsumePublicFacadeUnmountRefCleanupExecution(descriptor) {
  return (
    descriptor !== null &&
    typeof descriptor === 'object' &&
    descriptor.consumeRefCleanupExecution === true
  );
}

function createPublicFacadeUnmountRefEvidenceContext({
  hostOutputPayload,
  payload,
  ref,
  unmountRecord
  ...
  if (hostOwner === null || rootOwner !== payload.createRecord.owner) {
    throwInvalidRootPublicFacadeHostOutputUnmount(
      'Private public-facade root.unmount ref evidence requires mounted host ownership metadata.'
    );
  }
  ...
  return {
    ...
    hostNode: hostOutputPayload.hostNode,
    refHandle: freezeRecord({
...
function createPublicFacadeUnmountInitialRefAttachEvidence({
  hostOutputPayload,
  payload,
  ref,
  refContext
  ...
  if (
    hostOutputPayload.renderRecord === undefined ||
    hostOutputPayload.renderRecord === null
  ) {
    throwInvalidRootPublicFacadeHostOutputUnmount(
      'Private public-facade root.unmount ref cleanup evidence requires the source render request record.'
    );
  }

  const attachRecord = refCallbackGate.createRefAttachMetadataRecord({
    rootOwner: refContext.rootOwner,
    hostOwner: refContext.hostOwner,
    hostInstanceToken: hostOutputPayload.hostToken,
    fiber: freezeRecord({
      kind: 'FastReactDomPrivateRootPublicFacadeUnmountInitialRefFiber',
      rootId: hostOutputPayload.renderRecord.rootId,
      renderRequestId: hostOutputPayload.renderRecord.requestId
    }),
    stateNode: hostOutputPayload.hostNode,
    refHandle: refContext.refHandle,
    ref,
    sourceToken: `${hostOutputPayload.renderRecord.requestId}:public-facade-unmount-ref-cleanup-handle`,
  ...
  const metadataRecord = payload.bridge.admitRootCommitRefMetadata(
    hostOutputPayload.renderRecord,
    {
      detach: [],
      attach: [attachRecord]
    },
    {label: 'public-facade-root-unmount-ref-cleanup-handle'}
  );

  if (
    metadataRecord.metadataStatus !==
      ROOT_BRIDGE_ROOT_COMMIT_REF_METADATA_ACCEPTED ||
    metadataRecord.hostOutputCanary !== 'initial-host-output' ||
    metadataRecord.attachCount !== 1 ||
    metadataRecord.detachCount !== 0
  ) {
    throwInvalidRootPublicFacadeHostOutputUnmount(
      'Private public-facade root.unmount ref cleanup evidence requires accepted initial attach metadata.'
    );
  }

  return freezeRecord({
    kind: 'FastReactDomPrivateRootPublicFacadeUnmountInitialRefAttachEvidence',
    status:
...
function createPublicFacadeUnmountRefDetachEvidence({
  ...
  const detachRecord = refCallbackGate.createRefDetachMetadataRecord({
    rootOwner: refContext.rootOwner,
    hostOwner: refContext.hostOwner,
    hostInstanceToken: refContext.hostInstanceToken,
    fiber: freezeRecord({
      kind: 'FastReactDomPrivateRootPublicFacadeUnmountRefFiber',
      rootId: unmountRecord.rootId,
      unmountRequestId: unmountRecord.requestId
    }),
    stateNode: refContext.hostNode,
    refHandle: refContext.refHandle,
    ref,
...
function createPublicFacadeUnmountRefCleanupExecutionEvidence({
  initialRefAttachEvidence,
  refContext,
  refDetachEvidence
}) {
  if (
    initialRefAttachEvidence === null ||
    initialRefAttachEvidence.attachRecord == null ||
    refDetachEvidence.detachRecord == null
  ) {
    throwInvalidRootPublicFacadeHostOutputUnmount(
      'Private public-facade root.unmount ref cleanup execution requires accepted attach and detach metadata.'
    );
  }
  ...
  const executionSnapshot =
    refCallbackGate.createRefCallbackCleanupReturnExecutionGateSnapshot({
      steps: [
        {
          label: 'root-unmount-records-ref-cleanup-handle',
          rootCommitRefMetadata: {
            detach: [],
            attach: [initialRefAttachEvidence.attachRecord]
          }
        },
        {
          label: 'root-unmount-executes-ref-cleanup-before-passive-destroy',
          rootCommitRefMetadata: {
            detach: [refDetachEvidence.detachRecord],
            attach: []
          }
        }
      ]
  ...
  const executionPayload =
    refCallbackGate.getPrivateRefCallbackCleanupReturnExecutionGateSnapshotPayload(
      executionSnapshot
    );
  ...
  if (
    ...
    executionSnapshot.testOnlyExecution !== true ||
    executionSnapshot.attachCount !== 1 ||
    executionSnapshot.detachCount !== 1 ||
    executionSnapshot.callbackInvocationAttemptCount !== 1 ||
    executionSnapshot.cleanupInvocationAttemptCount !== 1 ||
    executionSnapshot.cleanupInvocationErrorCount !== 0 ||
    executionSnapshot.callbackInvocationErrorCount !== 0 ||
    executionSnapshot.cleanupReturnHandleConsumedCount !== 1 ||
    executionSnapshot.cleanupReturnHandleExecutionCount !== 1 ||
    executionSnapshot.publicRootsTouched !== false ||
    executionSnapshot.compatibilityClaimed !== false
  ) {
    throwInvalidRootPublicFacadeHostOutputUnmount(
      'Private public-facade root.unmount ref cleanup execution requires accepted test-control cleanup-return evidence.'
    );
  }

  return freezeRecord({
    kind: 'FastReactDomPrivateRootPublicFacadeUnmountRefCleanupExecutionEvidence',
    status:
      ROOT_BRIDGE_PUBLIC_FACADE_UNMOUNT_REF_CLEANUP_EXECUTION_ACCEPTED,
    executionGateStatus: executionSnapshot.status,
    testOnlyExecution: executionSnapshot.testOnlyExecution,
    stepCount: executionSnapshot.stepCount,
    recordCount: executionSnapshot.recordCount,
    attachCount: executionSnapshot.attachCount,
    detachCount: executionSnapshot.detachCount,
    callbackInvocationAttemptCount:
      executionSnapshot.callbackInvocationAttemptCount,
    cleanupInvocationAttemptCount:
      executionSnapshot.cleanupInvocationAttemptCount,
    cleanupInvocationErrorCount: executionSnapshot.cleanupInvocationErrorCount,
    cleanupReturnHandleConsumedCount:
      executionSnapshot.cleanupReturnHandleConsumedCount,
    cleanupReturnHandleExecutionCount:
      executionSnapshot.cleanupReturnHandleExecutionCount,
    callbackRefsInvoked: executionSnapshot.callbackRefsInvoked,
    callbackCleanupReturnsInvoked:
      executionSnapshot.callbackCleanupReturnsInvoked,
    objectRefsMutated: executionSnapshot.objectRefsMutated,
    publicRootsTouched: executionSnapshot.publicRootsTouched,
    compatibilityClaimed: executionSnapshot.compatibilityClaimed,
    refCleanupBeforePassiveDestroy: true,
    refCleanupHandleRootId: refContext.refHandle.rootId,
    executionSnapshot
  });
}

function createPublicFacadeUnmountPassiveDestroyEvidence(
  descriptor,
  options
) {
  ...
  const requiresOrderingMetadata =
    options != null && options.requiresOrderingMetadata === true;
  const deletionOrdering =
    passiveDiagnostics == null
      ? null
      : passiveDiagnostics.deletedSubtreeRefPassiveCleanupOrder;
  const destroyOrderingMetadataAccepted =
    requiresOrderingMetadata === true &&
    deletionOrdering != null &&
    deletionOrdering.status ===
      testUtilsActGate.passiveEffectDeletedSubtreeRefPassiveOrderingStatus &&
    deletionOrdering.refCleanupBeforePassiveDestroy === true &&
    deletionOrdering.passiveDestroyBeforeHostCleanup === true &&
    deletionOrdering.publicEffectExecution === false &&
    deletionOrdering.publicRootExecution === false &&
    deletionOrdering.compatibilityClaimed === false;
  ...
  return freezeRecord({
    ...
    refCleanupBeforePassiveDestroy:
      destroyOrderingMetadataAccepted === true
        ? deletionOrdering.refCleanupBeforePassiveDestroy
        : false,
    passiveDestroyBeforeHostCleanup:
      destroyOrderingMetadataAccepted === true
        ? deletionOrdering.passiveDestroyBeforeHostCleanup
        : false,
    hostCleanupAfterPassiveDestroy:
      destroyOrderingMetadataAccepted === true
        ? deletionOrdering.hostCleanupAfterPassiveDestroy
        : false,
    deletionCleanupOrderRecordCount:
      destroyOrderingMetadataAccepted === true
        ? deletionOrdering.records.length
        : 0,
...
function getPublicFacadeUnmountPassiveDestroyDescriptor(element) {
  ...
  if (element.privatePassiveDestroy === true) {
    return freezeRecord({
      ...
      destroyCount: 1,
      metadataOnly: true,
      consumeRefCleanupExecution: false
    });
  }
  if (
    ...
    return freezeRecord({
      source: 'element.privatePassiveDestroy',
      destroyCount,
      consumeRefCleanupExecution:
        element.privatePassiveDestroy.consumeRefCleanupExecution === true ||
        element.privatePassiveDestroy.consumeRefCleanup === true,
      metadataOnly:
...
function createPublicFacadeHostOutputUnmountAcceptedCapabilities(evidence) {
  ...
  if (evidence.refCleanupExecutionAccepted === true) {
    capabilities.push(
      ...ROOT_BRIDGE_PUBLIC_FACADE_HOST_OUTPUT_UNMOUNT_REF_CLEANUP_PASSIVE_ORDER_ACCEPTED_CAPABILITIES
    );
  }
...
function createPublicFacadeHostOutputUnmountBlockedCapabilities(evidence) {
  if (!isAcceptedPublicFacadeUnmountRefPassiveEvidence(evidence)) {
    return ROOT_BRIDGE_PUBLIC_FACADE_HOST_OUTPUT_UNMOUNT_BLOCKED_CAPABILITIES;
  }
  return evidence.refCleanupExecutionAccepted === true
    ? ROOT_BRIDGE_PUBLIC_FACADE_HOST_OUTPUT_UNMOUNT_REF_CLEANUP_PASSIVE_ORDER_BLOCKED_CAPABILITIES
...
function applyPrivateRootDangerousHtmlTextResetCommit(
  record,
  rootCommitHostComponentUpdateMetadata,
  diagnostic,
  options
...
function applyPrivateRootDangerousHtmlTextResetCommitWithBridge(
  bridgeState,
  record,
  rootCommitHostComponentUpdateMetadata,
  diagnostic,
  ...
  const normalized = normalizeRootCommitHostComponentUpdateOptions(options);
  if (normalized.textUpdate !== null) {
    throwInvalidDangerousHtmlTextResetCommitHandoff(
      'Private dangerousHTML/text reset commit handoffs do not accept HostText update metadata.'
    );
  }
  const metadata = normalizeRootCommitHostComponentUpdateMetadata(
    rootCommitHostComponentUpdateMetadata,
  ...
  if (hostRootOwner !== sourcePayload.rootHandle.owner) {
    throwInvalidDangerousHtmlTextResetCommitHandoff(
      'Private dangerousHTML/text reset commit handoffs require a host instance owned by the updated root.'
    );
  }
  ...
  const existing =
    rootDangerousHtmlTextResetCommitHandoffRecords.get(record);
  if (existing !== undefined) {
    const existingPayload =
      rootDangerousHtmlTextResetCommitHandoffPayloads.get(existing);
    if (
      existingPayload !== undefined &&
      existingPayload.diagnostic === diagnostic &&
      existingPayload.hostInstanceToken === normalized.hostInstanceToken &&
    ...
    throwInvalidDangerousHtmlTextResetCommitHandoff(
      'Private dangerousHTML/text reset commit handoff is already recorded for this root update.'
    );
  }

  const rootBridgeState = validation.rootHandleState.bridgeState;
  const sequence =
    rootBridgeState.nextDangerousHtmlTextResetCommitSequence++;
  const handoffId =
    `${rootBridgeState.dangerousHtmlTextResetCommitIdPrefix}:${sequence}`;
  let mutationRecords = null;
  let mutationPayload = null;
  let latestPropsCommitRecord = null;
  let latestPropsHandoffStale = false;
  let mutationRollbackApplied = false;
  let mutationRollbackRecordCount = 0;
  let mutationRollbackError = null;
  let publishedLatestProps = null;

  try {
    mutationRecords = applyDangerousHtmlTextResetFakeDomRows(
      hostNode,
      diagnosticValidation.payload.blockedMutationRows
    );
    mutationPayload =
      getDangerousHtmlTextResetFakeDomMutationPayload(mutationRecords);
    if (mutationPayload === null) {
      throwInvalidDangerousHtmlTextResetCommitHandoff(
        'Private dangerousHTML/text reset commit handoffs require applied fake-DOM mutation records.'
      );
    }
    ...
    if (
      ...
      latestPropsHandoffStale = true;
      throwInvalidDangerousHtmlTextResetCommitHandoff(
        'Private dangerousHTML/text reset commit handoffs reject stale latest-props rows before publication.'
      );
    }

    latestPropsCommitRecord = createLatestPropsCommitRecord(
      hostNode,
      normalized.nextProps,
      []
    );
    publishedLatestProps =
      commitLatestPropsFromMutationRecord(latestPropsCommitRecord);
  } catch (error) {
    if (mutationRecords !== null) {
      try {
        mutationRollbackRecordCount =
          rollbackDangerousHtmlTextResetFakeDomMutation(mutationRecords);
        mutationRollbackApplied = true;
      } catch (rollbackError) {
        mutationRollbackError = rollbackError;
      }
    }
    attachDangerousHtmlTextResetCommitRollbackEvidence(error, {
      latestPropsHandoffStale,
      mutationPayload,
      mutationRecords,
      mutationRollbackApplied,
  ...
  const mutationEvidence =
    createDangerousHtmlTextResetMutationEvidence(mutationPayload);
  const fakeDomCommitRows =
    createDangerousHtmlTextResetFakeDomExecutionRows(mutationRecords);
  const latestPropsPublished = publishedLatestProps === normalized.nextProps;
  const handoff = freezeRecord({
    $$typeof: privateRootDangerousHtmlTextResetCommitHandoffRecordType,
    kind: 'FastReactDomPrivateRootDangerousHtmlTextResetCommitHandoffRecord',
    operation: 'root-dangerous-html-text-reset-fake-dom-commit',
    handoffId,
    handoffSequence: sequence,
    handoffStatus: ROOT_BRIDGE_DANGEROUS_HTML_TEXT_RESET_COMMIT_APPLIED,
    sourceRequestId: record.requestId,
    sourceRequestSequence: record.requestSequence,
    sourceRequestType: record.requestType,
    sourceUpdateId: record.updateId,
    ...
    latestPropsPublished,
    latestPropsPublishOrder:
      'after-dangerous-html-text-reset-fake-dom-mutation',
    acceptedCapabilities:
      createDangerousHtmlTextResetExecutionAcceptedCapabilities(
        mutationEvidence
      ),
    blockedCapabilities:
      ROOT_BRIDGE_DANGEROUS_HTML_TEXT_RESET_EXECUTION_BLOCKED_CAPABILITIES,
    publicRootCreated: false,
    publicRootObjectExposed: false,
    nativeExecution: false,
    reconcilerExecution: false,
    ...
    domMutation: true,
    browserDomMutation: false,
    fakeDomInnerHTMLWritten: mutationEvidence.setInnerHTMLCount > 0,
    fakeDomTextContentWritten: mutationEvidence.textContentRowCount > 0,
    realDomInnerHTMLWritten: false,
    realDomTextContentWritten: false,
    markerWrites: false,
    listenerInstallation: false,
  ...
  rootDangerousHtmlTextResetCommitHandoffRecords.set(record, handoff);
  rootDangerousHtmlTextResetCommitHandoffPayloads.set(handoff, {
    bridgeState: rootBridgeState,
    diagnostic,
    diagnosticPayload: diagnosticValidation.payload,
    fakeDomCommitRows,
...
function getPrivateRootDangerousHtmlTextResetCommitHandoffPayload(record) {
  return rootDangerousHtmlTextResetCommitHandoffPayloads.get(record) || null;
}

function isPrivateRootDangerousHtmlTextResetCommitHandoffRecord(value) {
  return rootDangerousHtmlTextResetCommitHandoffPayloads.has(value);
}
...
function getPrivateHydrateRootPublicFacadePreflightPayload(preflight) {
  ...
  return freezeRecord({
    ...
    eventReplayPreflightRecordCount:
      payload.eventReplayPreflightRecords.length,
    ...
    targetClaimingPreflightRecords: freezeArray(
      payload.targetClaimingPreflightRecords
    ),
    textNodeClaimPatchExecutionRecordCount:
      payload.textNodeClaimPatchExecutionRecords.length,
    textNodeClaimPatchExecutionRecords: freezeArray(
      payload.textNodeClaimPatchExecutionRecords
    )
  });
}
...
function getPrivateHydrateRootPublicFacadeTargetClaimingPreflightPayload(
  record
) {
  return (
    rootHydratePublicFacadeTargetClaimingPreflightPayloads.get(record) ||
...
function isPrivateHydrateRootPublicFacadeTargetClaimingPreflightRecord(
  value
) {
  return rootHydratePublicFacadeTargetClaimingPreflightPayloads.has(value);
}

function getPrivateHydrateRootPublicFacadeEventReplayPreflightPayload(
  record
) {
  return (
    rootHydratePublicFacadeEventReplayPreflightPayloads.get(record) ||
...
function isPrivateHydrateRootPublicFacadeEventReplayPreflightRecord(
  value
) {
...
function getPrivateHydrateRootPublicFacadeTextNodeClaimPatchExecutionPayload(
  ...
  return (
    rootHydratePublicFacadeTextNodeClaimPatchExecutionPayloads.get(record) ||
    null
  );
}

function isPrivateHydrateRootPublicFacadeTextNodeClaimPatchExecutionRecord(
  value
) {
  return rootHydratePublicFacadeTextNodeClaimPatchExecutionPayloads.has(value);
}
...
function createBridgeState(options) {
  ...
  return {
    ...
    dangerousHtmlTextResetCommitIdPrefix: getIdPrefix(
      options && options.dangerousHtmlTextResetCommitIdPrefix,
      'dangerous-html-text-reset-execution'
    ),
    ...
    nextDangerousHtmlTextResetCommitSequence: 1,
...
function createDangerousHtmlTextResetMutationEvidence(mutationPayload) {
  const mutationRecords =
    mutationPayload === null ? [] : mutationPayload.mutationRecords;
  const rowKinds = [];
  let setInnerHTMLCount = 0;
  let setTextContentCount = 0;
  let resetTextContentCount = 0;

  for (const record of mutationRecords) {
    if (!rowKinds.includes(record.sourceKind)) {
      rowKinds.push(record.sourceKind);
    }
    if (record.operation === 'setInnerHTML') {
      setInnerHTMLCount += 1;
    } else if (record.operation === 'setTextContent') {
      setTextContentCount += 1;
    } else if (record.operation === 'resetTextContent') {
      resetTextContentCount += 1;
    }
  }

  const textContentRowCount = setTextContentCount + resetTextContentCount;
  return freezeRecord({
    fakeDomMutationBacked: true,
    rowCount: mutationRecords.length,
    mutatingRowCount: mutationRecords.length,
    setInnerHTMLCount,
    setTextContentCount,
    resetTextContentCount,
    textContentRowCount,
    dangerousHtmlRowCount: setInnerHTMLCount,
    rowKinds: freezeArray(rowKinds),
    rollbackSupported:
      mutationPayload === null ? false : mutationPayload.rollbackSupported,
    rollbackRecordCount:
      mutationPayload === null ? 0 : mutationPayload.rollbackRecordCount,
    gateMetadata:
      DOM_DANGEROUS_HTML_TEXT_RESET_FAKE_DOM_MUTATION_GATE_METADATA
  });
}

function createDangerousHtmlTextResetFakeDomExecutionRows(mutationRecords) {
  return freezeArray(
    mutationRecords.map((record, index) =>
      freezeRecord({
        kind: 'FastReactDomPrivateDangerousHtmlTextResetFakeDomExecutionRow',
        rowIndex: index,
        sourceRowId: record.sourceRowId,
        commitRowId: `${record.operation}-fake-dom-execution`,
        commitRowKind: record.operation,
        sourceKind: record.sourceKind,
        sourceMutation: record.mutation,
        propName: record.propName,
        propertyName: record.propertyName,
        valueLength: record.valueLength,
        metadataOnly: false,
        fakeDomCommitMetadata: false,
        fakeDomMutation: true,
        realDomMutation: false,
        browserDomMutation: false,
        realDomInnerHTMLWritten: false,
...
function createDangerousHtmlTextResetExecutionAcceptedCapabilities(
  mutationEvidence
) {
  const capabilities = [
    ...ROOT_BRIDGE_DANGEROUS_HTML_TEXT_RESET_EXECUTION_ACCEPTED_CAPABILITIES
  ];
  if (mutationEvidence.setInnerHTMLCount > 0) {
    capabilities.push(
      freezeRecord({
        id: 'fake-dom-inner-html-write',
        accepted: true,
        reason:
          'The private fake-DOM host node accepted an innerHTML write from admitted dangerousHTML rows.'
      })
    );
  }
  if (mutationEvidence.setTextContentCount > 0) {
    capabilities.push(
      freezeRecord({
        id: 'fake-dom-text-content-write',
        accepted: true,
        reason:
          'The private fake-DOM host node accepted a textContent write from admitted removal rows.'
      })
    );
  }
  if (mutationEvidence.resetTextContentCount > 0) {
    capabilities.push(
      freezeRecord({
        id: 'fake-dom-reset-text-content-write',
        accepted: true,
        reason:
          'The private fake-DOM host node accepted a resetTextContent write before managed child work remains blocked.'
      })
    );
  }
  return freezeArray(capabilities);
...
function attachDangerousHtmlTextResetCommitRollbackEvidence(error, options) {
  if (!isObjectOrFunction(error)) {
    return;
  }

  const internalEvidence = error.domDangerousHtmlTextResetRollbackEvidence;
  const internalMutationAttempted =
    isObjectOrFunction(internalEvidence) &&
    internalEvidence.mutationAttempted === true;
  const internalRollbackAttempted =
    isObjectOrFunction(internalEvidence) &&
    internalEvidence.rollbackAttempted === true;
  const internalRollbackApplied =
    isObjectOrFunction(internalEvidence) &&
    internalEvidence.rollbackApplied === true;
  const internalRollbackRecordCount =
    isObjectOrFunction(internalEvidence) &&
    typeof internalEvidence.rollbackRecordCount === 'number'
      ? internalEvidence.rollbackRecordCount
      : 0;
  const mutationRollbackError = options.mutationRollbackError;
  error.privateRootDangerousHtmlTextResetRollbackEvidence = freezeRecord({
    ...
    mutationAttempted:
      options.mutationRecords !== null || internalMutationAttempted,
    mutationRollbackAttempted:
      options.mutationRecords !== null || internalRollbackAttempted,
    mutationRollbackApplied:
      options.mutationRollbackApplied || internalRollbackApplied,
    mutationRollbackRecordCount:
      options.mutationRollbackRecordCount || internalRollbackRecordCount,
    unsupportedRowRejected:
      isUnsupportedDangerousHtmlTextResetCommitError(error) ||
      (isObjectOrFunction(internalEvidence) &&
        internalEvidence.unsupportedRowRejected === true),
    mutationRollbackError:
      mutationRollbackError === null
        ? null
        : freezeRecord({
            code: mutationRollbackError.code || null,
            message: mutationRollbackError.message
          })
  });
}

function isUnsupportedDangerousHtmlTextResetCommitError(error) {
  return (
    isObjectOrFunction(error) &&
...
function throwInvalidDangerousHtmlTextResetCommitHandoff(message) {
  const error = new Error(message);
  error.code =
...
module.exports = {
  ...
  applyPrivateRootDangerousHtmlTextResetCommit,
  ...
  privateRootCreateSideEffectRecordType,
  privateRootCommitHostComponentUpdateHandoffRecordType,
  privateRootDangerousHtmlTextResetCommitHandoffRecordType,
