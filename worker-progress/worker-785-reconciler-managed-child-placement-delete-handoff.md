...
- Acceptance audit follow-up: hardened placement complete-work metadata so the candidate must be the parent’s sole finished child, including rejecting a final child in a multi-child parent.
...
- `cargo test -p fast-react-reconciler complete_managed_child -- --nocapture`
