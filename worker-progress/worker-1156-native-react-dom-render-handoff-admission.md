...
- Repaired stale native `check` script snapshots in the import-entrypoints smoke guard and benchmark accepted-command provenance.
...
- `source /Users/user/.nvm/nvm.sh && nvm use 26.1.0 >/dev/null && node tests/smoke/import-entrypoints.mjs` passed.
- `source /Users/user/.nvm/nvm.sh && nvm use 26.1.0 >/dev/null && node tests/benchmarks/scripts/check-benchmark-manifests.mjs` passed: 13 manifests, 150 scenarios, 34 milestones, 0 result artifacts.
