...
mod tests {
    ...
    #[test]
    fn function_component_passive_metadata_carries_destroy_handle_for_update() {
        let (mut arena, current, work_in_progress, component) = function_component_pair();
        let mut hook_store = FunctionComponentHookRenderStore::new();
        ...
        assert_eq!(pending_metadata[0].instance(), registration.instance());
        assert_eq!(pending_metadata[0].create(), callback(74));
        assert_eq!(pending_metadata[0].destroy(), Some(callback(721)));
        assert_eq!(pending_metadata[0].dependencies(), deps(740));
        assert_eq!(pending_metadata[0].lanes(), Lanes::DEFAULT);
        assert_eq!(
            hook_store
                .hook_effects()
