...
impl RootSchedulerState {
    ...
    #[allow(
        dead_code,
        reason = "private same-lane multi-update currentness canaries may inspect source lifecycle counts"
    )]
    fn pending_finished_work_queue_lane_commit_currentness_source_count(&self) -> usize {
        self.finished_work_queue_lane_commit_currentness_sources
            .len()
    }
    ...
    fn consumed_finished_work_queue_lane_commit_currentness_source_count(&self) -> usize {
        self.consumed_finished_work_queue_lane_commit_currentness
            .len()
    }
}
...
#[cfg(test)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct RootEntangledTransitionLaneSchedulerUpdateRecordForCanary {
    sequence: usize,
    update: UpdateId,
    lane: Lane,
    pending_lanes_after_enqueue: Lanes,
    selected_next_lanes_after_enqueue: Lanes,
    entangled_lanes_after_enqueue: Lanes,
}

#[cfg(test)]
#[allow(
    dead_code,
    reason = "crate-private entangled transition queue-lane canaries inspect per-update diagnostics"
)]
impl RootEntangledTransitionLaneSchedulerUpdateRecordForCanary {
    #[must_use]
    ...
    #[must_use]
    pub(crate) const fn entangled_lanes_after_enqueue(self) -> Lanes {
        self.entangled_lanes_after_enqueue
    }
}

#[cfg(test)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct RootEntangledTransitionLaneSchedulerRequestRecordForCanary {
    root: FiberRootId,
    fiber: FiberId,
    queue: UpdateQueueHandle,
    first: RootEntangledTransitionLaneSchedulerUpdateRecordForCanary,
    second: RootEntangledTransitionLaneSchedulerUpdateRecordForCanary,
    selected_lanes: Lanes,
    entangled_lanes: Lanes,
    root_pending_lanes_after_enqueue: Lanes,
    root_entangled_lanes_after_enqueue: Lanes,
    current_event_transition_lane_before: Lane,
    current_event_transition_lane_after: Lane,
    scheduled_root: ScheduledRootUpdateResult,
}
...
#[allow(
    dead_code,
    reason = "crate-private entangled transition queue-lane scheduler diagnostics are reserved for canaries"
)]
impl RootEntangledTransitionLaneSchedulerRequestRecordForCanary {
    #[must_use]
    pub(crate) const fn root(self) -> FiberRootId {
        self.root
    }
    ...
    #[must_use]
    pub(crate) const fn first(self) -> RootEntangledTransitionLaneSchedulerUpdateRecordForCanary {
        self.first
    }

    #[must_use]
    pub(crate) const fn second(self) -> RootEntangledTransitionLaneSchedulerUpdateRecordForCanary {
        self.second
    }
    ...
    #[must_use]
    pub(crate) const fn root_pending_lanes_after_enqueue(self) -> Lanes {
        self.root_pending_lanes_after_enqueue
    }

    #[must_use]
    pub(crate) const fn root_entangled_lanes_after_enqueue(self) -> Lanes {
        self.root_entangled_lanes_after_enqueue
    }
    ...
    #[must_use]
    pub(crate) const fn update_sequence_ids(self) -> [UpdateId; 2] {
        [self.first.update, self.second.update]
    }

    #[must_use]
    pub(crate) const fn lanes(self) -> [Lane; 2] {
        [self.first.lane, self.second.lane]
    }
...
#[cfg(test)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum RootEntangledTransitionSchedulerQueueLaneContinuationStatusForCanary {
    StaleCallbackNode,
    NoTransitionWork,
    StaleTransitionDiagnostics,
    BlockedByTransitionEntanglementMismatch,
...
#[cfg(test)]
#[derive(Debug, Clone, PartialEq, Eq)]
struct RootEntangledTransitionSchedulerQueueLaneContinuationMetadataForCanary {
    request: RootEntangledTransitionLaneSchedulerRequestRecordForCanary,
    callback_execution: RootSchedulerCallbackExecutionRecord,
    current_callback_node: RootSchedulerCallbackHandle,
    current_event_transition_lane: Lane,
    root_current_before_continuation: FiberId,
    ...
    root_entangled_lanes_before_continuation: Lanes,
    finished_work_handoff_identity: Option<RootSyncSchedulerFinishedWorkHandoffIdentityForCanary>,
    status: RootEntangledTransitionSchedulerQueueLaneContinuationStatusForCanary,
    queue_handoff: Option<HostRootUpdateQueueLaneHandoffRecordForCanary>,
    queue_handoff_error: Option<HostRootUpdateQueueFinishedWorkCommitHandoffErrorForCanary>,
    queue_commit_handoff: Option<HostRootUpdateQueueFinishedWorkCommitHandoffRecordForCanary>,
    commit: Option<HostRootCommitRecord>,
...
#[cfg(test)]
#[derive(Debug, PartialEq, Eq)]
pub(crate) struct RootEntangledTransitionSchedulerQueueLaneContinuationRecordForCanary {
    request: RootEntangledTransitionLaneSchedulerRequestRecordForCanary,
    callback_execution: RootSchedulerCallbackExecutionRecord,
    current_callback_node: RootSchedulerCallbackHandle,
    current_event_transition_lane: Lane,
    ...
    queue_commit_handoff: Option<HostRootUpdateQueueFinishedWorkCommitHandoffRecordForCanary>,
    commit: Option<HostRootCommitRecord>,
    source_metadata: RootEntangledTransitionSchedulerQueueLaneContinuationMetadataForCanary,
    currentness_source_token:
        Option<RootFinishedWorkQueueLaneCommitCurrentnessSourceTokenForCanary>,
}

#[cfg(test)]
impl Clone for RootEntangledTransitionSchedulerQueueLaneContinuationRecordForCanary {
    fn clone(&self) -> Self {
        Self {
            request: self.request,
            callback_execution: self.callback_execution,
...
#[allow(
    dead_code,
    reason = "private entangled transition queue-lane continuation canaries consume this evidence"
)]
impl RootEntangledTransitionSchedulerQueueLaneContinuationRecordForCanary {
    #[must_use]
    ...
    pub(crate) const fn status(
        &self,
    ) -> RootEntangledTransitionSchedulerQueueLaneContinuationStatusForCanary {
        self.status
    }
    ...
    #[must_use]
    pub(crate) const fn did_execute_entangled_transition_queue_lane_scheduler_continuation(
        &self,
    ) -> bool {
        matches!(
            self.status,
    ...
    pub(crate) const fn rejected_stale_callback_node(&self) -> bool {
        matches!(
            self.status,
            RootEntangledTransitionSchedulerQueueLaneContinuationStatusForCanary::StaleCallbackNode
        )
    }
    ...
    pub(crate) const fn no_transition_work(&self) -> bool {
        matches!(
            self.status,
            RootEntangledTransitionSchedulerQueueLaneContinuationStatusForCanary::NoTransitionWork
        )
    }
    ...
    pub(crate) const fn stale_transition_diagnostics(&self) -> bool {
        matches!(
            self.status,
            RootEntangledTransitionSchedulerQueueLaneContinuationStatusForCanary::StaleTransitionDiagnostics
        )
    }
    ...
    pub(crate) const fn blocked_by_finished_work_handoff_mismatch(&self) -> bool {
        matches!(
            self.status,
            RootEntangledTransitionSchedulerQueueLaneContinuationStatusForCanary::BlockedByFinishedWorkHandoffMismatch
        )
    }
    ...
    #[must_use]
    pub(crate) fn source_metadata_intact_for_canary(&self) -> bool {
        let source = &self.source_metadata;
        self.request == source.request
            && self.callback_execution == source.callback_execution
            && self.current_callback_node == source.current_callback_node
            && self.current_event_transition_lane == source.current_event_transition_lane
            && self.root_current_before_continuation == source.root_current_before_continuation
            && self.root_pending_lanes_before_continuation
                == source.root_pending_lanes_before_continuation
            && self.root_entangled_lanes_before_continuation
                == source.root_entangled_lanes_before_continuation
            && self.finished_work_handoff_identity == source.finished_work_handoff_identity
            && self.status == source.status
            && self.queue_handoff == source.queue_handoff
            && self.queue_handoff_error == source.queue_handoff_error
            && self.queue_commit_handoff == source.queue_commit_handoff
            && self.commit == source.commit
    }

    #[must_use]
    pub(crate) fn accepted_entangled_transition_scheduler_evidence_for_canary(&self) -> bool {
        let request_lanes = self.request.lanes();
        self.source_metadata_intact_for_canary()
            && self.did_execute_entangled_transition_queue_lane_scheduler_continuation()
            && self.request.routed_to_private_root_scheduler()
            && request_lanes[0].is_transition()
            && request_lanes[1].is_transition()
            && request_lanes[0] != request_lanes[1]
            && self.request.selected_lanes().includes_only_transitions()
            && self
                .request
                .selected_lanes()
                .contains_lane(request_lanes[0])
            && self
                .request
                .selected_lanes()
                .contains_lane(request_lanes[1])
            && self.request.entangled_lanes() == self.request.selected_lanes()
            && self
                .root_pending_lanes_before_continuation
                .contains_all(self.request.selected_lanes())
            && self
                .root_entangled_lanes_before_continuation
                .contains_all(self.request.entangled_lanes())
            && self.root_current_before_continuation == self.request.fiber()
            && self.current_event_transition_lane
                == self.request.current_event_transition_lane_after()
            && self.callback_execution.status() == RootSchedulerCallbackExecutionStatus::Rendered
            && self.callback_execution.root() == self.request.root()
            && self.callback_execution.callback_node() == self.current_callback_node
            && !self.callback_execution.validation().is_stale()
            && self.callback_execution.selected_lanes() == self.request.selected_lanes()
            && self.render_phase().is_some_and(|render| {
                render.root() == self.request.root()
                    && render.current() == self.request.fiber()
                    && render.render_lanes() == self.request.selected_lanes()
            })
    }

    #[must_use]
    pub(crate) fn accepted_root_scheduler_execution_evidence_for_canary(&self) -> bool {
        self.accepted_entangled_transition_scheduler_evidence_for_canary()
            && self.finished_work_handoff_identity.is_some_and(
                RootSyncSchedulerFinishedWorkHandoffIdentityForCanary::accepted_for_root_scheduler_commit_handoff,
            )
            && self.commit.as_ref().is_some_and(|commit| {
                self.render_phase().is_some_and(|render| {
                    commit.root() == self.request.root()
                        ...
                        && commit.current() == render.finished_work()
                        && commit.finished_work() == render.finished_work()
                        && commit.finished_lanes() == self.request.selected_lanes()
                        && commit.remaining_lanes() == render.remaining_lanes()
                        && commit.pending_lanes() == render.remaining_lanes()
                })
            })
    ...
    pub(crate) fn accepted_queue_lane_handoff_evidence_for_canary(&self) -> bool {
        self.queue_commit_handoff
            ...
            .is_some_and(|commit_handoff| {
                let queue_handoff = commit_handoff.queue_handoff();
                let expected_updates = self.request.update_sequence_ids();
                self.queue_handoff
                    .as_ref()
                    .is_some_and(|accepted| accepted == queue_handoff)
                    && commit_handoff.proves_queue_lane_handoff_gated_current_switch()
                    && commit_handoff.update_sequence_ids() == expected_updates.as_slice()
                    && commit_handoff.selected_lanes() == self.request.selected_lanes()
                    && commit_handoff.finished_lanes() == self.request.selected_lanes()
                    && queue_handoff.update_records().len() == 2
                    && queue_handoff.records_have_distinct_compatible_lanes()
                    && self.render_phase().is_some_and(|render| {
                        commit_handoff.remaining_lanes() == render.remaining_lanes()
                            && commit_handoff.applied_update_count()
                                == render.applied_update_count()
    ...
    #[must_use]
    pub(crate) fn routed_through_entangled_transition_queue_lane_and_commit_evidence_for_canary(
        &self,
    ) -> bool {
        self.accepted_root_scheduler_execution_evidence_for_canary()
            && self.accepted_root_commit_execution_evidence_for_canary()
    ...
    #[must_use]
    pub(crate) fn treats_entangled_transition_host_root_updates_as_current_only_with_queue_lane_handoff_for_canary(
        &self,
    ) -> bool {
        self.routed_through_entangled_transition_queue_lane_and_commit_evidence_for_canary()
            && self.queue_handoff.as_ref().is_some_and(|queue_handoff| {
                self.commit.as_ref().is_some_and(|commit| {
                    commit.root() == queue_handoff.root()
                        && commit.previous_current() == queue_handoff.current()
...
#[cfg(test)]
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct RootEntangledTransitionQueueLaneCommitCurrentnessRecordForCanary {
    transition_continuation: RootEntangledTransitionSchedulerQueueLaneContinuationRecordForCanary,
...
#[allow(
    dead_code,
    reason = "private entangled transition currentness is exercised by focused canaries"
)]
impl RootEntangledTransitionQueueLaneCommitCurrentnessRecordForCanary {
    #[must_use]
    ...
    #[must_use]
    pub(crate) const fn entangled_lanes(&self) -> Lanes {
        self.transition_continuation.request().entangled_lanes()
    }
    ...
    #[must_use]
    pub(crate) fn ties_entangled_transition_queue_lane_commit_to_live_tree_state_for_canary(
        &self,
    ) -> bool {
        ...
        let request_lanes = transition.request().lanes();
        let update_sequence_ids = self.currentness.update_sequence_ids();
        let expected_update_sequence_ids = transition.request().update_sequence_ids();

        transition.routed_through_entangled_transition_queue_lane_and_commit_evidence_for_canary()
            && self
                .currentness
            ...
            && update_sequence_ids == expected_update_sequence_ids.as_slice()
            && self.currentness.resulting_element() == render.resulting_element()
            && request_lanes[0].is_transition()
            && request_lanes[1].is_transition()
            && request_lanes[0] != request_lanes[1]
            && transition.selected_lanes().contains_lane(request_lanes[0])
            && transition.selected_lanes().contains_lane(request_lanes[1])
...
#[cfg(test)]
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum RootEntangledTransitionQueueLaneCommitCurrentnessErrorForCanary {
...
#[cfg(test)]
impl Display for RootEntangledTransitionQueueLaneCommitCurrentnessErrorForCanary {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        match self {
            Self::UnacceptedTransitionContinuation { root, status } => write!(
                formatter,
                "root {} entangled transition currentness rejected continuation {:?}",
                root.raw(),
                status
            ),
            Self::TransitionWrapperMetadataMismatch { root, field } => write!(
                formatter,
                "root {} entangled transition currentness wrapper metadata mismatch for {}",
                root.raw(),
                field
            ),
...
#[cfg(test)]
impl Error for RootEntangledTransitionQueueLaneCommitCurrentnessErrorForCanary {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
...
impl From<RootFinishedWorkQueueLaneCommitCurrentnessErrorForCanary>
    for RootEntangledTransitionQueueLaneCommitCurrentnessErrorForCanary
{
...
#[allow(
    ...
    dead_code,
    reason = "private entangled transition currentness consumer is exercised by focused canaries"
)]
pub(crate) fn consume_entangled_transition_queue_lane_commit_currentness_for_canary<
    H: HostTypes,
>(
    store: &mut FiberRootStore<H>,
    transition_continuation: &RootEntangledTransitionSchedulerQueueLaneContinuationRecordForCanary,
) -> Result<
    RootEntangledTransitionQueueLaneCommitCurrentnessRecordForCanary,
    RootEntangledTransitionQueueLaneCommitCurrentnessErrorForCanary,
> {
    if let Some(field) =
        entangled_transition_queue_lane_continuation_source_metadata_mismatch_for_canary(
    ...
    let Some(queue_lane_continuation) =
        entangled_transition_queue_lane_currentness_sync_continuation_for_canary(
            transition_continuation,
        )
    else {
        return Err(
    ...
    if let Some(field) =
        entangled_transition_queue_lane_currentness_wrapper_metadata_mismatch_for_canary(
            transition_continuation,
...
#[cfg(test)]
fn entangled_transition_queue_lane_currentness_sync_continuation_for_canary(
    transition_continuation: &RootEntangledTransitionSchedulerQueueLaneContinuationRecordForCanary,
...
#[cfg(test)]
fn entangled_transition_queue_lane_currentness_wrapper_metadata_mismatch_for_canary(
    transition_continuation: &RootEntangledTransitionSchedulerQueueLaneContinuationRecordForCanary,
    ...
    let expected_update_sequence_ids = transition_continuation.request().update_sequence_ids();
    if identity.update_sequence_ids() != expected_update_sequence_ids.as_slice() {
...
#[cfg(test)]
fn entangled_transition_queue_lane_continuation_source_metadata_mismatch_for_canary(
    transition_continuation: &RootEntangledTransitionSchedulerQueueLaneContinuationRecordForCanary,
) -> Option<&'static str> {
...
#[cfg(test)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum RootTransitionSameLaneMultiUpdateQueueLaneContinuationStatusForCanary {
    StaleCallbackNode,
    NoTransitionWork,
    StaleTransitionDiagnostics,
    BlockedByTransitionRequestMismatch,
    BlockedByTransitionEntanglementMismatch,
    BlockedByLaneMismatch,
    BlockedByFinishedWorkHandoffMismatch,
    BlockedByQueueLaneHandoffMismatch,
...
struct RootTransitionSameLaneMultiUpdateQueueLaneContinuationMetadataForCanary {
    ...
    root_entangled_lanes_before_continuation: Lanes,
    finished_work_handoff_identity: Option<RootSyncSchedulerFinishedWorkHandoffIdentityForCanary>,
    status: RootTransitionSameLaneMultiUpdateQueueLaneContinuationStatusForCanary,
    queue_handoff: Option<HostRootUpdateQueueLaneHandoffRecordForCanary>,
    queue_handoff_error: Option<HostRootUpdateQueueFinishedWorkCommitHandoffErrorForCanary>,
...
pub(crate) struct RootTransitionSameLaneMultiUpdateQueueLaneContinuationRecordForCanary {
    ...
    root_commit_handoff: Option<HostRootFinishedWorkCommitHandoffRecordForCanary>,
    commit: Option<HostRootCommitRecord>,
    source_metadata: RootTransitionSameLaneMultiUpdateQueueLaneContinuationMetadataForCanary,
    currentness_source_token:
        Option<RootFinishedWorkQueueLaneCommitCurrentnessSourceTokenForCanary>,
}

#[cfg(test)]
impl Clone for RootTransitionSameLaneMultiUpdateQueueLaneContinuationRecordForCanary {
    fn clone(&self) -> Self {
        Self {
            ...
            queue_handoff: self.queue_handoff.clone(),
            queue_handoff_error: self.queue_handoff_error.clone(),
            root_commit_handoff: self.root_commit_handoff.clone(),
            commit: self.commit.clone(),
            source_metadata: self.source_metadata.clone(),
            currentness_source_token: None,
        }
...
#[allow(
    dead_code,
    reason = "private same-transition multi-update queue-lane canaries consume this evidence"
)]
impl RootTransitionSameLaneMultiUpdateQueueLaneContinuationRecordForCanary {
    ...
    pub(crate) const fn status(
        &self,
    ) -> RootTransitionSameLaneMultiUpdateQueueLaneContinuationStatusForCanary {
        self.status
    }
    ...
    #[must_use]
    pub(crate) const fn did_execute_same_lane_multi_update_queue_lane_scheduler_continuation(
        &self,
    ) -> bool {
        matches!(
            self.status,
    ...
    #[must_use]
    pub(crate) const fn blocked_by_transition_request_mismatch(&self) -> bool {
        matches!(
            self.status,
    ...
    #[must_use]
    pub(crate) fn accepted_transition_scheduler_evidence_for_canary(&self) -> bool {
        self.did_execute_same_lane_multi_update_queue_lane_scheduler_continuation()
            && transition_same_lane_multi_update_request_batch_shape_for_canary(self.requests)
            && self.requests.iter().all(|request| {
                request.routed_to_private_root_scheduler()
                    && request.lane() == self.transition_lane()
                    && request.selected_next_lanes() == self.selected_lanes()
                    && request.entangled_lanes() == self.selected_lanes()
            })
            && self
                .root_pending_lanes_before_continuation
    ...
    pub(crate) fn accepted_root_scheduler_execution_evidence_for_canary(&self) -> bool {
        self.accepted_transition_scheduler_evidence_for_canary()
            ...
            && self.root_commit_handoff.as_ref().is_some_and(|handoff| {
                handoff.proves_private_root_finished_work_commit_metadata_handoff()
                    && self.commit.as_ref() == Some(handoff.commit())
            })
            && self.commit.as_ref().is_some_and(|commit| {
                self.render_phase().is_some_and(|render| {
    ...
    pub(crate) fn accepted_queue_lane_handoff_evidence_for_canary(&self) -> bool {
        self.queue_handoff.as_ref().is_some_and(|queue_handoff| {
            self.queue_handoff_error.is_none()
                && self.render_phase().is_some_and(|render| {
                    transition_same_lane_multi_update_queue_handoff_matches_render_for_canary(
                        self.root(),
                        render,
                        self.requests,
                        queue_handoff,
                ...
                && transition_same_lane_multi_update_queue_handoff_proves_source_owned_handoff_for_canary(
                    queue_handoff,
                    self.requests,
                )
                && self.commit.as_ref().is_some_and(|commit| {
                    commit.root() == queue_handoff.root()
                        ...
                        && commit.finished_lanes() == queue_handoff.finished_lanes()
                        && commit.remaining_lanes() == queue_handoff.remaining_lanes()
                        && commit.pending_lanes() == queue_handoff.remaining_lanes()
                })
        })
    }

    #[must_use]
    pub(crate) fn routed_through_same_lane_multi_update_transition_queue_lane_and_commit_evidence_for_canary(
        &self,
    ) -> bool {
        self.accepted_root_scheduler_execution_evidence_for_canary()
            && self.accepted_queue_lane_handoff_evidence_for_canary()
    ...
    #[must_use]
    pub(crate) fn treats_same_lane_multi_update_transition_host_root_updates_as_current_only_with_queue_lane_handoff_for_canary(
        &self,
    ) -> bool {
        self.routed_through_same_lane_multi_update_transition_queue_lane_and_commit_evidence_for_canary()
            && self.queue_handoff.as_ref().is_some_and(|queue_handoff| {
                self.commit.as_ref().is_some_and(|commit| {
                    commit.root() == queue_handoff.root()
                        && commit.previous_current() == queue_handoff.current()
...
fn transition_same_lane_multi_update_queue_handoff_matches_render_for_canary(
    ...
    if !transition_same_lane_multi_update_queue_handoff_proves_source_owned_handoff_for_canary(
        queue_handoff,
        requests,
    ) {
        ...
        && queue_handoff
            ...
            .all(|(record, request)| {
                record.update() == request.update()
                    ...
                    && record.source_lanes() == request.lane().to_lanes()
            })
}

#[cfg(test)]
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct RootTransitionSameLaneMultiUpdateQueueLaneCommitCurrentnessRecordForCanary {
    transition_continuation: RootTransitionSameLaneMultiUpdateQueueLaneContinuationRecordForCanary,
    currentness: RootFinishedWorkQueueLaneCommitCurrentnessRecordForCanary,
}
...
#[allow(
    dead_code,
    reason = "private same-transition multi-update currentness is exercised by focused canaries"
)]
impl RootTransitionSameLaneMultiUpdateQueueLaneCommitCurrentnessRecordForCanary {
    #[must_use]
    pub(crate) const fn transition_continuation(
        &self,
    ...
    #[must_use]
    pub(crate) const fn transition_lane(&self) -> Lane {
        self.transition_continuation.transition_lane()
    }
    ...
    #[must_use]
    pub(crate) fn ties_same_lane_multi_update_transition_queue_lane_commit_to_live_tree_state_for_canary(
        &self,
    ) -> bool {
        let transition = &self.transition_continuation;
        let Some(render) = transition.render_phase() else {
        ...
        transition
            .routed_through_same_lane_multi_update_transition_queue_lane_and_commit_evidence_for_canary()
            && self
                .currentness
                .ties_finished_work_queue_lane_commit_to_live_tree_state_for_canary()
            && self.currentness.root() == transition.root()
            ...
            && self.currentness.requested_callback_node() == transition.callback_node()
            && self.currentness.current_callback_node() == transition.current_callback_node()
            && update_sequence_ids == transition.update_sequence_ids()
            && self.currentness.resulting_element() == render.resulting_element()
            && transition.transition_lane().is_transition()
            && transition
                .selected_lanes()
                .contains_lane(transition.transition_lane())
            && transition.selected_lanes().includes_only_transitions()
    }
...
#[cfg(test)]
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum RootTransitionSameLaneMultiUpdateQueueLaneCommitCurrentnessErrorForCanary {
    UnacceptedTransitionContinuation {
        root: FiberRootId,
        status: RootTransitionSameLaneMultiUpdateQueueLaneContinuationStatusForCanary,
    },
...
#[cfg(test)]
impl Display for RootTransitionSameLaneMultiUpdateQueueLaneCommitCurrentnessErrorForCanary {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        match self {
            Self::UnacceptedTransitionContinuation { root, status } => write!(
                formatter,
                "root {} same-transition multi-update currentness rejected continuation {:?}",
                root.raw(),
                status
            ),
            Self::TransitionWrapperMetadataMismatch { root, field } => write!(
                formatter,
                "root {} same-transition multi-update currentness wrapper metadata mismatch for {}",
                root.raw(),
                field
            ),
            Self::FinishedWorkQueueLaneCurrentness(error) => Display::fmt(error, formatter),
...
#[cfg(test)]
impl Error for RootTransitionSameLaneMultiUpdateQueueLaneCommitCurrentnessErrorForCanary {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            Self::FinishedWorkQueueLaneCurrentness(error) => Some(error),
            Self::UnacceptedTransitionContinuation { .. }
...
#[cfg(test)]
impl From<RootFinishedWorkQueueLaneCommitCurrentnessErrorForCanary>
    for RootTransitionSameLaneMultiUpdateQueueLaneCommitCurrentnessErrorForCanary
{
    fn from(error: RootFinishedWorkQueueLaneCommitCurrentnessErrorForCanary) -> Self {
        Self::FinishedWorkQueueLaneCurrentness(error)
    }
...
#[allow(
    clippy::result_large_err,
    dead_code,
    reason = "private same-transition multi-update currentness consumer is exercised by focused canaries"
)]
pub(crate) fn consume_transition_same_lane_multi_update_queue_lane_commit_currentness_for_canary<
    H: HostTypes,
>(
    store: &mut FiberRootStore<H>,
    transition_continuation: &RootTransitionSameLaneMultiUpdateQueueLaneContinuationRecordForCanary,
) -> Result<
    RootTransitionSameLaneMultiUpdateQueueLaneCommitCurrentnessRecordForCanary,
    RootTransitionSameLaneMultiUpdateQueueLaneCommitCurrentnessErrorForCanary,
> {
    if let Some(field) =
        transition_same_lane_multi_update_queue_lane_continuation_source_metadata_mismatch_for_canary(
            transition_continuation,
        )
    {
        return Err(
    ...
    {
        return Err(
            RootTransitionSameLaneMultiUpdateQueueLaneCommitCurrentnessErrorForCanary::UnacceptedTransitionContinuation {
                root: transition_continuation.root(),
                status: transition_continuation.status(),
            },
        );
    ...
    let currentness =
        consume_transition_same_lane_multi_update_finished_work_queue_lane_commit_currentness_for_canary(
            store,
            transition_continuation,
        )?;
...
#[allow(
    clippy::result_large_err,
    reason = "private same-transition multi-update currentness keeps the accepted source-token lifecycle"
)]
fn consume_transition_same_lane_multi_update_finished_work_queue_lane_commit_currentness_for_canary<
    H: HostTypes,
>(
    store: &mut FiberRootStore<H>,
    transition_continuation: &RootTransitionSameLaneMultiUpdateQueueLaneContinuationRecordForCanary,
...
> {
    let Some(identity) =
        transition_same_lane_multi_update_queue_lane_commit_currentness_identity_for_canary(
            transition_continuation,
        )
    else {
        return Err(
    ...
    let Some(source_token) = transition_continuation.currentness_source_token() else {
        return Err(
    ...
    validate_transition_same_lane_multi_update_queue_lane_commit_currentness_live_state_for_canary(
        store,
        transition_continuation,
        &identity,
    )?;
    ...
    let (root_token, committed_element_after_consume, committed_root_children) =
        committed_host_root_tree_state_for_transition_same_lane_multi_update_currentness_for_canary(
            store,
            identity.root(),
    ...
    let Some(commit) = transition_continuation.commit() else {
        return Err(
            RootFinishedWorkQueueLaneCommitCurrentnessErrorForCanary::UnacceptedSchedulerContinuation,
        );
    };
    let commit_mutation_record_count = commit.mutation_log().len();
    let commit_deletion_list_count = commit.deletion_lists().len();
...
#[cfg(test)]
fn transition_same_lane_multi_update_queue_lane_commit_currentness_identity_for_canary(
    transition_continuation: &RootTransitionSameLaneMultiUpdateQueueLaneContinuationRecordForCanary,
) -> Option<RootFinishedWorkQueueLaneCommitCurrentnessIdentityForCanary> {
    let root_commit_handoff = transition_continuation.root_commit_handoff_for_canary()?;
    let queue_handoff = transition_continuation.queue_handoff()?;
    let commit = root_commit_handoff.commit();
    ...
    Some(
        RootFinishedWorkQueueLaneCommitCurrentnessIdentityForCanary {
            ...
            handoff_order: 0,
            commit_order: root_commit_handoff.commit_order(),
...
#[allow(
    clippy::result_large_err,
    reason = "private currentness diagnostics preserve exact rejection shape"
)]
fn validate_transition_same_lane_multi_update_queue_lane_commit_currentness_live_state_for_canary<
    H: HostTypes,
>(
    ...
    let Some(root_commit_handoff) = transition_continuation.root_commit_handoff_for_canary() else {
        return Err(
            RootFinishedWorkQueueLaneCommitCurrentnessErrorForCanary::UnacceptedSchedulerContinuation,
        );
    };
    let Some(queue_handoff) = transition_continuation.queue_handoff() else {
        return Err(
            RootFinishedWorkQueueLaneCommitCurrentnessErrorForCanary::UnacceptedSchedulerContinuation,
        );
    };
    let commit = root_commit_handoff.commit();
    if transition_continuation.commit() != Some(commit)
        ...
        || transition_continuation.selected_lanes() != identity.selected_lanes()
        || queue_handoff.finished_lanes() != identity.finished_lanes()
        || queue_handoff.remaining_lanes() != identity.remaining_lanes()
        || queue_handoff.update_sequence_ids() != identity.update_sequence_ids()
        || queue_handoff.resulting_element() != identity.resulting_element()
    {
        return Err(
            RootFinishedWorkQueueLaneCommitCurrentnessErrorForCanary::UnacceptedSchedulerContinuation,
        );
    ...
    validate_transition_same_lane_multi_update_queue_lane_commit_currentness_rows_for_canary(
        store,
        transition_continuation,
        queue_handoff,
    )?;
...
#[cfg(test)]
fn validate_transition_same_lane_multi_update_queue_lane_commit_currentness_rows_for_canary<
    H: HostTypes,
>(
    store: &FiberRootStore<H>,
    transition_continuation: &RootTransitionSameLaneMultiUpdateQueueLaneContinuationRecordForCanary,
...
#[cfg(test)]
fn committed_host_root_tree_state_for_transition_same_lane_multi_update_currentness_for_canary<
    H: HostTypes,
>(
    store: &FiberRootStore<H>,
    root: FiberRootId,
...
#[cfg(test)]
fn transition_same_lane_multi_update_queue_lane_continuation_source_metadata_mismatch_for_canary(
    transition_continuation: &RootTransitionSameLaneMultiUpdateQueueLaneContinuationRecordForCanary,
) -> Option<&'static str> {
    let source = &transition_continuation.source_metadata;
    if transition_continuation.requests != source.requests {
    ...
    if transition_continuation.root_commit_handoff != source.root_commit_handoff {
        return Some("root_commit_handoff");
    }
    if transition_continuation.commit != source.commit {
        return Some("commit");
    }
...
#[allow(
    clippy::result_large_err,
    reason = "private same-lane multi-update validator preserves queue-lane rejection details"
)]
fn validate_transition_same_lane_multi_update_queue_handoff_for_commit_for_canary<H: HostTypes>(
    store: &FiberRootStore<H>,
    root_id: FiberRootId,
    render: HostRootRenderPhaseRecord,
    requests: RootTransitionSameLaneMultiUpdateRequestsForCanary,
...
fn transition_same_lane_multi_update_queue_lane_continuation_record_for_canary(
    ...
    let source_metadata = RootTransitionSameLaneMultiUpdateQueueLaneContinuationMetadataForCanary {
        requests,
        callback_execution,
        ...
        root_commit_handoff: root_commit_handoff.clone(),
        commit: commit.clone(),
    };
...
#[allow(
    clippy::result_large_err,
    dead_code,
    reason = "private same-transition multi-update HostRoot queue/lane scheduler continuation is exercised by focused canaries"
)]
...
> {
    ...
    if transition_same_lane_multi_update_callback_execution_is_stale_for_canary(
        requests,
        callback_execution,
        current_callback_node,
    ) {
    ...
    if !transition_same_lane_multi_update_request_entanglement_is_current_for_canary(
        requests,
        root_entangled_lanes_before_continuation,
    ) {
        return Ok(
    ...
    if !transition_same_lane_multi_update_callback_render_lanes_match_request_for_canary(
        requests,
        callback_execution,
    ) {
        return Ok(
    ...
    let root_commit_handoff = match commit_finished_host_root_with_finished_work_handoff_for_canary(
        store,
        render_phase,
        Some(pending_finished_work),
        1,
    ) {
        Ok(root_commit_handoff) => root_commit_handoff,
        Err(error) => {
            return Ok(
                transition_same_lane_multi_update_queue_lane_continuation_record_for_canary(
                    requests,
                    ...
                    RootTransitionSameLaneMultiUpdateQueueLaneContinuationStatusForCanary::BlockedByFinishedWorkHandoffMismatch,
                    queue_handoff,
                    Some(HostRootUpdateQueueFinishedWorkCommitHandoffErrorForCanary::FinishedWork(Box::new(error))),
                    None,
                    None,
                ),
            );
    ...
    let currentness_identity = RootFinishedWorkQueueLaneCommitCurrentnessIdentityForCanary {
        ...
        finished_lanes: commit.finished_lanes(),
        remaining_lanes: commit.remaining_lanes(),
        requested_callback_node: callback_execution.callback_node(),
        current_callback_node,
        handoff_order: render_handoff.order(),
        commit_order: root_commit_handoff.commit_order(),
        update_sequence_ids: queue_handoff.update_sequence_ids(),
        resulting_element: queue_handoff.resulting_element(),
    };
...
#[allow(
    dead_code,
    reason = "crate-private entangled transition scheduler route is exercised by canary tests"
)]
pub(crate) fn record_entangled_transition_lane_scheduler_request_from_update_diagnostics_for_canary<
    H: HostTypes,
>(
    store: &mut FiberRootStore<H>,
    first: &UpdateContainerResult,
    second: &UpdateContainerResult,
) -> Result<RootEntangledTransitionLaneSchedulerRequestRecordForCanary, RootSchedulerError> {
    let root = first.schedule().root();
    let fiber = first.schedule().fiber();
    let queue = first.queue();

    let root_pending_lanes_after_enqueue = store.root(root)?.lanes().pending_lanes();
    let root_entangled_lanes_after_enqueue = store.root(root)?.lanes().entangled_lanes();
    let selected_lanes = select_lanes_for_scheduled_task(store, root)?.render_lanes();

    let first = entangled_transition_scheduler_update_record_for_canary(
        store, root, fiber, queue, 0, first,
    )?;
    let second = entangled_transition_scheduler_update_record_for_canary(
        store, root, fiber, queue, 1, second,
    )?;

    let entangled_lanes = first.lane().to_lanes().merge_lane(second.lane());
    validate_entangled_transition_lane_scheduler_request_lanes_for_canary(
        root,
        first.lane(),
        second.lane(),
        selected_lanes,
        entangled_lanes,
        root_entangled_lanes_after_enqueue,
    )?;

    let current_event_transition_lane_before =
        store.root_scheduler().current_event_transition_lane();
    let scheduled_root = ensure_root_schedule_entry(store, root)?;
    let current_event_transition_lane_after =
        store.root_scheduler().current_event_transition_lane();

    Ok(RootEntangledTransitionLaneSchedulerRequestRecordForCanary {
        root,
        fiber,
        queue,
        first,
        ...
        selected_lanes,
        entangled_lanes,
        root_pending_lanes_after_enqueue,
        root_entangled_lanes_after_enqueue,
        current_event_transition_lane_before,
        current_event_transition_lane_after,
        scheduled_root,
    })
}
...
#[allow(
    clippy::result_large_err,
    dead_code,
    reason = "private entangled transition HostRoot queue/lane scheduler continuation is exercised by focused canaries"
)]
pub(crate) fn execute_entangled_transition_scheduler_continuation_for_queue_lane_handoff_for_canary<
    H: HostTypes,
>(
    store: &mut FiberRootStore<H>,
    request: RootEntangledTransitionLaneSchedulerRequestRecordForCanary,
...
> {
    ...
    if entangled_transition_callback_execution_is_stale_for_canary(
        request,
        callback_execution,
        current_callback_node,
    ) {
        return Ok(
            entangled_transition_scheduler_queue_lane_continuation_record_for_canary(
                request,
                callback_execution,
                current_callback_node,
                current_event_transition_lane,
    ...
    if !root_pending_lanes_before_continuation.contains_all(request.selected_lanes()) {
        return Ok(
            entangled_transition_scheduler_queue_lane_continuation_record_for_canary(
                request,
                callback_execution,
    ...
    if root_current_before_continuation != request.fiber()
        || current_event_transition_lane != request.current_event_transition_lane_after()
    {
        return Ok(
            entangled_transition_scheduler_queue_lane_continuation_record_for_canary(
                request,
    ...
    if !entangled_transition_scheduler_request_entanglement_is_current_for_canary(
        request,
        root_entangled_lanes_before_continuation,
    ) {
        return Ok(
    ...
    if !entangled_transition_callback_render_lanes_match_request_for_canary(
        request,
        callback_execution,
    ) {
        return Ok(
    ...
    let mut continuation = entangled_transition_scheduler_queue_lane_continuation_record_for_canary(
        request,
        callback_execution,
        current_callback_node,
        current_event_transition_lane,
...
#[cfg(test)]
fn entangled_transition_callback_execution_is_stale_for_canary(
    request: RootEntangledTransitionLaneSchedulerRequestRecordForCanary,
    callback_execution: RootSchedulerCallbackExecutionRecord,
    current_callback_node: RootSchedulerCallbackHandle,
) -> bool {
...
#[cfg(test)]
fn entangled_transition_scheduler_request_entanglement_is_current_for_canary(
    request: RootEntangledTransitionLaneSchedulerRequestRecordForCanary,
    root_entangled_lanes: Lanes,
) -> bool {
    let [first_lane, second_lane] = request.lanes();
    first_lane.is_transition()
        && second_lane.is_transition()
        && first_lane != second_lane
        && request.first().sequence() == 0
        && request.second().sequence() == 1
        && request.first().update() != request.second().update()
        && request.selected_lanes().is_non_empty()
        && request.selected_lanes().includes_only_transitions()
        && request.selected_lanes().contains_lane(first_lane)
        && request.selected_lanes().contains_lane(second_lane)
        && request.entangled_lanes() == request.selected_lanes()
        && request.entangled_lanes().includes_only_transitions()
        && request.entangled_lanes().contains_lane(first_lane)
        && request.entangled_lanes().contains_lane(second_lane)
        && request
            .first()
            .entangled_lanes_after_enqueue()
            .contains_lane(first_lane)
        && request
            .second()
            .entangled_lanes_after_enqueue()
            .contains_all(request.entangled_lanes())
        && request
            .root_pending_lanes_after_enqueue()
            .contains_all(request.selected_lanes())
        && request
            .root_entangled_lanes_after_enqueue()
            .contains_all(request.entangled_lanes())
        && root_entangled_lanes.contains_all(request.entangled_lanes())
...
#[cfg(test)]
fn entangled_transition_callback_render_lanes_match_request_for_canary(
    request: RootEntangledTransitionLaneSchedulerRequestRecordForCanary,
    callback_execution: RootSchedulerCallbackExecutionRecord,
) -> bool {
    callback_execution.status() == RootSchedulerCallbackExecutionStatus::Rendered
        && callback_execution.selected_lanes() == request.selected_lanes()
        && callback_execution.render_phase().is_some_and(|render| {
            render.root() == request.root()
                && render.current() == request.fiber()
                && render.render_lanes() == request.selected_lanes()
                && render.render_lanes() == request.entangled_lanes()
        })
}
...
#[allow(
    clippy::too_many_arguments,
    reason = "private entangled transition queue/lane scheduler evidence mirrors the canary assertion shape"
)]
fn entangled_transition_scheduler_queue_lane_continuation_record_for_canary(
    request: RootEntangledTransitionLaneSchedulerRequestRecordForCanary,
    callback_execution: RootSchedulerCallbackExecutionRecord,
    current_callback_node: RootSchedulerCallbackHandle,
    current_event_transition_lane: Lane,
    ...
    let queue_handoff = queue_handoff.cloned();
    let source_metadata = RootEntangledTransitionSchedulerQueueLaneContinuationMetadataForCanary {
        request,
        callback_execution,
        current_callback_node,
        current_event_transition_lane,
...
#[cfg(test)]
fn entangled_transition_scheduler_update_record_for_canary<H: HostTypes>(
    store: &FiberRootStore<H>,
    root: FiberRootId,
    fiber: FiberId,
    queue: UpdateQueueHandle,
    sequence: usize,
    result: &UpdateContainerResult,
) -> Result<RootEntangledTransitionLaneSchedulerUpdateRecordForCanary, RootSchedulerError> {
    validate_schedule_record(store, result.schedule())?;

    let actual_root = result.schedule().root();
    if actual_root != root {
        return Err(RootSchedulerError::RescheduleRecordWrongRoot {
            expected: root,
            actual: actual_root,
    ...
    if result.schedule().fiber() != fiber {
        return Err(RootSchedulerError::ScheduleRecordWrongFiber {
            root,
            expected: fiber,
        ...
        || !lane.is_transition()
    {
        return Err(RootSchedulerError::TransitionSchedulerUnsupportedLane {
            ...
            lane,
            source_priority: result.source_priority(),
            selected_lanes: result.selected_next_lanes(),
        });
    }
    ...
    if result.queue() != queue
        || entanglement.root() != root
        || entanglement.queue() != queue
        || entanglement.lane() != lane
        || !entanglement.entangled_lanes().contains_lane(lane)
        || !entanglement.entangled_lanes().includes_only_transitions()
    {
        return Err(
    ...
    let current_queue = store
        .fiber_arena()
        .get(root_ref.current())
        .map_err(RootUpdateError::from)?
        .update_queue();
    let pending_updates = store
        .update_queues()
        ...
        || actual_lanes != lane.to_lanes()
    {
        return Err(RootUpdateError::StaleQueueEvidence {
            ...
            actual_pending_lanes: root_ref.lanes().pending_lanes(),
        }
        .into());
    }

    Ok(RootEntangledTransitionLaneSchedulerUpdateRecordForCanary {
        sequence,
        update: result.update(),
        lane,
        pending_lanes_after_enqueue: result.pending_lanes_after_enqueue(),
        selected_next_lanes_after_enqueue: result.selected_next_lanes(),
        entangled_lanes_after_enqueue: entanglement.entangled_lanes(),
    })
}

#[cfg(test)]
fn validate_entangled_transition_lane_scheduler_request_lanes_for_canary(
    root: FiberRootId,
    first_lane: Lane,
    second_lane: Lane,
    selected_lanes: Lanes,
    ...
    if !first_lane.is_transition()
        || !second_lane.is_transition()
        || first_lane == second_lane
        || selected_lanes.is_empty()
        || !selected_lanes.includes_only_transitions()
        || !selected_lanes.contains_lane(first_lane)
        || !selected_lanes.contains_lane(second_lane)
        || entangled_lanes != selected_lanes
