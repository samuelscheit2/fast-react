...
export const PRIVATE_ADMISSION_722_723_GATE_ID =
  "private-admission-722-723-local-gate-1";
export const PRIVATE_ADMISSION_722_723_GATE_STATUS =
  "recognized-accepted-private-diagnostics-722-723-public-compatibility-blocked";
export const PRIVATE_ADMISSION_722_723_VIOLATION_STATUS =
  "blocked-accepted-private-diagnostics-722-723-with-violations";

export const PRIVATE_ADMISSION_722_723_PUBLIC_COMPATIBILITY_CLAIMS =
  Object.freeze([
...
export const PRIVATE_ADMISSION_722_723_BLOCKED_SURFACES = Object.freeze([
  "test-renderer",
...
export const PRIVATE_ADMISSION_722_723_SKIPPED_WORKERS = Object.freeze([
  "worker-722-package-private-admission-audit-715-721"
]);

export const PRIVATE_ADMISSION_722_723_REQUIRED_DEPENDENCIES = freezeRecord({
  "worker-723-test-renderer-native-serialization-identity-gate": freezeArray([
    "worker-720-test-renderer-serialization-finished-work-identity"
  ])
});

const privateAdmission722723SkippedRowData = Object.freeze([
  skippedRowData({
    workerId: "worker-722-package-private-admission-audit-715-721",
    area: "previous private-admission ledger audit for Workers 715-721",
    skipReason: "ledger-audit-no-new-runtime-capability",
    evidenceRows: [
      evidenceData({
        role: "worker-722-skip-meta-report",
        path: "worker-progress/worker-722-package-private-admission-audit-715-721.md",
        tokens: [
          "# Worker 722: Package Private Admission Audit 715-721",
...
const privateAdmission722723RowData = Object.freeze([
  rowData({
    workerId: "worker-723-test-renderer-native-serialization-identity-gate",
    area: "react-test-renderer create-path native serialization identity diagnostics",
    primaryCompatibilityArea: "test-renderer-create-native-serialization",
    ...
    evidenceRows: [
      evidenceData({
        role: "worker-720-finished-work-identity-dependency",
      ...
      evidenceData({
        role: "worker-723-create-path-native-serialization-report",
        path: "worker-progress/worker-723-test-renderer-native-serialization-identity-gate.md",
        tokens: [
          "# Worker 723 Progress",
          "Added a narrow create-path private native serialization admission gate",
          "`TestRendererPrivateSerializationFinishedWorkIdentityGate`",
...
function admissionClaims(extraClaims = {}) {
  return freezeRecord({
    ...Object.fromEntries(
      PRIVATE_ADMISSION_722_723_PUBLIC_COMPATIBILITY_CLAIMS.map((claimId) => [
        claimId,
        false
      ])
    ),
...
function row({
  ...
  return freezeRecord({
    ...
    localGateCoverage: "private-admission-722-723-local-gate",
...
export const PRIVATE_ADMISSION_722_723_SKIPPED_ROWS = freezeArray(
  privateAdmission722723SkippedRowData.map((sourceRow) =>
    skippedRow({
...
export const PRIVATE_ADMISSION_722_723_ROWS = freezeArray(
  privateAdmission722723RowData.map((sourceRow) =>
    row({
      workerId: sourceRow.workerId,
      area: sourceRow.area,
...
export const PRIVATE_ADMISSION_722_723_WORKERS = freezeArray(
  PRIVATE_ADMISSION_722_723_ROWS.map((row) => row.workerId)
);

export function evaluatePrivateAdmission722723Gate({
  workspaceRoot = DEFAULT_WORKSPACE_ROOT,
  rowOverrides = {},
  skippedRowOverrides = {}
} = {}) {
  const fileCache = new Map();
  const rows = PRIVATE_ADMISSION_722_723_ROWS.map((baseRow) => {
    const override = rowOverrides[baseRow.workerId] ?? {};
    return mergeRowOverride(baseRow, override);
  });
  const skippedRows = PRIVATE_ADMISSION_722_723_SKIPPED_ROWS.map((baseRow) => {
  ...
  const missingWorkerIds = PRIVATE_ADMISSION_722_723_WORKERS.filter(
    (workerId) => !manifestWorkerIds.includes(workerId)
  );
  const unexpectedWorkerIds = manifestWorkerIds.filter(
    (workerId) => !PRIVATE_ADMISSION_722_723_WORKERS.includes(workerId)
  );
  ...
  const missingSkippedWorkerIds =
    PRIVATE_ADMISSION_722_723_SKIPPED_WORKERS.filter(
      (workerId) => !skippedManifestWorkerIds.includes(workerId)
    );
  const unexpectedSkippedWorkerIds = skippedManifestWorkerIds.filter(
    (workerId) => !PRIVATE_ADMISSION_722_723_SKIPPED_WORKERS.includes(workerId)
  );
  ...
  const dependencyMismatches = evaluatedRows.flatMap((evaluatedRow) => {
    ...
    const expectedDependencyWorkerIds =
      PRIVATE_ADMISSION_722_723_REQUIRED_DEPENDENCIES[evaluatedRow.workerId];
  ...
  return freezeRecord({
    id: PRIVATE_ADMISSION_722_723_GATE_ID,
    status:
      violations.length === 0
        ? PRIVATE_ADMISSION_722_723_GATE_STATUS
        : PRIVATE_ADMISSION_722_723_VIOLATION_STATUS,
    queueWorkers: PRIVATE_ADMISSION_722_723_WORKERS,
    skippedWorkers: PRIVATE_ADMISSION_722_723_SKIPPED_WORKERS,
    ...
    manifest: freezeRecord({
      expectedWorkerIds: PRIVATE_ADMISSION_722_723_WORKERS,
      ...
      skippedWorkerIds: PRIVATE_ADMISSION_722_723_SKIPPED_WORKERS,
      expectedSkippedWorkerIds: PRIVATE_ADMISSION_722_723_SKIPPED_WORKERS,
