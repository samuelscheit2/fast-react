...
import { DOM_STYLE_DANGEROUS_HTML_SCENARIOS } from "./dom-style-dangerous-html-scenarios.mjs";
import {
  DOM_STYLE_DANGEROUS_HTML_PROBE_MODES,
  DOM_STYLE_DANGEROUS_HTML_RUNTIME_INVENTORY_PATH,
  DOM_STYLE_DANGEROUS_HTML_SOURCE_DOCUMENTS,
  DOM_STYLE_DANGEROUS_HTML_SUPPORTING_TARGETS,
  DOM_STYLE_DANGEROUS_HTML_TARGET
} from "./dom-style-dangerous-html-targets.mjs";
import { stringifyDomStyleDangerousHtmlOracle } from "./dom-style-dangerous-html-oracle.mjs";

const PROBE_TIMEOUT_MS = 10_000;
const FETCH_TIMEOUT_MS = 30_000;
const ORACLE_TEMP_PREFIX = "fast-react-dom-style-dangerous-html-oracle-";

export async function generateDomStyleDangerousHtmlOracle() {
  const inventoryText = readFileSync(
    new URL(`../${DOM_STYLE_DANGEROUS_HTML_RUNTIME_INVENTORY_PATH}`, import.meta.url),
    "utf8"
  );
  ...
  try {
    ...
    const packageNames = [
      DOM_STYLE_DANGEROUS_HTML_TARGET.packageName,
      ...DOM_STYLE_DANGEROUS_HTML_SUPPORTING_TARGETS.map(
    ...
    for (const mode of DOM_STYLE_DANGEROUS_HTML_PROBE_MODES) {
      serverSerializationObservations[mode.id] =
        DOM_STYLE_DANGEROUS_HTML_SCENARIOS.map((scenario) =>
          runDomStyleDangerousHtmlProbe({
            action: "server",
            mode,
            packageRoots,
            probeFile,
    ...
    return {
      schemaVersion: 1,
      oracleKind: "react-19.2.6-dom-style-dangerous-html-oracle",
      deterministic: true,
      generatedArtifacts: true,
      compatibilityLabel:
        "React DOM 19.2.6 DOM style and dangerouslySetInnerHTML serialization and mutation oracle",
      sources: DOM_STYLE_DANGEROUS_HTML_SOURCE_DOCUMENTS,
      generation: {
        method:
      ...
      sourceInventory: {
        path: DOM_STYLE_DANGEROUS_HTML_RUNTIME_INVENTORY_PATH,
      ...
      reactDomTarget: DOM_STYLE_DANGEROUS_HTML_TARGET,
      supportingRuntimePackages: DOM_STYLE_DANGEROUS_HTML_SUPPORTING_TARGETS,
      probeModes: DOM_STYLE_DANGEROUS_HTML_PROBE_MODES,
      scenarios: DOM_STYLE_DANGEROUS_HTML_SCENARIOS,
      packages,
      fakeDomSubstrate: {
        purpose:
          "exercise React DOM style and innerHTML mutation calls deterministically in Node without adding a DOM dependency to the conformance workspace",
        ...
        doesNotModel: [
          ...
          "CSS parsing, cascade, computed styles, layout, focus, selection, and accessibility tree behavior",
      ...
      intentionalGaps: [
        ...
        {
          id: "browser-cssom-parser-events-and-namespaces-out-of-scope",
          reason:
            "Browser CSSOM parsing/cascade, native HTML parsing, controlled inputs/selects/textareas/forms, event delegation, SVG, MathML, hydration, resources, and server Fizz markers have separate oracle tracks or remain intentionally outside this deterministic fake-DOM probe."
        }
      ]
    };
  } finally {
...
function writeProbeFiles(projectRoot) {
  const scenariosFile = join(
    projectRoot,
    "dom-style-dangerous-html-scenarios.mjs"
  );
  writeFileSync(
    scenariosFile,
    readFileSync(
      new URL("./dom-style-dangerous-html-scenarios.mjs", import.meta.url),
      "utf8"
    ),
    "utf8"
  );

  const probeFile = join(projectRoot, "dom-style-dangerous-html-probe-runner.mjs");
  writeFileSync(
    probeFile,
    readFileSync(
      new URL("./dom-style-dangerous-html-probe-runner.mjs", import.meta.url),
      "utf8"
    ),
    "utf8"
  );
...
function runDomStyleDangerousHtmlProbe({
  action,
  mode,
  packageRoots,
  probeFile,
...
export function stringifyGeneratedDomStyleDangerousHtmlOracle(oracle) {
  return stringifyDomStyleDangerousHtmlOracle(oracle);
}
