...
import { REF_OBJECT_ORACLE_ARTIFACT_PATH } from "./ref-object-targets.mjs";

export function stringifyRefObjectOracle(oracle) {
  return `${JSON.stringify(oracle, null, 2)}\n`;
}

export function readCheckedRefObjectOracle(baseUrl = import.meta.url) {
  return JSON.parse(readCheckedRefObjectOracleText(baseUrl));
}

export function readCheckedRefObjectOracleText(baseUrl = import.meta.url) {
  return readFileSync(
    new URL(`../${REF_OBJECT_ORACLE_ARTIFACT_PATH}`, baseUrl),
    "utf8"
  );
}

export function formatRefObjectOracleAsMarkdown(oracle) {
  ...
  return [
    "# Fast React Ref Object Oracle",
    "",
    "Generated from exact React 19.2.6 artifacts with local Fast React createRef comparisons. This artifact does not claim Fast React behavior compatibility.",
    "",
    "## Scenarios",
    "",
    ...scenarioLines,
...
export function findRefObjectObservation(oracle, modeId, packageName, scenarioId) {
  const collection =
  ...
  if (!observation) {
    throw new Error(
      `Missing ref-object observation: ${modeId}:${packageName}:${scenarioId}`
    );
  }
  return observation;
}
