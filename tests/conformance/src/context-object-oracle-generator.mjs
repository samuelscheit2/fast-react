...
import { stringifyContextObjectOracle } from "./context-object-oracle.mjs";
import { CONTEXT_OBJECT_SCENARIOS } from "./context-object-scenarios.mjs";
import {
  CONTEXT_OBJECT_FAST_REACT_TARGET,
  CONTEXT_OBJECT_PROBE_MODES,
  CONTEXT_OBJECT_REACT_TARGET,
  CONTEXT_OBJECT_SOURCE_DOCUMENTS
} from "./context-object-targets.mjs";

const PROBE_TIMEOUT_MS = 10_000;
const FETCH_TIMEOUT_MS = 30_000;

export async function generateContextObjectOracle() {
  const tempRoot = mkdtempSync(join(tmpdir(), "fast-react-context-oracle-"));
  ...
  try {
    ...
    for (const mode of CONTEXT_OBJECT_PROBE_MODES) {
      ...
      for (const scenario of CONTEXT_OBJECT_SCENARIOS) {
        const reactObservation = runContextObjectProbe({
          mode,
          packageName: CONTEXT_OBJECT_REACT_TARGET.packageName,
          probeFile,
          projectRoot,
          scenarioId: scenario.id
        });
        const fastReactObservation = runContextObjectProbe({
          mode,
          packageName: CONTEXT_OBJECT_FAST_REACT_TARGET.packageName,
          probeFile,
          projectRoot,
          scenarioId: scenario.id
        });
    ...
    return {
      schemaVersion: 1,
      oracleKind: "react-19.2.6-context-object-oracle",
      deterministic: true,
      generatedArtifacts: true,
      compatibilityLabel: "React 19.2.6 createContext direct-object oracle",
      sources: CONTEXT_OBJECT_SOURCE_DOCUMENTS,
      generation: {
        method:
      ...
      evidenceClaims: {
        ...
        reactContextObjectBehaviorProbed: true,
      ...
      reactTarget: CONTEXT_OBJECT_REACT_TARGET,
      fastReactTarget: CONTEXT_OBJECT_FAST_REACT_TARGET,
      probeModes: CONTEXT_OBJECT_PROBE_MODES,
      scenarios: CONTEXT_OBJECT_SCENARIOS,
      packages: {
        react: reactEvidence,
        "@fast-react/react": {
          packageName: CONTEXT_OBJECT_FAST_REACT_TARGET.packageName,
          version: CONTEXT_OBJECT_FAST_REACT_TARGET.version,
          role: CONTEXT_OBJECT_FAST_REACT_TARGET.role,
          source:
            "local workspace package copied for createContext behavior comparison",
...
async function fetchAndExtractReact({ nodeModulesRoot, tempRoot }) {
  const metadata = await fetchPackageMetadata(CONTEXT_OBJECT_REACT_TARGET);
  ...
  if (packageJson.name !== CONTEXT_OBJECT_REACT_TARGET.packageName) {
    throw new Error(`react tarball package name mismatch: ${packageJson.name}`);
  }
  if (packageJson.version !== CONTEXT_OBJECT_REACT_TARGET.version) {
    throw new Error(
  ...
  return {
    packageName: CONTEXT_OBJECT_REACT_TARGET.packageName,
    version: CONTEXT_OBJECT_REACT_TARGET.version,
    role: CONTEXT_OBJECT_REACT_TARGET.role,
    registry: {
      metadataUrl: registryPackageVersionUrl(CONTEXT_OBJECT_REACT_TARGET),
...
function writeProbeFile(projectRoot) {
  const probeFile = join(projectRoot, "context-object-probe-runner.mjs");
  writeFileSync(
    probeFile,
    readFileSync(
...
function runContextObjectProbe({
  mode,
  packageName,
  probeFile,
  projectRoot,
...
function compareFastReactToReact({
  ...
  return {
    ...
    reason: fastReactUnsupported
      ? "Fast React currently throws structured placeholder errors for this createContext behavior."
      : equal
        ? "Normalized observations matched, but this oracle does not claim Fast React compatibility."
        : "Fast React normalized observation differs from the React 19.2.6 createContext oracle."
  };
}
...
export function stringifyGeneratedContextObjectOracle(oracle) {
  return stringifyContextObjectOracle(oracle);
}
