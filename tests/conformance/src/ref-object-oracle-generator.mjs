...
import { stringifyRefObjectOracle } from "./ref-object-oracle.mjs";
import { REF_OBJECT_SCENARIOS } from "./ref-object-scenarios.mjs";
import {
  REF_OBJECT_FAST_REACT_TARGET,
  REF_OBJECT_PROBE_MODES,
  REF_OBJECT_REACT_TARGET,
  REF_OBJECT_SOURCE_DOCUMENTS
} from "./ref-object-targets.mjs";

const PROBE_TIMEOUT_MS = 10_000;
const FETCH_TIMEOUT_MS = 30_000;

export async function generateRefObjectOracle() {
  const tempRoot = mkdtempSync(join(tmpdir(), "fast-react-ref-oracle-"));
  ...
  try {
    ...
    for (const mode of REF_OBJECT_PROBE_MODES) {
      ...
      for (const scenario of REF_OBJECT_SCENARIOS) {
        const reactObservation = runRefObjectProbe({
          mode,
          packageName: REF_OBJECT_REACT_TARGET.packageName,
          probeFile,
          projectRoot,
          scenarioId: scenario.id
        });
        const fastReactObservation = runRefObjectProbe({
          mode,
          packageName: REF_OBJECT_FAST_REACT_TARGET.packageName,
          probeFile,
          projectRoot,
          scenarioId: scenario.id
        });
    ...
    return {
      schemaVersion: 1,
      oracleKind: "react-19.2.6-ref-object-oracle",
      deterministic: true,
      generatedArtifacts: true,
      compatibilityLabel: "React 19.2.6 createRef ref-object oracle",
      sources: REF_OBJECT_SOURCE_DOCUMENTS,
      generation: {
        method:
      ...
      evidenceClaims: {
        ...
        reactRefObjectBehaviorProbed: true,
        ...
        beforeWorker024: {
          ...
          statusCounts: {
            "unsupported-placeholder":
              REF_OBJECT_PROBE_MODES.length * REF_OBJECT_SCENARIOS.length
          },
          compatibilityClaimed: false
        },
        afterWorker024: {
      ...
      reactTarget: REF_OBJECT_REACT_TARGET,
      fastReactTarget: REF_OBJECT_FAST_REACT_TARGET,
      probeModes: REF_OBJECT_PROBE_MODES,
      scenarios: REF_OBJECT_SCENARIOS,
      packages: {
        react: reactEvidence,
        "@fast-react/react": {
          packageName: REF_OBJECT_FAST_REACT_TARGET.packageName,
          version: REF_OBJECT_FAST_REACT_TARGET.version,
          role: REF_OBJECT_FAST_REACT_TARGET.role,
          source: "local workspace package copied for createRef behavior comparison",
...
async function fetchAndExtractReact({ nodeModulesRoot, tempRoot }) {
  const metadata = await fetchPackageMetadata(REF_OBJECT_REACT_TARGET);
  ...
  if (packageJson.name !== REF_OBJECT_REACT_TARGET.packageName) {
    throw new Error(`react tarball package name mismatch: ${packageJson.name}`);
  }
  if (packageJson.version !== REF_OBJECT_REACT_TARGET.version) {
    throw new Error(
  ...
  return {
    packageName: REF_OBJECT_REACT_TARGET.packageName,
    version: REF_OBJECT_REACT_TARGET.version,
    role: REF_OBJECT_REACT_TARGET.role,
    registry: {
      metadataUrl: registryPackageVersionUrl(REF_OBJECT_REACT_TARGET),
...
function writeProbeFile(projectRoot) {
  const probeFile = join(projectRoot, "ref-object-probe-runner.mjs");
  writeFileSync(
    probeFile,
    readFileSync(new URL("./ref-object-probe-runner.mjs", import.meta.url), "utf8"),
    "utf8"
  );
  return probeFile;
}

function runRefObjectProbe({
  mode,
  packageName,
  probeFile,
  projectRoot,
...
function compareFastReactToReact({
  ...
  return {
    ...
    reason: fastReactUnsupported
      ? "Fast React currently throws structured placeholder errors for this createRef behavior."
      : equal
        ? "Normalized observations matched, but this oracle does not claim Fast React compatibility."
        : "Fast React normalized observation differs from the React 19.2.6 createRef oracle."
  };
}
...
export function stringifyGeneratedRefObjectOracle(oracle) {
  return stringifyRefObjectOracle(oracle);
}
