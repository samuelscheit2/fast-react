...
import { stringifySchedulerVariantOracle } from "./scheduler-variant-oracle.mjs";
import {
  ...
  SCHEDULER_VARIANT_SOURCE_DOCUMENTS,
...
export async function generateSchedulerVariantOracle() {
  const tempRoot = mkdtempSync(
    join(tmpdir(), "fast-react-scheduler-variant-oracle-")
  );
  ...
  try {
    ...
    const scenarioOptions = {
      resolutionSpecifiers: SCHEDULER_VARIANT_RESOLUTION_SPECIFIERS,
      deepCjsSpecifiers: SCHEDULER_VARIANT_DEEP_CJS_SPECIFIERS
    };
    const observations = {};
    ...
    for (const mode of SCHEDULER_VARIANT_PROBE_MODES) {
      ...
      for (const scenario of SCHEDULER_VARIANT_SCENARIOS) {
        observations[mode.id][scenarioObservationKey(scenario.id)] =
          runSchedulerVariantProbe({
            mode,
            packageName: SCHEDULER_VARIANT_TARGET.packageName,
            probeFile,
            projectRoot,
    ...
    return {
      schemaVersion: 1,
      oracleKind: "scheduler-0.27.0-variant-oracle",
      deterministic: true,
      generatedArtifacts: true,
      compatibilityLabel:
        "scheduler@0.27.0 variant and physical deep-import oracle",
      sources: SCHEDULER_VARIANT_SOURCE_DOCUMENTS,
      generation: {
        method:
      ...
      schedulerTarget: SCHEDULER_VARIANT_TARGET,
      probeModes: SCHEDULER_VARIANT_PROBE_MODES,
      scenarios: SCHEDULER_VARIANT_SCENARIOS,
      compatibilityGateRecommendations: SCHEDULER_VARIANT_GATE_DECISIONS,
      packages: {
        scheduler: schedulerEvidence
      },
      packageResolution:
        observations[SCHEDULER_VARIANT_PROBE_MODES[0].id]
          .packageMetadataAndResolution,
      observations
    };
  } finally {
...
async function fetchAndExtractScheduler({ nodeModulesRoot, tempRoot }) {
  const metadata = await fetchPackageMetadata(SCHEDULER_VARIANT_TARGET);
  const dist = metadata.dist ?? {};
  if (!dist.tarball) {
    throw new Error("scheduler@0.27.0 metadata did not include a tarball URL");
  }

  if (dist.integrity !== SCHEDULER_VARIANT_TARGET.expectedDistIntegrity) {
    throw new Error("scheduler@0.27.0 dist.integrity did not match evidence");
  }
  if (dist.shasum !== SCHEDULER_VARIANT_TARGET.expectedDistShasum) {
  ...
  if (packageJson.name !== SCHEDULER_VARIANT_TARGET.packageName) {
    throw new Error(`scheduler tarball package name mismatch: ${packageJson.name}`);
  }
  if (packageJson.version !== SCHEDULER_VARIANT_TARGET.version) {
    throw new Error(
  ...
  return {
    packageName: SCHEDULER_VARIANT_TARGET.packageName,
    version: SCHEDULER_VARIANT_TARGET.version,
    role: SCHEDULER_VARIANT_TARGET.role,
    targetStatus: SCHEDULER_VARIANT_TARGET.targetStatus,
    registry: {
      metadataUrl: registryPackageVersionUrl(SCHEDULER_VARIANT_TARGET),
...
function writeProbeFile(projectRoot) {
  const probeFile = join(projectRoot, "scheduler-variant-probe-runner.mjs");
  writeFileSync(
    probeFile,
    readFileSync(
      new URL("./scheduler-variant-probe-runner.mjs", import.meta.url),
      "utf8"
    ),
    "utf8"
  );
...
function runSchedulerVariantProbe({
  mode,
  packageName,
  probeFile,
  projectRoot,
...
export function stringifyGeneratedSchedulerVariantOracle(oracle) {
  return stringifySchedulerVariantOracle(oracle);
}
