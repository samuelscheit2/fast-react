...
import { stringifyWrapperObjectOracle } from "./wrapper-object-oracle.mjs";
import { WRAPPER_OBJECT_SCENARIOS } from "./wrapper-object-scenarios.mjs";
import {
  WRAPPER_OBJECT_FAST_REACT_TARGET,
  WRAPPER_OBJECT_PROBE_MODES,
  WRAPPER_OBJECT_REACT_TARGET,
  WRAPPER_OBJECT_SOURCE_DOCUMENTS
} from "./wrapper-object-targets.mjs";

const PROBE_TIMEOUT_MS = 10_000;
const FETCH_TIMEOUT_MS = 30_000;

export async function generateWrapperObjectOracle() {
  const tempRoot = mkdtempSync(join(tmpdir(), "fast-react-wrapper-oracle-"));
  ...
  try {
    ...
    for (const mode of WRAPPER_OBJECT_PROBE_MODES) {
      ...
      for (const scenario of WRAPPER_OBJECT_SCENARIOS) {
        const reactObservation = runWrapperObjectProbe({
          mode,
          packageName: WRAPPER_OBJECT_REACT_TARGET.packageName,
          probeFile,
          projectRoot,
          scenarioId: scenario.id
        });
        const fastReactObservation = runWrapperObjectProbe({
          mode,
          packageName: WRAPPER_OBJECT_FAST_REACT_TARGET.packageName,
          probeFile,
          projectRoot,
          scenarioId: scenario.id
        });
    ...
    return {
      schemaVersion: 1,
      oracleKind: "react-19.2.6-wrapper-object-oracle",
      deterministic: true,
      generatedArtifacts: true,
      compatibilityLabel: "React 19.2.6 memo/lazy wrapper-object oracle",
      sources: WRAPPER_OBJECT_SOURCE_DOCUMENTS,
      generation: {
        method:
      ...
      evidenceClaims: {
        ...
        reactWrapperObjectBehaviorProbed: true,
      ...
      implementationComparison: {
        beforeWorker026: {
          ...
          statusCounts: {
            "known-mismatch": WRAPPER_OBJECT_PROBE_MODES.length,
            "unsupported-placeholder":
              WRAPPER_OBJECT_PROBE_MODES.length *
              (WRAPPER_OBJECT_SCENARIOS.length - 1)
          },
          compatibilityClaimed: false
        },
        afterWorker026: {
      ...
      reactTarget: WRAPPER_OBJECT_REACT_TARGET,
      fastReactTarget: WRAPPER_OBJECT_FAST_REACT_TARGET,
      probeModes: WRAPPER_OBJECT_PROBE_MODES,
      scenarios: WRAPPER_OBJECT_SCENARIOS,
      packages: {
        react: reactEvidence,
        "@fast-react/react": {
          packageName: WRAPPER_OBJECT_FAST_REACT_TARGET.packageName,
          version: WRAPPER_OBJECT_FAST_REACT_TARGET.version,
          role: WRAPPER_OBJECT_FAST_REACT_TARGET.role,
          source:
            "local workspace package copied for memo/lazy wrapper-object behavior comparison",
...
async function fetchAndExtractReact({ nodeModulesRoot, tempRoot }) {
  const metadata = await fetchPackageMetadata(WRAPPER_OBJECT_REACT_TARGET);
  ...
  if (packageJson.name !== WRAPPER_OBJECT_REACT_TARGET.packageName) {
    throw new Error(`react tarball package name mismatch: ${packageJson.name}`);
  }
  if (packageJson.version !== WRAPPER_OBJECT_REACT_TARGET.version) {
    throw new Error(
  ...
  return {
    packageName: WRAPPER_OBJECT_REACT_TARGET.packageName,
    version: WRAPPER_OBJECT_REACT_TARGET.version,
    role: WRAPPER_OBJECT_REACT_TARGET.role,
    registry: {
      metadataUrl: registryPackageVersionUrl(WRAPPER_OBJECT_REACT_TARGET),
...
function writeProbeFile(projectRoot) {
  const probeFile = join(projectRoot, "wrapper-object-probe-runner.mjs");
  writeFileSync(
    probeFile,
    readFileSync(
      new URL("./wrapper-object-probe-runner.mjs", import.meta.url),
      "utf8"
    ),
    "utf8"
  );
...
function runWrapperObjectProbe({
  mode,
  packageName,
  probeFile,
  projectRoot,
...
function compareFastReactToReact({
  ...
  return {
    ...
    reason: fastReactUnsupported
      ? "Fast React currently throws structured placeholder errors for this memo/lazy wrapper-object behavior."
      : equal
        ? "Normalized observations matched, but this oracle does not claim Fast React compatibility."
        : "Fast React normalized observation differs from the React 19.2.6 memo/lazy wrapper-object oracle."
  };
}
...
export function stringifyGeneratedWrapperObjectOracle(oracle) {
  return stringifyWrapperObjectOracle(oracle);
}
