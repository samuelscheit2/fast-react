...
import { CONTEXT_OBJECT_ORACLE_ARTIFACT_PATH } from "./context-object-targets.mjs";

export function stringifyContextObjectOracle(oracle) {
  return `${JSON.stringify(oracle, null, 2)}\n`;
}

export function readCheckedContextObjectOracle(baseUrl = import.meta.url) {
  return JSON.parse(readCheckedContextObjectOracleText(baseUrl));
}

export function readCheckedContextObjectOracleText(baseUrl = import.meta.url) {
  return readFileSync(
    new URL(`../${CONTEXT_OBJECT_ORACLE_ARTIFACT_PATH}`, baseUrl),
    "utf8"
  );
}

export function formatContextObjectOracleAsMarkdown(oracle) {
  ...
  return [
    "# Fast React Context Object Oracle",
    "",
    "Generated from exact React 19.2.6 artifacts with local Fast React createContext direct-object comparisons. This artifact does not claim Fast React behavior compatibility.",
    "",
    "## Scenarios",
    "",
    ...scenarioLines,
...
export function findContextObjectObservation(
  oracle,
  modeId,
  packageName,
  scenarioId
  ...
  if (!observation) {
    throw new Error(
      `Missing context-object observation: ${modeId}:${packageName}:${scenarioId}`
    );
  }
  return observation;
}
