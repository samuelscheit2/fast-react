...
import {
  ...
  ELEMENT_OBJECT_SOURCE_DOCUMENTS
} from "./element-object-targets.mjs";
import { ELEMENT_OBJECT_SCENARIOS } from "./element-object-scenarios.mjs";
import { stringifyElementObjectOracle } from "./element-object-oracle.mjs";

const PROBE_TIMEOUT_MS = 10_000;
const FETCH_TIMEOUT_MS = 30_000;

export async function generateElementObjectOracle() {
  const tempRoot = mkdtempSync(join(tmpdir(), "fast-react-element-oracle-"));
  ...
  try {
    ...
    for (const mode of ELEMENT_OBJECT_PROBE_MODES) {
      ...
      for (const scenario of ELEMENT_OBJECT_SCENARIOS) {
        const reactObservation = runElementObjectProbe({
          mode,
          packageName: ELEMENT_OBJECT_REACT_TARGET.packageName,
          probeFile,
          projectRoot,
          scenarioId: scenario.id
        });
        const fastReactObservation = runElementObjectProbe({
          mode,
          packageName: ELEMENT_OBJECT_FAST_REACT_TARGET.packageName,
          probeFile,
          projectRoot,
          scenarioId: scenario.id
        });
    ...
    return {
      schemaVersion: 1,
      oracleKind: "react-19.2.6-element-object-oracle",
      deterministic: true,
      generatedArtifacts: true,
      compatibilityLabel: "React 19.2.6 element object oracle",
      sources: ELEMENT_OBJECT_SOURCE_DOCUMENTS,
      generation: {
        method:
      ...
      evidenceClaims: {
        ...
        reactElementObjectBehaviorProbed: true,
      ...
      reactTarget: ELEMENT_OBJECT_REACT_TARGET,
      fastReactTarget: ELEMENT_OBJECT_FAST_REACT_TARGET,
      probeModes: ELEMENT_OBJECT_PROBE_MODES,
      scenarios: ELEMENT_OBJECT_SCENARIOS,
      packages: {
        react: reactEvidence,
        "@fast-react/react": {
          packageName: ELEMENT_OBJECT_FAST_REACT_TARGET.packageName,
          version: ELEMENT_OBJECT_FAST_REACT_TARGET.version,
          role: ELEMENT_OBJECT_FAST_REACT_TARGET.role,
          source: "local workspace package copied for behavior comparison",
...
async function fetchAndExtractReact({ nodeModulesRoot, tempRoot }) {
  const metadata = await fetchPackageMetadata(ELEMENT_OBJECT_REACT_TARGET);
  ...
  if (packageJson.name !== ELEMENT_OBJECT_REACT_TARGET.packageName) {
    throw new Error(`react tarball package name mismatch: ${packageJson.name}`);
  }
  if (packageJson.version !== ELEMENT_OBJECT_REACT_TARGET.version) {
    throw new Error(
  ...
  return {
    packageName: ELEMENT_OBJECT_REACT_TARGET.packageName,
    version: ELEMENT_OBJECT_REACT_TARGET.version,
    role: ELEMENT_OBJECT_REACT_TARGET.role,
    registry: {
      metadataUrl: registryPackageVersionUrl(ELEMENT_OBJECT_REACT_TARGET),
...
function writeProbeFile(projectRoot) {
  const probeFile = join(projectRoot, "element-object-probe-runner.mjs");
  writeFileSync(
    probeFile,
    readFileSync(
      new URL("./element-object-probe-runner.mjs", import.meta.url),
      "utf8"
    ),
    "utf8"
  );
...
function runElementObjectProbe({
  mode,
  packageName,
  probeFile,
  projectRoot,
...
function compareFastReactToReact({
  ...
  return {
    ...
    reason: fastReactUnsupported
      ? "Fast React currently throws structured placeholder errors for this element behavior."
      : equal
...
export function stringifyGeneratedElementObjectOracle(oracle) {
  return stringifyElementObjectOracle(oracle);
}
