...
if (!action || !scenarioId) {
  process.stderr.write(
    "Usage: node dom-style-dangerous-html-probe-runner.mjs <server|client> <scenario>\n"
  );
  process.exit(1);
}

const scenario = DOM_STYLE_DANGEROUS_HTML_SCENARIOS.find(
  (candidate) => candidate.id === scenarioId
);
if (!scenario) {
  process.stderr.write(`Unknown dom-style-dangerous-html scenario: ${scenarioId}\n`);
  process.exit(1);
}
...
process.stderr.write(`Unsupported dom-style-dangerous-html probe action: ${action}\n`);
process.exit(1);
...
function materializeValue(descriptor) {
  switch (descriptor.type) {
    ...
    case "number":
      if (descriptor.nonFinite && descriptor.value === "NaN") {
        return NaN;
      }
      if (descriptor.nonFinite && descriptor.value === "Infinity") {
        return Infinity;
      }
      return descriptor.value;
    case "null":
...
function installFakeDom() {
  ...
  function dumpNode(node) {
    ...
    if (node.nodeType === 1) {
      const styleProperties =
        node.style?._properties instanceof Map
          ? Array.from(node.style._properties.entries()).sort(([left], [right]) =>
              left.localeCompare(right)
            )
          : [];
      result.localName = node.localName;
      result.namespaceURI = node.namespaceURI;
      result.styleProperties = styleProperties;
      result.activeStyleProperties = styleProperties.filter(
        ([, value]) => value !== ""
      );
  ...
  Object.defineProperty(globalThis, "navigator", {
    value: {
      userAgent: "fast-react-style-dangerous-html-fake-dom"
    },
    configurable: true
  });
    ...
    describeSubstrate() {
      return {
        ...
        records: [
          ...
          "style.setProperty and style.removeProperty calls",
        ...
        limitations: [
          ...
          "no CSS parser, cascade, layout, selection, focus, or accessibility tree",
