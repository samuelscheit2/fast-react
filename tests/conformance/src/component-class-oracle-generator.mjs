...
import { stringifyComponentClassOracle } from "./component-class-oracle.mjs";
import { COMPONENT_CLASS_SCENARIOS } from "./component-class-scenarios.mjs";
import {
  COMPONENT_CLASS_FAST_REACT_TARGET,
  COMPONENT_CLASS_PROBE_MODES,
  COMPONENT_CLASS_REACT_TARGET,
  COMPONENT_CLASS_SOURCE_DOCUMENTS
} from "./component-class-targets.mjs";

const PROBE_TIMEOUT_MS = 10_000;
const FETCH_TIMEOUT_MS = 30_000;

export async function generateComponentClassOracle() {
  const tempRoot = mkdtempSync(join(tmpdir(), "fast-react-component-oracle-"));
  ...
  try {
    ...
    for (const mode of COMPONENT_CLASS_PROBE_MODES) {
      ...
      for (const scenario of COMPONENT_CLASS_SCENARIOS) {
        const reactObservation = runComponentClassProbe({
          mode,
          packageName: COMPONENT_CLASS_REACT_TARGET.packageName,
          probeFile,
          projectRoot,
          scenarioId: scenario.id
        });
        const fastReactObservation = runComponentClassProbe({
          mode,
          packageName: COMPONENT_CLASS_FAST_REACT_TARGET.packageName,
          probeFile,
          projectRoot,
          scenarioId: scenario.id
        });
    ...
    return {
      schemaVersion: 1,
      oracleKind: "react-19.2.6-component-class-oracle",
      deterministic: true,
      generatedArtifacts: true,
      compatibilityLabel:
        "React 19.2.6 Component and PureComponent direct-behavior oracle",
      sources: COMPONENT_CLASS_SOURCE_DOCUMENTS,
      generation: {
        method:
      ...
      evidenceClaims: {
        ...
        reactComponentClassBehaviorProbed: true,
      ...
      reactTarget: COMPONENT_CLASS_REACT_TARGET,
      fastReactTarget: COMPONENT_CLASS_FAST_REACT_TARGET,
      probeModes: COMPONENT_CLASS_PROBE_MODES,
      scenarios: COMPONENT_CLASS_SCENARIOS,
      packages: {
        react: reactEvidence,
        "@fast-react/react": {
          packageName: COMPONENT_CLASS_FAST_REACT_TARGET.packageName,
          version: COMPONENT_CLASS_FAST_REACT_TARGET.version,
          role: COMPONENT_CLASS_FAST_REACT_TARGET.role,
          source:
            "local workspace package copied for Component and PureComponent direct-behavior comparison",
...
async function fetchAndExtractReact({ nodeModulesRoot, tempRoot }) {
  const metadata = await fetchPackageMetadata(COMPONENT_CLASS_REACT_TARGET);
  ...
  if (packageJson.name !== COMPONENT_CLASS_REACT_TARGET.packageName) {
    throw new Error(`react tarball package name mismatch: ${packageJson.name}`);
  }
  if (packageJson.version !== COMPONENT_CLASS_REACT_TARGET.version) {
    throw new Error(
  ...
  return {
    packageName: COMPONENT_CLASS_REACT_TARGET.packageName,
    version: COMPONENT_CLASS_REACT_TARGET.version,
    role: COMPONENT_CLASS_REACT_TARGET.role,
    registry: {
      metadataUrl: registryPackageVersionUrl(COMPONENT_CLASS_REACT_TARGET),
...
function writeProbeFile(projectRoot) {
  const probeFile = join(projectRoot, "component-class-probe-runner.mjs");
  writeFileSync(
    probeFile,
    readFileSync(
      new URL("./component-class-probe-runner.mjs", import.meta.url),
      "utf8"
    ),
    "utf8"
  );
...
function runComponentClassProbe({
  mode,
  packageName,
  probeFile,
  projectRoot,
...
function compareFastReactToReact({
  ...
  return {
    ...
    reason: fastReactUnsupported
      ? "Fast React currently throws structured placeholder errors for this Component or PureComponent behavior."
      : equal
        ? "Normalized observations matched, but this oracle does not claim Fast React compatibility."
        : "Fast React normalized observation differs from the React 19.2.6 Component/PureComponent oracle."
  };
}
...
export function stringifyGeneratedComponentClassOracle(oracle) {
  return stringifyComponentClassOracle(oracle);
}
