...
const actNestedScopePassiveFlushOrder = [
...
const actSchedulerCjsDevelopmentReactQueueDiagnosticRecordIds = [
  ...
  actNestedScopePassiveFlushRecordId,
  "react-private-act-internal-test-queue-factories"
];
...
const acceptedPrivateActFlushCjsDevelopmentPrerequisiteIds = [
  ...
  actNestedScopePassiveFlushPrerequisiteId,
...
test("react-test-renderer CJS private act diagnostics consume update execution with passive drain metadata", () => {
  for (const entry of cjsEntrypoints) {
    ...
    if (entry.production) {
      assert.equal(
        Object.hasOwn(report, "hostComponentStyleUpdateRecorded"),
        false,
        entry.entrypoint
      );
    } else {
...
test("react-test-renderer CJS development private act diagnostics flush accepted nested-scope passive metadata in order", () => {
  ...
  assert.deepEqual(
    ...
    actNestedScopePassiveFlushOrder
  );

  const metadata = diagnostics.createAcceptedPendingPassiveFlushMetadata([
    diagnostics.createAcceptedPendingPassiveFlushRecord({
      label: "outer-passive-work",
  ...
  assert.equal(described.id, actNestedScopePassiveFlushRecordId);
  assert.equal(described.status, actNestedScopePassiveFlushStatus);
  assert.equal(described.accepted, true);
  assert.equal(described.rejectionReason, null);
  assert.deepEqual(described.passiveFlushOrder, actNestedScopePassiveFlushOrder);
  assert.equal(described.outerScopeDepth, 1);
  assert.equal(described.innerScopeDepth, 2);
  assert.equal(described.passiveFlushOrderIndex, 2);
  assert.equal(described.pendingBefore, 2);
  assert.equal(described.pendingPassiveRecordCount, 2);
  assert.equal(described.consumesNestedScopeBlockerDiagnostics, true);
  assert.equal(described.drainsAcceptedPendingPassiveFlushMetadata, true);
  assert.equal(described.deterministicFlushOrder, true);
  assert.equal(described.publicActCompatibilityClaimed, false);
  assert.equal(described.executesPassiveEffects, false);

  const report = diagnostics.flushAcceptedNestedActScopePassiveWork(metadata);
  assert.equal(report.id, actNestedScopePassiveFlushRecordId);
  assert.equal(report.status, actNestedScopePassiveFlushStatus);
  assert.equal(report.accepted, true);
  assert.deepEqual(report.passiveFlushOrder, actNestedScopePassiveFlushOrder);
  assert.deepEqual(
    ...
    [
      [0, "outer-passive-work", "PendingPassiveCommitHandoff", 1, 0],
      [1, "inner-passive-work", "PassiveEffectSchedulerFlushExecutionRecord", 1, 1]
    ]
  );
  ...
  assert.equal(
    bridge.canConsumePrivateCreateNativeBridgeHostOutputHandoff(
      createRequest,
      {
        ...createHostOutputHandoff,
        commitCurrent: {
          ...createHostOutputHandoff.commitCurrent,
          slot: createHostOutputHandoff.commitCurrent.slot + 1
        }
      }
    ),
    false
...
test("react-test-renderer CJS development private toJSON facade consumes accepted native execution records", () => {
  ...
  assert.equal(
    facade.unmountNativeExecutionFinishedWorkIdentityAdmissionWorker,
    "worker-733-test-renderer-unmount-finished-work-identity"
  );
  assert.equal(
    facade.unmountNativeExecutionRequiresFinishedWorkIdentity,
    true
  );
  assert.equal(facade.rejectsStaleUpdateFinishedWorkIdentity, true);
  assert.equal(facade.rejectsStaleUnmountFinishedWorkIdentity, true);
  assert.equal(facade.requiresUnmountDeletionCleanupHandoffEvidence, true);
  assert.equal(
  ...
  assert.equal(createEvidence.finishedWorkIdentity.publicSurface, "create().toJSON");
  assert.equal(
  ...
  const missingIdentityError = captureThrown(() =>
    facade.createAcceptedNativeExecutionDiagnosticResult(
      createResult,
      createJSONReport
    )
  ...
  assert.equal(missingIdentityError.nativeExecution, false);
  assert.equal(
    facade.canCreateAcceptedNativeExecutionDiagnosticResult(
      createResult,
      createJSONReport,
      {
        ...createJSONIdentity,
        rootRequestSequence: createJSONIdentity.rootRequestSequence + 1
      }
    ),
    false
  );
  const staleIdentityError = captureThrown(() =>
    facade.createAcceptedNativeExecutionDiagnosticResult(
      createResult,
      createJSONReport,
      {
  ...
  const foreignIdentityError = captureThrown(() =>
    facade.createAcceptedNativeExecutionDiagnosticResult(
      ...
      {
        ...createJSONIdentity,
        rootId: `${createJSONIdentity.rootId}:foreign`
      }
    )
  );
  assert.equal(
    foreignIdentityError.name,
    "FastReactTestRendererPrivateToJSONSerializationError"
  );
  assert.match(foreignIdentityError.message, /foreign root/u);
  const sourceReportError = captureThrown(() =>
    facade.createAcceptedNativeExecutionDiagnosticResult(
      createResult,
      undefined,
      createJSONIdentity
  ...
  assert.equal(
    unmountEvidence.finishedWorkIdentity.rootRequestId,
  ...
  const staleUnmountIdentityError = captureThrown(() =>
    facade.createAcceptedNativeExecutionDiagnosticResult(
      unmountResult,
      privateToJSONReport({
        hostOutputUpdateKind: "Unmount",
  ...
  assert.equal(
    staleUnmountIdentityError.name,
    "FastReactTestRendererPrivateToJSONSerializationError"
  );
  assert.match(staleUnmountIdentityError.message, /request sequence is stale/u);
  const nativeUnmountClaimError = captureThrown(() =>
    facade.createAcceptedNativeExecutionDiagnosticResult(
      {
        ...unmountResult,
  ...
  assert.equal(
    nativeUnmountClaimError.name,
    "FastReactTestRendererPrivateToJSONSerializationError"
  );
  assert.equal(nativeUnmountClaimError.nativeExecution, false);
  const missingCleanupEvidenceError = captureThrown(() =>
    facade.createAcceptedNativeExecutionDiagnosticResult(
      {
        ...unmountResult,
        privateUnmountNativeBridgeAdmission: {
          ...unmountResult.privateUnmountNativeBridgeAdmission,
          cleanupHandoffAccepted: false
        }
      },
      privateToJSONReport({
        hostOutputUpdateKind: "Unmount",
  ...
  assert.equal(
    missingCleanupEvidenceError.name,
    "FastReactTestRendererPrivateToJSONSerializationError"
  );
});
...
test("react-test-renderer CJS development private toTree facade consumes accepted native execution records", () => {
  ...
  const createTreeReport = privateToTreeReport({
    hostOutputUpdateKind: "Create",
    rowId: null,
    rowShape: null,
    rootChildCount: 1,
  ...
  const createTreeIdentity = privateSerializationFinishedWorkIdentityEvidence({
    rootRequest: createRequest,
  ...
  const wrongSurfaceIdentityError = captureThrown(() =>
    facade.createAcceptedNativeExecutionDiagnosticResult(
      createResult,
      createTreeReport,
      {
  ...
  assert.equal(
    wrongSurfaceIdentityError.name,
    "FastReactTestRendererPrivateToTreeMetadataError"
  );
  assert.match(
    wrongSurfaceIdentityError.message,
  ...
  assert.equal(unmountEvidence.nativeExecution, false);
  assert.equal(unmountEvidence.compatibilityClaimed, false);
  const publicUnmountTreeIdentityError = captureThrown(() =>
    facade.createAcceptedNativeExecutionDiagnosticResult(
      unmountResult,
      privateToTreeReport({
        hostOutputUpdateKind: "Unmount",
  ...
  assert.equal(
    publicUnmountTreeIdentityError.name,
    "FastReactTestRendererPrivateToTreeMetadataError"
  );
  const staleUnmountTreeExecutionError = captureThrown(() =>
    facade.createAcceptedNativeExecutionDiagnosticResult(
      {
        ...unmountResult,
        requestSequence: unmountResult.requestSequence + 1
      },
      privateToTreeReport({
        hostOutputUpdateKind: "Unmount",
        rowId: privateToJSONUnmountHostOutputRowId,
  ...
  assert.equal(
    staleUnmountTreeExecutionError.name,
    "FastReactTestRendererPrivateToTreeMetadataError"
  );
  const missingTreeCleanupEvidenceError = captureThrown(() =>
    facade.createAcceptedNativeExecutionDiagnosticResult(
      {
        ...unmountResult,
  ...
  assert.equal(
    missingTreeCleanupEvidenceError.name,
    "FastReactTestRendererPrivateToTreeMetadataError"
  );
...
test("react-test-renderer package root and CJS consume source-owned root lifecycle execution rows only", () => {
  for (const entry of entrypoints) {
    ...
    assert.equal(
      bridge.canConsumePrivateRootLifecycleExecutionEvidence(rows),
      false,
      `${entry.entrypoint} replay rejected`
    );
    const replayError = captureThrown(() =>
      bridge.consumePrivateRootLifecycleExecutionEvidence(rows)
    );
    assert.match(replayError.message, /already been accepted/u);
...
test("react-test-renderer private serialization currentness requires source-owned lifecycle evidence", () => {
  const contexts = entrypoints
    .filter((entry) =>
      hasCompletePrivateRootLifecycleExecutionSourceRecords(entry.entrypoint)
    )
    .map((entry) => {
    const moduleExports = loadFresh(entry.modulePath);
    const bridge = assertPrivateRootRequestBridge(
  ...
  assert.notEqual(contexts.length, 0);
  ...
  if (packageContext !== undefined && cjsContext !== undefined) {
...
test("react-test-renderer lifecycle-gated TestInstance diagnostics reject cross-entrypoint rows", () => {
  const packageEntry = entrypoints.find(
  ...
  const packageBridge = assertPrivateRootRequestBridge(
    packageExports,
    packageEntry.entrypoint
  );
  const packageRows = createPrivateRootLifecycleExecutionRows(
    packageExports,
    packageBridge
  );

  const cjsExports = loadFresh(cjsEntry.modulePath);
  const cjsBridge = assertPrivateRootRequestBridge(
    cjsExports,
    cjsEntry.entrypoint
  );
  const crossEntrypointRows = {
    create: packageRows.createResult,
    update: packageRows.updateResult,
    unmount: packageRows.unmountResult
  };
  assert.equal(
    cjsBridge.canConsumePrivateRootLifecycleExecutionEvidence(
      crossEntrypointRows
    ),
    false
  );
...
test("react-test-renderer private TestInstance wrapper requires accepted lifecycle evidence", () => {
  for (const entry of entrypoints) {
    const moduleExports = loadFresh(entry.modulePath);
    const bridge = assertPrivateRootRequestBridge(
      moduleExports,
    ...
    assertPrivateTestInstanceLifecycleGate(initialGate, entry.entrypoint, {
      acceptedLifecycleEvidenceAvailable: false,
      currentRootRequest: createRequest,
      exposesPrivateQueryDiagnostics: false,
      rootRequest: createRequest
    });
    assertPrivateTestInstanceLifecycleGate(
    ...
    const rows = completePrivateRootLifecycleExecutionRowsForRenderer(
      moduleExports,
      bridge,
      renderer
    );
    ...
    assertPrivateRootLifecycleExecutionEvidence(lifecycleEvidence, {
      ...
      entrypoint: entry.entrypoint,
      updateResult: rows.updateResult,
      unmountResult: rows.unmountResult
    });
    assert.equal(
      bridge.canConsumePrivateRootLifecycleExecutionEvidence({
        create: rows.createResult,
        update: rows.updateResult,
        unmount: rows.unmountResult
      }),
    ...
    assertPrivateTestInstanceWrapperSkeleton(record, entry.entrypoint, {
      bridge,
      lifecycleEvidence,
      rootRequest: rows.unmountResult.request
    });
...
test("react-test-renderer CJS development private TestInstance query consumes accepted native create/update execution records", () => {
  ...
  const [createRequest] = bridge.getRendererRootRequests(renderer);
  const createAdmission = bridge.getRootCreateRouteAdmission(createRequest);
  const initialGate = Object.getOwnPropertyDescriptor(
    renderer,
    privateTestInstanceWrapperRecordSymbol
  ).value;
  assertPrivateTestInstanceLifecycleGate(initialGate, entry.entrypoint, {
...
test("react-test-renderer CJS development private TestInstance class-root query evidence stays private", () => {
  ...
  const { record, renderer, updateResult } =
    createAcceptedTestInstanceLifecycle(moduleExports, bridge, {
      element: { props: { children: "hello" }, type: "span" },
      updateElements: [{ props: { children: "goodbye" }, type: "span" }]
    });
...
function createPrivateRootLifecycleExecutionRows(
  ...
  return completePrivateRootLifecycleExecutionRowsForRenderer(
    moduleExports,
    bridge,
    renderer,
    options
...
function createAcceptedTestInstanceLifecycle(
  moduleExports,
  bridge,
  options = {}
) {
  const rows = createPrivateRootLifecycleExecutionRows(
    moduleExports,
    bridge,
    options
  );
...
function completePrivateRootLifecycleExecutionRowsForRenderer(
  moduleExports,
  bridge,
  renderer,
  options = {}
...
function assertPrivateCreateRouteAdmissionGate(gate, label) {
  ...
  if (gate.consumesCurrentRustRootFinishedWorkIdentity === true) {
    expectedAcceptedRustApis.splice(
      2,
      0,
  ...
  if (supportsCreateHostOutputHandoff) {
    ...
    if (gate.consumesCurrentRustRootFinishedWorkIdentity !== undefined) {
      assert.equal(
        gate.consumesCurrentRustRootFinishedWorkIdentity,
        true,
        label
      );
      assert.equal(
        gate.requiresCommitCurrentMatchesRenderFinishedWork,
        true,
        label
      );
    }
...
function assertPrivateRootCreatePreflightConsumption(
  ...
  assert.equal(
    Object.isFrozen(
      consumed.sourceDiagnostic.workLoopFinishedWorkPreflight.finishedWork
    ),
    true
  );
  assert.notDeepEqual(
    consumed.sourceDiagnostic.workLoopFinishedWorkPreflight.previousCurrent,
    consumed.sourceDiagnostic.workLoopFinishedWorkPreflight.finishedWork
  );
  assert.equal(
...
function assertPrivateCreateNativeBridgeHostOutputHandoff(handoff, request) {
  ...
  assert.equal(Object.isFrozen(handoff.workLoopFinishedWorkPreflight), true);
  assert.equal(Object.isFrozen(handoff.renderFinishedWork), true);
  assert.equal(Object.isFrozen(handoff.commitCurrent), true);
  assert.deepEqual(
...
function createRustUnmountNativeBridgeCleanupHandoffSource(
  ...
  const refCleanupReturnCount =
    passiveRefCleanupOrder.refCleanupReturnCount ??
    passiveRefCleanupOrder.ref_cleanup_return_count;
  const passiveDestroyCount =
    passiveRefCleanupOrder.passiveDestroyCount ??
    passiveRefCleanupOrder.passive_destroy_count;
  ...
  const nativeCleanupAfterRefAndPassiveOrdering =
    passiveRefCleanupOrder.nativeCleanupAfterRefAndPassiveOrdering ??
    passiveRefCleanupOrder.native_cleanup_after_ref_and_passive_ordering;
  return {
    ...
    root_request_sequence: request.requestSequence,
    root: request.rootId,
    ...
    host_node_cleanup_count: hostNodeCleanupCount,
    ref_cleanup_return_count: refCleanupReturnCount,
    passive_destroy_count: passiveDestroyCount,
    cleanup_order_record_count: cleanupOrderRecordCount,
    native_cleanup_after_ref_and_passive_ordering:
      nativeCleanupAfterRefAndPassiveOrdering,
    minimal_tree_cleanup_handoff:
      refCleanupReturnCount === 0 &&
      passiveDestroyCount === 0 &&
      cleanupOrderRecordCount === hostNodeCleanupCount &&
      nativeCleanupAfterRefAndPassiveOrdering === true,
    ...
    passive_ref_cleanup_order: passiveRefCleanupOrder,
...
function assertRustCanaryMetadata(metadata, label) {
  ...
  if (metadata.rootCreateRouteAdmission !== undefined) {
    ...
    if (admission.hostOutputHandoffRecord !== undefined) {
      ...
      if (
        admission.consumesCurrentRustRootFinishedWorkIdentity !== undefined
      ) {
        assert.equal(
          admission.consumesCurrentRustRootFinishedWorkIdentity,
          true
        );
        assert.equal(
          admission.requiresCommitCurrentMatchesRenderFinishedWork,
          true
        );
      }
...
function assertPrivateRoute(privateRoute, expected) {
  ...
  if (privateRoute.createRouteAdmissionGate !== undefined) {
    ...
    if (
      ...
      if (
        privateRoute.consumesCurrentRustRootFinishedWorkIdentity !==
        undefined
      ) {
        assert.equal(
          privateRoute.consumesCurrentRustRootFinishedWorkIdentity,
        ...
        assert.equal(
          privateRoute.requiresCommitCurrentMatchesRenderFinishedWork,
          true
        );
      }
  ...
  if (privateRoute.consumesCurrentRustRootFinishedWorkIdentity === true) {
    expectedAcceptedRustApis = [
      ...expectedAcceptedRustApis.slice(0, 2),
      "TestRendererRoot::describe_private_root_create_preflight_from_render_for_canary",
      ...expectedAcceptedRustApis.slice(2)
    ];
  }
...
function assertPrivateToTreeFacadeGate(gate, entrypoint) {
  ...
  if (entrypoint.includes("/cjs/")) {
    assert.equal(
      gate.privateUnmountFinishedWorkIdentityGateAvailable,
      true,
      entrypoint
    );
    assert.equal(
      gate.unmountNativeExecutionFinishedWorkIdentityAdmissionWorker,
      "worker-733-test-renderer-unmount-finished-work-identity",
      entrypoint
    );
    assert.equal(
      gate.unmountNativeExecutionRequiresFinishedWorkIdentity,
      true,
      entrypoint
    );
    assert.equal(gate.rejectsStaleUnmountFinishedWorkIdentity, true, entrypoint);
    assert.equal(
      gate.requiresUnmountDeletionCleanupHandoffEvidence,
      true,
      entrypoint
    );
  }
...
function assertPrivateToTreeFacade(record, entrypoint) {
  ...
  if (entrypoint.includes("/cjs/")) {
    assert.equal(
      record.privateUnmountFinishedWorkIdentityGateAvailable,
      true,
      entrypoint
    );
    assert.equal(
      record.unmountNativeExecutionFinishedWorkIdentityAdmissionWorker,
      "worker-733-test-renderer-unmount-finished-work-identity",
      entrypoint
    );
    assert.equal(
      record.unmountNativeExecutionRequiresFinishedWorkIdentity,
      true,
      entrypoint
    );
    assert.equal(record.rejectsStaleUnmountFinishedWorkIdentity, true, entrypoint);
    assert.equal(
      record.requiresUnmountDeletionCleanupHandoffEvidence,
      true,
      entrypoint
    );
  }
...
function assertPrivateTestInstanceLifecycleGate(record, entrypoint, expected) {
  assert.equal(Object.isFrozen(record), true, entrypoint);
  assert.equal(
    record.kind,
  ...
  assert.equal(
    record.id,
    "react-test-renderer-private-test-instance-root-lifecycle-gate-record",
    entrypoint
  );
  assert.equal(
    record.diagnosticName,
    privateTestInstanceLifecycleGateDiagnosticName,
    entrypoint
  );
  assert.equal(
    record.status,
    expected.exposesPrivateQueryDiagnostics
  ...
  assert.equal(record.rootRequest, expected.rootRequest, entrypoint);
  assert.equal(
    record.currentRootRequest,
    expected.currentRootRequest,
    entrypoint
  );
  assert.equal(record.rootHandle, expected.rootRequest.rootHandle, entrypoint);
  assert.equal(record.rootId, expected.rootRequest.rootId, entrypoint);
  assert.equal(record.rootSequence, expected.rootRequest.rootSequence, entrypoint);
  assert.equal(
    record.requestedRootRequestSequence,
    expected.rootRequest.requestSequence,
    entrypoint
  );
  assert.equal(
    record.currentRootRequestSequence,
    expected.currentRootRequest.requestSequence,
    entrypoint
  );
  assert.equal(
    record.sourceLifecycleDiagnosticName,
    privateRootLifecycleExecutionDiagnosticName,
    entrypoint
  );
  assert.equal(
    record.sourceLifecycleStatus,
    privateRootLifecycleExecutionStatus,
    entrypoint
  );
  assert.equal(
    record.acceptedLifecycleEvidenceAvailable,
    expected.acceptedLifecycleEvidenceAvailable,
    entrypoint
  );
  assert.equal(
    record.exposesPrivateQueryDiagnostics,
    expected.exposesPrivateQueryDiagnostics,
    entrypoint
  );
  assert.equal(record.requiresSourceOwnedCreateUpdateUnmountEvidence, true);
  assert.equal(record.requiresCurrentRootRequest, true);
  assert.equal(
    typeof record.canAcceptPrivateRootLifecycleExecutionEvidence,
    "function"
  );
  assert.equal(
    typeof record.acceptPrivateRootLifecycleExecutionEvidence,
    "function"
  );
  assert.equal(
    typeof record.getAcceptedPrivateTestInstanceQueryDiagnostics,
    "function"
  );
  assertPrivateTestInstanceLifecycleCompatibilityBlockers(record);
  ...
  assert.equal(gate.status, privateTestInstanceLifecycleGateRequiredStatus);
  assert.equal(
    gate.acceptedStatus,
    privateTestInstanceLifecycleGateAcceptedStatus
  );
  assert.equal(gate.requiresSourceOwnedCreateUpdateUnmountEvidence, true);
  assert.equal(gate.requiresCurrentRootRequest, true);
  assert.equal(gate.validatesEntrypointIdentity, true);
  assert.equal(gate.validatesRootHandleIdentity, true);
  assert.equal(gate.validatesRootRequestCurrentness, true);
  assert.equal(gate.validatesFinishedWorkAndHostOutputSourceRecords, true);
  assert.equal(gate.rejectsReplayLifecycleEvidence, true);
  assert.equal(gate.rejectsStaleUpdateLifecycleEvidence, true);
  assert.equal(gate.rejectsStaleAfterUnmountLifecycleEvidence, true);
  assert.equal(gate.rejectsCrossSurfaceLifecycleEvidence, true);
  assert.equal(gate.rejectsClonedLifecycleRows, true);
  assert.equal(gate.rejectsCallerBuiltLifecycleRows, true);
  assert.equal(gate.rejectsCallerShapedQueryRows, true);
  assertPrivateTestInstanceLifecycleCompatibilityBlockers(gate);
}

function assertPrivateTestInstanceLifecycleCompatibilityBlockers(record) {
  assert.equal(record.publicRootAvailable, false);
  assert.equal(record.publicQueryMethodsAvailable, false);
  assert.equal(record.publicTestInstanceObjectAvailable, false);
  assert.equal(record.publicSerializationAvailable, false);
  assert.equal(record.publicActAvailable, false);
  assert.equal(record.publicSchedulerAvailable, false);
  assert.equal(record.nativeBridgeAvailable, false);
...
function assertPrivateTestInstanceWrapperSkeleton(
  ...
  assert.equal(
    record.privateRootLifecycleExecutionDiagnosticName,
    privateRootLifecycleExecutionDiagnosticName,
    entrypoint
  );
  assert.equal(
    record.privateRootLifecycleExecutionStatus,
    privateRootLifecycleExecutionStatus,
    entrypoint
  );
  assert.equal(record.privateRootLifecycleEvidenceRequired, true, entrypoint);
  assert.equal(record.nativeBridgeAvailable, false, entrypoint);
  assert.equal(record.nativeExecution, false, entrypoint);
  assert.equal(record.compatibilityClaimed, false, entrypoint);
  ...
  if (bridgeRouted) {
    ...
    assert.equal(
      record.consumesPrivateRootLifecycleExecutionEvidence,
      true,
      entrypoint
    );
    assert.equal(record.privateRootLifecycleEvidenceAccepted, true, entrypoint);
    assert.equal(
      record.privateRootLifecycleExecutionEvidence,
      options.lifecycleEvidence,
      entrypoint
    );
    assert.equal(
      record.sourceLifecycleDiagnosticName,
    ...
    assert.deepEqual(
      record.sourceLifecycleRequestSequences,
      options.lifecycleEvidence.requestSequences,
      entrypoint
    );
    ...
    assert.equal(
      record.rootRequestOperation,
      options.rootRequest.operation,
      entrypoint
    );
    assert.equal(
...
function assertPrivateTestInstanceRootBridgeMetadata(
  ...
  assert.equal(metadata.privateRootLifecycleGate.diagnosticName, privateTestInstanceLifecycleGateDiagnosticName, entrypoint);
  assert.equal(
    metadata.privateRootLifecycleExecutionEvidence,
    lifecycleEvidence,
    entrypoint
  );
  assert.equal(metadata.privateRootLifecycleEvidenceAccepted, true, entrypoint);
  assert.equal(
    metadata.sourceLifecycleDiagnosticName,
    privateRootLifecycleExecutionDiagnosticName,
    entrypoint
  );
  assert.equal(
    metadata.sourceLifecycleStatus,
    privateRootLifecycleExecutionStatus,
    entrypoint
  );
  assert.deepEqual(
    metadata.sourceLifecycleRequestSequences,
    lifecycleEvidence.requestSequences,
    entrypoint
  );
...
function assertPrivateTestInstanceQueryBridgePreflight(
  ...
  assert.equal(
    preflight.privateRootLifecycleExecutionEvidence,
    owner.privateRootLifecycleExecutionEvidence,
    entrypoint
  );
  assert.equal(preflight.privateRootLifecycleEvidenceAccepted, true, entrypoint);
  assert.equal(
    preflight.sourceLifecycleDiagnosticName,
    privateRootLifecycleExecutionDiagnosticName,
    entrypoint
  );
  assert.equal(
    preflight.sourceLifecycleStatus,
    privateRootLifecycleExecutionStatus,
    entrypoint
  );
  assert.deepEqual(
    preflight.sourceLifecycleRequestSequences,
    owner.sourceLifecycleRequestSequences,
    entrypoint
  );
  assert.equal(
  ...
  assert.equal(
    preflight.consumesPrivateRootLifecycleExecutionEvidence,
    true,
    entrypoint
  );
...
function assertActSchedulerGate(gate, entrypoint) {
  ...
  const isCjsDevelopment = entrypoint === cjsDevelopmentEntrypoint;
  ...
  const expectedAcceptedPrerequisiteIds = cjsPrivateMetadataParity
    ? isCjsDevelopment
      ...
      : acceptedPrivateActFlushCjsDevelopmentPrerequisiteIds.filter(
          (id) => id !== actNestedScopePassiveFlushPrerequisiteId
        )
    : isCjs
  ...
  const expectedReactQueueDiagnosticRecordIds = cjsPrivateMetadataParity
    ? isCjsDevelopment
      ...
      : actSchedulerCjsDevelopmentReactQueueDiagnosticRecordIds.filter(
          (id) => id !== actNestedScopePassiveFlushRecordId
        )
  ...
  if (cjsPrivateMetadataParity) {
    ...
    assert.equal(
      ...
      isCjsDevelopment ? true : undefined
    );
    assert.equal(
      gate.privateNestedScopePassiveFlushDiagnosticId,
      isCjsDevelopment ? actNestedScopePassiveFlushRecordId : undefined
    );
    assert.equal(
      gate.privateNestedScopePassiveFlushPrerequisiteId,
      isCjsDevelopment ? actNestedScopePassiveFlushPrerequisiteId : undefined
    );
  }
  ...
  if (cjsPrivateMetadataParity) {
    ...
    if (isCjsDevelopment) {
      const nestedScopePassiveFlushRecord =
        gate.recognizedSchedulerReactActQueueDiagnostics.find(
          (record) => record.id === actNestedScopePassiveFlushRecordId
        );
      ...
      assert.equal(
        nestedScopePassiveFlushRecord.publicActScopeDepthTrackingAvailable,
        false
      );
      assert.equal(
        nestedScopePassiveFlushRecord.publicNestedActQueueReuseAvailable,
        false
      );
      ...
      assert.equal(nestedScopePassiveFlushRecord.compatibilityClaimed, false);
    } else {
      assert.equal(
        gate.recognizedNestedScopePassiveFlushDiagnostics,
        undefined
