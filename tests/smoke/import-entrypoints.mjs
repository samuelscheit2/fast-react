  ...
  const probeSource = `
    ...
    function assertNativeKeys(moduleExports, label) {
      ...
      for (const key of Reflect.ownKeys(moduleExports)) {
        ...
        if (acceptedNativeDiagnosticRuntimeKeys.has(key)) {
          continue;
        }
        assert.equal(
    ...
    function assertNativeDiagnostics(moduleExports, label) {
      ...
      assert.equal(
        ...
        label + ' worker-thread teardown status'
      );
      assert.equal(
        workerThreadTeardownGate.batchGateStatus,
        'validated-native-root-bridge-batched-json-transport-records',
        label + ' worker-thread teardown batch mirror status'
      );
      assert.equal(
        workerThreadTeardownGate.crossEnvironmentTeardownGateStatus,
        'diagnosed-native-root-bridge-cross-environment-teardown-isolation',
        label + ' worker-thread teardown cross-environment mirror status'
      );
      assert.deepEqual(
        ...
        label + ' worker-thread teardown row ids'
      );
      assert.deepEqual(
        ...
        label + ' worker-thread teardown row errors'
      );
      assert.deepEqual(
        ...
        label + ' worker-thread teardown boundary errors'
      );
      for (const row of workerThreadTeardownGate.rows) {
        assert.deepEqual(
          Object.keys(row),
          workerThreadTeardownGate.workerThreadTeardownDiagnosticRowFields,
          label + ' worker-thread teardown row fields ' + row.id
        );
        assertNoExecution(row, label + ' worker-thread teardown ' + row.id);
      }
      assertNoExecution(
        workerThreadTeardownGate,
        label + ' worker-thread teardown gate'
      );
      assert.equal(
        workerThreadTeardownGate.publicNativeCompatibility,
        false,
        label + ' worker-thread teardown public compatibility'
      );

      assert.equal(
        workerThreadExecutablePreflight.preflightStatus,
        'preflighted-native-root-bridge-worker-thread-teardown-boundary',
        label + ' worker-thread executable preflight status'
      );
      assert.equal(
        ...
        label + ' worker-thread executable preflight scope'
      );
      assert.equal(
        workerThreadExecutablePreflight.workerThreadId,
        764,
        label + ' worker-thread executable preflight worker id'
      );
      assert.equal(
        workerThreadExecutablePreflight.peerEnvironmentId,
        1764,
        label + ' worker-thread executable preflight peer environment'
      );
      assert.equal(
        workerThreadExecutablePreflight.transportWorkerThreadTeardownGateStatus,
        workerThreadTeardownGate.workerThreadTeardownGateStatus,
        label + ' worker-thread executable preflight transport mirror'
      );
      assert.equal(
        workerThreadExecutablePreflight.crossEnvironmentTeardownRowCount,
        teardownGate.rows.length,
        label + ' worker-thread executable preflight teardown row count'
      );
      assert.deepEqual(
        ...
        label + ' worker-thread executable preflight row ids'
      );
      assert.deepEqual(
        ...
        label + ' worker-thread executable preflight stale evidence'
      );
      assert.deepEqual(
        ...
        label + ' worker-thread executable preflight rejected rows'
      );
      assert.deepEqual(
        workerThreadExecutablePreflight.rows.map(
        ...
        label + ' worker-thread executable preflight peer rows'
      );
      assert.deepEqual(
        workerThreadExecutablePreflight.rows.map((row) => row.slot),
        [1, 2, 3, 1, 2],
        label + ' worker-thread executable preflight slots'
      );
      for (const row of workerThreadExecutablePreflight.rows) {
        assert.deepEqual(
          Object.keys(row),
          workerThreadExecutablePreflight.executablePreflightRowFields,
          label + ' worker-thread executable preflight row fields ' + row.id
        );
        assert.equal(
          row.nodeWorkerThreadsExecution,
          false,
          label + ' worker-thread executable preflight worker execution ' + row.id
        );
        assert.equal(
          row.napiCleanupHookExecution,
          false,
          label + ' worker-thread executable preflight cleanup hook ' + row.id
        );
        assertNoExecution(
          row,
          label + ' worker-thread executable preflight ' + row.id
        );
        assert.equal(
          row.publicNativeCompatibility,
          false,
          label + ' worker-thread executable preflight compatibility ' + row.id
        );
      }
      ...
      assert.deepEqual(
        ...
        [2, 1, 2, 1],
        label + ' cleanup-hook registration order'
      );
      assert.deepEqual(
        workerThreadCleanupHookPreflight.rows.map(
        ...
        [1, 2, 1, 2],
        label + ' cleanup-hook expected execution order'
      );
      assert.deepEqual(
        workerThreadCleanupHookPreflight.rows.map(
        ...
        [1, 2, null, null],
        label + ' cleanup-hook observed execution order'
      );
      assert.deepEqual(
        workerThreadCleanupHookPreflight.rows.map((row) => row.status),
        ['accepted', 'accepted', 'rejected', 'rejected'],
        label + ' cleanup-hook row statuses'
      );
      assert.deepEqual(
        ...
        label + ' cleanup-hook rejection codes'
      );
      assert.deepEqual(
        workerThreadCleanupHookPreflight.rows.map(
        ...
        label + ' cleanup-hook canonical evidence'
      );
      assert.deepEqual(
        workerThreadCleanupHookPreflight.rows.map(
        ...
        label + ' cleanup-hook stale or forged rejections'
      );
      for (const row of workerThreadCleanupHookPreflight.rows) {
        assert.deepEqual(
          Object.keys(row),
          workerThreadCleanupHookPreflight.cleanupHookPreflightRowFields,
          label + ' cleanup-hook row fields ' + row.id
        );
        assert.equal(
          row.nodeWorkerThreadsExecution,
          false,
          label + ' cleanup-hook worker execution ' + row.id
        );
        assert.equal(
          row.napiCleanupHookExecution,
          false,
          label + ' cleanup-hook execution ' + row.id
        );
        assertNoExecution(row, label + ' cleanup-hook ' + row.id);
        assert.equal(
          row.publicNativeCompatibility,
          false,
          label + ' cleanup-hook compatibility ' + row.id
        );
      }
      assert.equal(
      ...
      assertNoExecution(
        ...
        label + ' cleanup-hook preflight'
      );
      assert.equal(
        workerThreadCleanupHookPreflight.publicNativeCompatibility,
        false,
        label + ' cleanup-hook compatibility'
      );
    }
