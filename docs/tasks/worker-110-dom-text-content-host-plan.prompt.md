...
Objective:
Produce a report-only plan for DOM text-content host behavior, including `shouldSetTextContent`, text node creation/update/reset, element text-content shortcuts, namespace interactions, `dangerouslySetInnerHTML` exclusions, and the tests needed before a minimal DOM host can render text reliably.
...
- Anchor the plan in merged workers 040, 051, 055, 061, 062, 063, 091, and 106 if present.
- Treat workers 064, 088, 089, and 105 as potentially unmerged unless their reports/oracles are present in your worktree; label any dependency on them as provisional.
- Keep this plan below controlled inputs, event dispatch, hydration, resources, and public root facade work.
- Specify exact future source files, edge-case tests, conformance oracle gaps, and completion gates.
...
- Record progress in `worker-progress/worker-110-dom-text-content-host-plan.md`.
