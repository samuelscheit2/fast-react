...
Write scope:
`tests/conformance/src/react-dom-form-actions-*.mjs`
`tests/conformance/scripts/*react-dom-form-actions*.mjs`
...
- Keep filenames uniquely under the `react-dom-form-actions` prefix.
- Generate evidence from exact React DOM 19.2.6 packages using existing conformance patterns where possible.
- Do not claim full form-action semantics if the probe only covers rootless or hook-boundary behavior.
...
- Record progress in `worker-progress/worker-060-react-dom-form-actions-oracle.md`.
