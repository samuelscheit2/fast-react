...
Objective:
Add deterministic React DOM 19.2.6 style prop and `dangerouslySetInnerHTML` oracle files, covering serialization, numeric unit behavior, CSS custom properties, style removal, invalid style warnings, inner HTML shape validation, and update/removal behavior.

Write scope:
`tests/conformance/src/dom-style-dangerous-html-*.mjs`
`tests/conformance/scripts/*dom-style-dangerous-html*.mjs`
...
- Keep filenames uniquely under the `dom-style-dangerous-html` prefix.
...
- Record progress in `worker-progress/worker-062-dom-style-dangerous-html-oracle.md`.
