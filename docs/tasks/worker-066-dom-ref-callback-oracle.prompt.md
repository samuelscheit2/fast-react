...
Objective:
Add deterministic React DOM 19.2.6 ref callback attach/detach ordering oracle files for host nodes, callback cleanup returns, object refs, StrictMode-relevant observable behavior where deterministic, updates, unmounts, and error propagation.

Write scope:
`tests/conformance/src/dom-ref-callback-*.mjs`
...
- Keep filenames uniquely under the `dom-ref-callback` prefix.
...
- Record progress in `worker-progress/worker-066-dom-ref-callback-oracle.md`.
