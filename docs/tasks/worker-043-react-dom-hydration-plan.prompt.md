...
- Use worker 008, worker 033, worker 042 if available in local history, and React DOM 19.2.6 source/package evidence. If worker 042 is still running, proceed from worker 033 and clearly mark cross-dependencies.
- Cover `hydrateRoot`, hydratable element/text/Suspense/Activity/form marker matching, mismatch diagnostics, recoverable errors, dehydrated boundary hide/unhide/clear operations, event replay, explicit hydration targets, and Fizz marker compatibility.
- Keep hydration separate from generic `createRoot` rendering and from server/Fizz output generation while documenting their contracts.
...
- Record progress in `worker-progress/worker-043-react-dom-hydration-plan.md`.
