...
Objective:
Add deterministic React DOM 19.2.6 attribute/property serialization and mutation oracle files for common host props, booleanish attributes, custom attributes, warnings, unknown props, `className`, `htmlFor`, data/aria props, and removal/update behavior.

Write scope:
`tests/conformance/src/dom-attribute-property-*.mjs`
`tests/conformance/scripts/*dom-attribute-property*.mjs`
...
- Keep filenames uniquely under the `dom-attribute-property` prefix.
...
- Record progress in `worker-progress/worker-061-dom-attribute-property-oracle.md`.
