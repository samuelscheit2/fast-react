...
Objective:
Add deterministic React DOM 19.2.6 controlled input/select/textarea oracle files, covering controlled/uncontrolled warnings, value/defaultValue behavior, checked/defaultChecked behavior, select multiple behavior, textarea children behavior, and update behavior where practical.

Write scope:
`tests/conformance/src/dom-controlled-input-*.mjs`
`tests/conformance/scripts/*dom-controlled-input*.mjs`
...
- Keep filenames uniquely under the `dom-controlled-input` prefix.
- Generate evidence from exact React DOM 19.2.6 packages using existing conformance patterns where possible.
- Prefer observable DOM output and warnings over source-derived assumptions.
...
- Record progress in `worker-progress/worker-064-dom-controlled-input-oracle.md`.
