...
Objective:
Produce a report-only DOM events, event priority, and hydration replay plan for Fast React.
...
- Map delegated root/portal listeners, event plugin dispatch, discrete/continuous/default/idle priority, current update priority, event replay for hydration, selection/focus/beforeinput/composition/scroll/pointer/wheel/form events, and scheduler/lane boundary implications.
- Keep public `scheduler` priorities separate from internal lane bitsets.
...
- Record progress in `worker-progress/worker-041-dom-events-priority-plan.md`.
