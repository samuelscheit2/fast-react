...
Objective:
Add deterministic scheduler variant and deep-import inventory/oracle files for `scheduler@0.27.0`.

Write scope:
`tests/conformance/src/scheduler-variant-*.mjs`
...
- Use pinned `scheduler@0.27.0` package evidence from worker 034.
- Cover `scheduler/unstable_mock`, `scheduler/unstable_post_task`, `index.native.js`, package metadata, and physical deep CJS import exposure caused by the absence of an exports map.
- Decide in the report which surfaces should be first-milestone compatibility gates and which should remain documented gaps.
- Do not implement scheduler package code.
...
- Record progress in `worker-progress/worker-039-scheduler-variant-oracles.md`.
...
- Summarize variant/deep-import coverage and recommended compatibility gates.
- List changed files.
- List commands run.
