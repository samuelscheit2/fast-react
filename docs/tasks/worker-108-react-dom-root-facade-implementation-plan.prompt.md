...
Objective:
Produce a report-only implementation plan for the React DOM root facade slice, including `createRoot`, `hydrateRoot`, root object shape, `render`, `unmount`, option validation, marker ownership, and how the JS facade should call future reconciler/root APIs without doing DOM mutation directly.
...
- Anchor the plan in merged workers 033, 036, 044, 055, 058, 079, 080, 088 if present, 090, 092, 093, 094, and 095.
- Treat workers 046, 049, 088, and 089 as potentially unmerged unless their reports/oracles are present in your worktree; label any dependency on them as provisional.
- Keep this facade slice separate from DOM mutation host implementation, event dispatch, controlled forms, resources, Fizz, and real hydration matching.
- Specify exact future source files, public API behavior gates, smoke/conformance tests, and blockers before compatibility claims.
...
- Record progress in `worker-progress/worker-108-react-dom-root-facade-implementation-plan.md`.
