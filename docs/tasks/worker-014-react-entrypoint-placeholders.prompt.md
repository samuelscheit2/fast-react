...
After setting the goal, call get_goal if available and record the active goal status/objective in your report, then read `WORKER_BRIEF.md`, `MASTER_PLAN.md`, `MASTER_PROGRESS.md`, `worker-progress/worker-004-api-inventory.md`, `worker-progress/worker-010-initial-scaffold.md`, and `worker-progress/worker-011-core-element-model.md` if it exists in your worktree. Do not read `ORCHESTRATOR.md`; it is for the orchestrator role, not workers.
