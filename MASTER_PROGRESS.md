...
| --- | --- | --- | --- |
| worker-001-architecture | needs tmux relaunch after CLI usage-limit failure | Test Rust core and renderer-boundary architecture hypotheses | `worker-progress/worker-001-architecture.md` |
| worker-002-conformance | needs tmux relaunch; managed-subagent output quarantined | Design React 19.2.6 compatibility inventory and test strategy | `worker-progress/worker-002-conformance.md` |
| worker-003-scaffold | needs tmux relaunch; managed-subagent output quarantined | Propose Cargo workspace, JS package scaffold, and worktree task split | `worker-progress/worker-003-scaffold.md` |
